...
export const useWritingActions = ({
    ...
    const getLastName = (authorName: string): string => {
        ...
        // 空格分隔的西文名（First Last 格式，无逗号），取最后一个词作为姓（Family Name/Surname）
    ...
    const getCitationTag = (res: Literature) => {
        ...
        // 消歧：检查同项目中是否有同姓同年的其他文献（大小写不敏感）
        const projectRes = (resources || []).filter(r => r.projectId === selectedProject?.id);
        const lastNameLower = lastName.toLowerCase();
        const sameGroup = projectRes.filter(r => {
            if (r.type === '专利' || !r.year || !r.authors?.length) return false;
            return getLastName(r.authors[0]).toLowerCase() === lastNameLower && r.year === res.year;
        });
        let suffix = '';
        if (sameGroup.length > 1) {
            const idx = sameGroup.findIndex(r => r.id === res.id);
            if (idx >= 0) suffix = String.fromCharCode(97 + idx); // a, b, c...
        }
        return `(${lastName} et al., ${res.year}${suffix})`;
    };
    ...
    const handleFindToken = (tokenType: 'Fig' | 'Table' | 'Math' | 'Log' | 'Cite', id: string) => {
        ...
        if (tokenType === 'Cite') {
            ...
            if (res.type === '专利') {
                ...
                regex = new RegExp(`\\(?${escapedName}\\s+et\\s+al\\.?\\s*,?\\s*${res.year}[a-z]?\\)?`, 'gi');
            }
        } else {
    ...
    const handleEditorDoubleClick = (e: React.MouseEvent<HTMLTextAreaElement>) => {
        ...
        // 自动扩展选区：从光标位置向前找 ( 或 [，向后找 ) 或 ]
        let expandedSelection = '';
        // 尝试匹配 (...) 包围的引用标签
        let openIdx = -1;
        for (let i = cursorPos; i >= Math.max(0, cursorPos - 80); i--) {
            if (text[i] === '(') { openIdx = i; break; }
            if (text[i] === ')' && i < cursorPos) break; // 光标不在括号内
        }
        if (openIdx >= 0) {
            let closeIdx = -1;
            for (let i = cursorPos; i < Math.min(text.length, cursorPos + 80); i++) {
                if (text[i] === ')') { closeIdx = i; break; }
                if (text[i] === '(' && i > cursorPos) break;
            }
            if (closeIdx > openIdx) {
                expandedSelection = text.substring(openIdx, closeIdx + 1);
            }
        }
        // 尝试匹配 [...] 包围的标签
        if (!expandedSelection) {
            let openBracket = -1;
            for (let i = cursorPos; i >= Math.max(0, cursorPos - 80); i--) {
                if (text[i] === '[') { openBracket = i; break; }
                if (text[i] === ']' && i < cursorPos) break;
            }
            if (openBracket >= 0) {
                let closeBracket = -1;
                for (let i = cursorPos; i < Math.min(text.length, cursorPos + 80); i++) {
                    if (text[i] === ']') { closeBracket = i; break; }
                    if (text[i] === '[' && i > cursorPos) break;
                }
                if (closeBracket > openBracket) {
                    expandedSelection = text.substring(openBracket, closeBracket + 1);
                }
            }
        }

        const selection = expandedSelection || text.substring(textarea.selectionStart, textarea.selectionEnd).trim();
        if (!selection) return;
        ...
        const citeMatch = selection.match(/^\(([^)]+?et\s+al\.?,\s*(\d{4})([a-z]?))\)$/i);
        if (citeMatch) {
            const lastName = citeMatch[1].split(/[\s,]+/)[0].toLowerCase();
            const year = parseInt(citeMatch[2]);
            const disambigSuffix = citeMatch[3] || '';
            const candidates = resources.filter(r => {
                if (r.projectId !== selectedProjectId || r.year !== year || !r.authors?.length) return false;
                const firstSurname = getLastName(r.authors[0]).toLowerCase();
                return firstSurname === lastName;
            });
    ...
    const triggerProjectWideCitationSync = (project: ResearchProject, nextTables: ProjectTable[]) => {
        ...
        for (const cm of citeMatches) {
            ...
            if (raw.startsWith('[')) {
                ...
                // 学术引文匹配 (Author et al., 2024) 或 (Author et al., 2024a)
                const m = raw.match(/\((.+?)(?:(?:\s+et\s+al\.?|等))?,?\s*(\d{4})([a-z]?)\)/i);
                if (m) {
                    const name = m[1].trim().toLowerCase();
                    const year = parseInt(m[2]);
                    const disambigSuffix = m[3] || '';
                    ...
                    const candidates = projectRes.filter(r => {
                        if (r.year !== year || !r.authors?.length) return false;
                        const firstSurname = getAuthorSurname(r.authors[0]);
