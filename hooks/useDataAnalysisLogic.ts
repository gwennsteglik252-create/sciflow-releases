...
import { ChartDataPoint, ResearchProject, ExperimentLog, Milestone, SampleEntry, AppView, DataSeries, ChartAnnotation, ExperimentFile, AnnotationType, SubplotLayoutMode, SubplotPanel } from '../types';
...
import { detectMetadata, extractChartStyleFromImage, discoverChartTemplate } from '../services/gemini/analysis';
import { useProjectContext } from '../context/ProjectContext';
import { getColumnLetter } from '../types/spreadsheet';
import { useDataAnalysisHistory } from './useDataAnalysisHistory';
...
export interface ChartTemplate {
  id: string;
  name: string;
  type: 'line' | 'bar' | 'scatter' | 'area' | 'box' | 'histogram' | 'contour' | 'heatmap' | 'surface3d' | 'violin' | 'bubble' | 'ternary' | 'polar' | 'waterfallPlotly' | 'parallel' | 'funnel' | 'treemap' | 'sunburst';
  color: string;
...
export const ACADEMIC_TEMPLATES: ChartTemplate[] = [
  ...
  // ── Plotly 高级科研图表模板 ──
  {
    ...
    xLabel: '变量',
    yLabel: '变量',
    description: 'Plotly 热力图，适合展示参数相关矩阵、元素分布。',
    pointShape: 'none',
    gridX: false,
    gridY: false,
    dataType: '矩阵/相关系数',
    dataRequirement: '多列 Y 数据，行列交叉形成矩阵',
    typicalExperiment: 'XPS / 元素分布 / 统计相关',
    isStandard: true
  },
  {
    id: 'plotly-contour-energy',
    name: '能量面等高线图',
    type: 'contour',
    color: '#4338ca',
    ...
    xLabel: 'X 坐标',
    yLabel: 'Y 坐标',
    description: 'Plotly 等高线图，适合展示势能面、浓度分布。',
    pointShape: 'none',
    gridX: false,
    gridY: false,
    dataType: '二维密度/能量分布',
    dataRequirement: 'X 列 + 多列 Y 构成矩阵',
    typicalExperiment: 'DFT 计算 / 传质分析',
    isStandard: true
  },
  {
    id: 'plotly-surface3d-rsm',
    name: '3D 响应面分析',
    type: 'surface3d',
    color: '#7c3aed',
    ...
    font: 12,
    axisLabelFontSize: 16,
    chartTitle: '响应面分析 (RSM)',
    xLabel: '因素 A',
    yLabel: '因素 B',
    description: 'Plotly 3D 曲面，可旋转交互，适合响应面方法。',
    pointShape: 'none',
    gridX: true,
    gridY: true,
    dataType: '三维响应数据',
    dataRequirement: 'X 列 + 多列 Y 构成 Z 矩阵',
    typicalExperiment: 'RSM / 工艺优化 / 材料配方',
    isStandard: true
  },
  {
    id: 'plotly-violin-dist',
    name: '样品分布小提琴图',
    type: 'violin',
    color: '#be185d',
    ...
    xLabel: '样品组',
    yLabel: '测量值',
    description: 'Plotly 小提琴图（含箱线 + 均值线），适合多组样品分布展示。',
    pointShape: 'none',
    gridX: false,
    gridY: true,
    dataType: '多组测量数据',
    dataRequirement: '每个系列代表一组样品的重复测量值',
    typicalExperiment: '力学性能 / 粒径分布 / 生物重复',
    isStandard: true
  },
  {
    id: 'plotly-bubble-3param',
    name: '三参数气泡图',
    type: 'bubble',
    color: '#0369a1',
    ...
    xLabel: '参数 X',
    yLabel: '参数 Y',
    description: 'Plotly 气泡图，气泡大小映射第三维数据（误差棒字段）。',
    pointShape: 'circle',
    pointSize: 8,
    gridX: true,
    gridY: true,
    dataType: '三维关联数据',
    dataRequirement: 'X/Y 数值 + error 字段作为气泡大小',
    typicalExperiment: '催化活性 / 药物筛选 / 材料比较',
    isStandard: true
  },
  {
    id: 'plotly-ternary-alloy',
    name: '合金组分三元相图',
    type: 'ternary',
    color: '#065f46',
    ...
    xLabel: '组分 A',
    yLabel: '组分 B',
    description: 'Plotly 三元相图，适合化学配比、合金组分、催化剂优化。',
    pointShape: 'circle',
    pointSize: 8,
    gridX: false,
    gridY: false,
    dataType: '三组分配比',
    dataRequirement: '3 列 Y 数据代表 A/B/C 三组分',
    typicalExperiment: '合金设计 / 催化剂筛选 / 陶瓷配方',
    isStandard: true
  },
  {
    id: 'plotly-polar-radar',
    name: '雷达/极坐标图',
    type: 'polar',
    color: '#9333ea',
    ...
    xLabel: '维度',
    yLabel: '数值',
    description: 'Plotly 极坐标图，适合多维性能对比、材料评估。',
    pointShape: 'circle',
    pointSize: 5,
    gridX: false,
    gridY: false,
    dataType: '多维评估数据',
    dataRequirement: '每个数据点的 name 为维度名，value 为评分',
    typicalExperiment: '综合评估 / 多指标对比 / 材料筛选',
    isStandard: true
  },
  {
    id: 'plotly-waterfall-process',
    name: '工艺流程瀑布图',
    type: 'waterfallPlotly',
    color: '#166534',
    ...
    xLabel: '工序步骤',
    yLabel: '产率变化',
    description: 'Plotly 瀑布图，自动着色增减项，适合逐步工艺分析。',
    pointShape: 'none',
    gridX: false,
    gridY: true,
    dataType: '逐步增减量',
    dataRequirement: '每个数据点为一步的增减值',
    typicalExperiment: '工艺优化 / 成本分析 / 产率追踪',
    isStandard: true
  },
  {
    id: 'plotly-parallel-multivar',
    name: '多参数平行坐标图',
    type: 'parallel',
    color: '#0e7490',
    ...
    xLabel: '-',
    yLabel: '-',
    description: 'Plotly 平行坐标图，每个系列为一个维度，适合高维数据分析。',
    pointShape: 'none',
    gridX: false,
    gridY: false,
    dataType: '高维多参数',
    dataRequirement: '多个系列，每个系列代表一个参数维度',
    typicalExperiment: '实验设计 / 材料筛选 / 工艺参数优化',
    isStandard: true
  },
  {
    id: 'plotly-funnel-yield',
    name: '产率/转化漏斗图',
    type: 'funnel',
    color: '#7e22ce',
    ...
    xLabel: '产物量',
    yLabel: '反应阶段',
    description: 'Plotly 漏斗图，可视化多级筛选/转化过程。',
    pointShape: 'none',
    gridX: false,
    gridY: false,
    dataType: '阶段递减数据',
    dataRequirement: '各阶段名称 + 对应数值（递减排列效果最佳）',
    typicalExperiment: '药物筛选 / 合成路径 / 纯化步骤',
    isStandard: true
  },
  {
    id: 'plotly-treemap-composition',
    name: '组分占比矩形树图',
    type: 'treemap',
    color: '#b45309',
    ...
    xLabel: '-',
    yLabel: '-',
    description: 'Plotly 矩形树图，面积表示占比，适合成分/分类数据。',
    pointShape: 'none',
    gridX: false,
    gridY: false,
    dataType: '分类占比',
    dataRequirement: '名称 + 对应数值',
    typicalExperiment: '元素组成 / 材料分类 / 分子量分布',
    isStandard: true
  },
  {
    id: 'plotly-sunburst-hierarchy',
    name: '层级结构旭日图',
    type: 'sunburst',
    color: '#dc2626',
    ...
    xLabel: '-',
    yLabel: '-',
    description: 'Plotly 旭日图，多系列自动生成层级，适合分类层级数据。',
    pointShape: 'none',
    gridX: false,
    gridY: false,
    dataType: '层级占比',
    dataRequirement: '多个系列 → 自动形成层级分组',
    typicalExperiment: '分子分类 / 合成路径层级 / 材料体系',
    isStandard: true
  }
];
...
export const useDataAnalysisLogic = (projects: ResearchProject[], onUpdateProject: (p: ResearchProject) => void, setAiStatus: any) => {
  const { showToast, dataAnalysisSession, updateDataAnalysisSession: rawUpdateSession } = useProjectContext();
  const history = useDataAnalysisHistory();

  // 包装 updateDataAnalysisSession：每次更新前推入快照用于 undo
  const updateDataAnalysisSession = React.useCallback((u: any) => {
    history.pushSnapshot(dataAnalysisSession);
    rawUpdateSession(u);
  }, [rawUpdateSession, history, dataAnalysisSession]);
  ...
  const handleUndo = React.useCallback(() => {
    ...
    if (!snapshot) return;
    // 当前状态推入 redo 栈
    history.pushToRedo(dataAnalysisSession);
    history.isUndoingRef.current = true;
    rawUpdateSession(snapshot);
    setTimeout(() => {
      history.isUndoingRef.current = false;
      history.syncHistorySize();
    }, 0);
  }, [history, dataAnalysisSession, rawUpdateSession]);

  const handleRedo = React.useCallback(() => {
    const snapshot = history.redo();
    if (!snapshot) return;
    // 当前状态推入 undo 栈
    history.pushToUndo(dataAnalysisSession);
    history.isUndoingRef.current = true;
    rawUpdateSession(snapshot);
    setTimeout(() => {
      history.isUndoingRef.current = false;
  ...
  const {
    ...
    currentSavedChartId, currentSavedChartName,
    ...
    spreadsheet,
    subplotLayout = 'single' as SubplotLayoutMode,
    subplotPanels = [] as SubplotPanel[],
    activeSubplotId = null as string | null,
    rightYAxisLabel = '' as string,
    rightYDomain = ['auto', 'auto'] as [any, any],
    rightYScale = 'auto' as 'auto' | 'log',
    rightYTickCount = 5,
    rightYLabelPos = { x: 0, y: 0 },
    rightYAxisDivision = 2,
    rightYAxisColor = '#334155' as string
  ...
  const activeTab = ((rawActiveTab as string) === 'vision' ? 'chart' : rawActiveTab) as 'chart' | 'mimic' | 'data';

  const updateSpreadsheet = useCallback((val: any) => updateDataAnalysisSession({ spreadsheet: val }), [updateDataAnalysisSession]);
  ...
  // ── 子图布局 actions ──
  const SUBPLOT_LABELS = ['(a)', '(b)', '(c)', '(d)', '(e)', '(f)'];
  const LAYOUT_PANEL_COUNT: Record<SubplotLayoutMode, number> = {
    'single': 1, '1x2': 2, '2x1': 2, '2x2': 4, '1x3': 3, '3x1': 3, '2x3': 6, '3x2': 6
  };

  const setSubplotLayout = useCallback((mode: SubplotLayoutMode) => {
    const count = LAYOUT_PANEL_COUNT[mode];
    // 如果当前面板数量不够，补足；如果多了，截断
    const currentPanels = subplotPanels || [];
    const newPanels: SubplotPanel[] = [];
    for (let i = 0; i < count; i++) {
      if (currentPanels[i]) {
        newPanels.push({ ...currentPanels[i], label: SUBPLOT_LABELS[i] || `(${String.fromCharCode(97 + i)})` });
      } else {
        newPanels.push({
          id: `subplot_${Date.now()}_${i}`,
          label: SUBPLOT_LABELS[i] || `(${String.fromCharCode(97 + i)})`,
          seriesIds: [],
    ...
    updateDataAnalysisSession({
      subplotLayout: mode,
      subplotPanels: newPanels,
      activeSubplotId: mode === 'single' ? null : (newPanels[0]?.id || null),
    });
  }, [subplotPanels, chartType, updateDataAnalysisSession]);

  const setActiveSubplotId = useCallback((id: string | null) => {
    updateDataAnalysisSession({ activeSubplotId: id });
  }, [updateDataAnalysisSession]);

  const updateSubplotPanel = useCallback((id: string, updates: Partial<SubplotPanel>) => {
    const current = subplotPanels || [];
    updateDataAnalysisSession({
      subplotPanels: current.map((p: SubplotPanel) =>
        p.id === id ? { ...p, ...updates } : p
      ),
    } as any);
  }, [subplotPanels, updateDataAnalysisSession]);

  const assignSeriesToPanel = useCallback((panelId: string, seriesIds: string[]) => {
    updateSubplotPanel(panelId, { seriesIds });
  }, [updateSubplotPanel]);
  ...
  const handleDiscoverTemplate = useCallback(async () => {
    ...
    try {
      const aiResult = await discoverChartTemplate(templateSearchQuery);

      const VALID_TYPES = ['line', 'bar', 'scatter', 'area', 'heatmap', 'contour', 'surface3d', 'violin', 'bubble', 'polar', 'waterfallPlotly', 'parallel', 'funnel', 'treemap', 'sunburst'] as const;
      const rawType = (aiResult.chartType || 'line').toLowerCase();
      const chartTypeResolved = VALID_TYPES.includes(rawType as any) ? rawType : 'line';

      const VALID_SHAPES = ['circle', 'square', 'diamond', 'triangleUp', 'triangleDown', 'cross', 'star', 'none'] as const;
      const rawShape = aiResult.pointShape || 'circle';
      const pointShapeResolved = VALID_SHAPES.includes(rawShape as any) ? rawShape : 'circle';

      const discoveredTpl: ChartTemplate = {
        id: `discovered_${Date.now()}`,
        name: aiResult.name || `${templateSearchQuery} 规范`,
        type: chartTypeResolved as any,
        color: aiResult.colors?.[0] || '#334155',
        colors: aiResult.colors,
        stroke: aiResult.strokeWidth || 2,
        font: 14,
        xLabel: aiResult.xLabel || 'X-Axis',
        yLabel: aiResult.yLabel || 'Y-Axis',
        description: aiResult.description || 'AI 智能建模的学术制图规范',
        pointShape: pointShapeResolved as any,
        pointSize: aiResult.pointSize || 4,
        gridX: aiResult.gridX ?? false,
        gridY: aiResult.gridY ?? false,
        axisBox: aiResult.axisBox ?? true,
        showErrorBar: aiResult.showErrorBar ?? false,
        xScale: aiResult.xScale || 'auto',
        yScale: aiResult.yScale || 'auto',
        aspectRatio: aiResult.aspectRatio || 1.33,
        labelFontFamily: aiResult.fontFamily === 'serif' ? 'serif' : 'inherit',
        tickFontFamily: aiResult.fontFamily === 'serif' ? 'serif' : 'inherit',
        titleFontFamily: aiResult.fontFamily === 'serif' ? 'serif' : 'inherit',
        dataType: aiResult.dataType || templateSearchQuery,
        dataRequirement: aiResult.dataRequirement || '标准双列数据',
        typicalExperiment: aiResult.typicalExperiment || 'Academic Standard',
        isStandard: false
      };
      ...
      showToast({ message: `AI 规范建模完成：${discoveredTpl.name}`, type: 'success' });
    } catch (err) {
      console.error('AI template discovery failed:', err);
      showToast({ message: '模板智能发现失败，请检查 AI 服务配置', type: 'error' });
    } finally {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      try {
        ...
        if (dataPoints.length > 0) {
          ...
          const newColor = colors[(seriesList.length + newSeriesArr.length) % colors.length];
          newSeriesArr.push({
    ...
    if (newSeriesArr.length > 0) {
      const allSeries = [...seriesList, ...newSeriesArr];

      // ── 自动同步到数据表 ──
      const hasError = allSeries.some(s => s.data.some((d: ChartDataPoint) => d.error && d.error !== 0));
      const colsPerSeries = hasError ? 2 : 1;
      const maxRows = Math.max(...allSeries.map(s => s.data.length), 10);

      const newCols: { id: string; name: string; role: 'X' | 'Y' | 'yErr' | 'Label' | 'none' }[] = [
        { id: 'col_x', name: 'A', role: 'X' },
      ];
      allSeries.forEach((s, si) => {
        const yIdx = 1 + si * colsPerSeries;
        newCols.push({ id: `col_y_${si}`, name: getColumnLetter(yIdx), role: 'Y' });
        if (hasError) {
          newCols.push({ id: `col_err_${si}`, name: getColumnLetter(yIdx + 1), role: 'yErr' });
        }
      });
      ...
      for (let r = 0; r < maxRows; r++) {
        const row: string[] = [];
        row.push(allSeries[0]?.data[r]?.name ?? '');
        allSeries.forEach((s) => {
      ...
      // ── 直接从数据计算坐标轴域（避免 setTimeout 闭包过期问题）──
      const allXVals = allSeries.flatMap(s => s.data.map((d: ChartDataPoint) => parseFloat(d.name)).filter((v: number) => !isNaN(v)));
      const allYVals = allSeries.flatMap(s => s.data.map((d: ChartDataPoint) => d.value).filter((v: number) => !isNaN(v)));
      let newXDomain: any = ['dataMin', 'dataMax'];
      let newYDomain: any = ['dataMin', 'dataMax'];
      if (allXVals.length > 0 && allYVals.length > 0) {
        newXDomain = calculateNiceScale(Math.min(...allXVals), Math.max(...allXVals), xTickCount);
        newYDomain = calculateNiceScale(Math.min(...allYVals), Math.max(...allYVals), yTickCount);
      }

      // 一次性原子更新：seriesList + spreadsheet + 坐标轴域
      updateDataAnalysisSession({
        seriesList: allSeries,
        spreadsheet: { columns: newCols, rows: newRows },
        xDomain: newXDomain,
        yDomain: newYDomain,
      });
    }
  ...
  return {
    ...
    actions: {
      ...
      setRightYAxisLabel: (v: string) => updateDataAnalysisSession({ rightYAxisLabel: v } as any),
      setRightYDomain: (v: [any, any]) => updateDataAnalysisSession({ rightYDomain: v } as any),
      setRightYScale: (v: 'auto' | 'log') => updateDataAnalysisSession({ rightYScale: v } as any),
      setRightYTickCount: (v: number) => updateDataAnalysisSession({ rightYTickCount: v } as any),
      setRightYLabelPos: (v: { x: number; y: number }) => updateDataAnalysisSession({ rightYLabelPos: v } as any),
      setRightYAxisDivision: (v: number) => updateDataAnalysisSession({ rightYAxisDivision: v } as any),
      setRightYAxisColor: (v: string) => updateDataAnalysisSession({ rightYAxisColor: v } as any),
      ...
      saveChartToLibrary: async (name: string, folderId?: string, existingId?: string) => {
        let thumbnailUrl = undefined;
        if (chartContainerRef.current) {
          try {
            const targetNode = chartContainerRef.current.querySelector('.lab-chart-responsive') as HTMLElement || chartContainerRef.current;
        ...
        delete currentSession.currentSavedChartId;
        delete currentSession.currentSavedChartName;

        if (existingId) {
          // 覆盖模式：找到已有存档并更新
          const updatedCharts = (savedCharts || []).map((c: any) =>
            c.id === existingId
              ? { ...c, name, timestamp: new Date().toLocaleString(), thumbnailUrl: thumbnailUrl || c.thumbnailUrl, sessionData: currentSession }
              : c
          );
          updateDataAnalysisSession({
            savedCharts: updatedCharts,
            currentSavedChartId: existingId,
            currentSavedChartName: name,
          });
          showToast({ message: `存档 [${name}] 已覆盖更新`, type: 'success' });
        } else {
          // 新建模式
          ...
          updateDataAnalysisSession({
            savedCharts: [newChart, ...(savedCharts || [])],
            currentSavedChartId: newId,
            currentSavedChartName: name,
          });
      ...
      loadChartFromLibrary: (id: string) => {
        ...
        if (chart && chart.sessionData) {
          updateDataAnalysisSession({
            ...chart.sessionData,
            currentSavedChartId: chart.id,
            currentSavedChartName: chart.name,
          });
          showToast({ message: `已成功载入表征存档 [${chart.name}]`, type: 'success' });
        }
      },
      overwriteCurrentChart: async () => {
        if (currentSavedChartId && currentSavedChartName) {
          let thumbnailUrl = undefined;
          if (chartContainerRef.current) {
            try {
              const targetNode = chartContainerRef.current.querySelector('.lab-chart-responsive') as HTMLElement || chartContainerRef.current;
            ...
            } catch (e) { console.error('Failed to capture thumbnail', e); }
          }
          const currentSession = { ...dataAnalysisSession };
          delete currentSession.savedCharts;
          delete currentSession.chartFolders;
          ...
          delete currentSession.currentSavedChartName;
          const updatedCharts = (savedCharts || []).map((c: any) =>
            c.id === currentSavedChartId
              ? { ...c, timestamp: new Date().toLocaleString(), thumbnailUrl: thumbnailUrl || c.thumbnailUrl, sessionData: currentSession }
              : c
          );
          updateDataAnalysisSession({ savedCharts: updatedCharts });
          showToast({ message: `存档 [${currentSavedChartName}] 已覆盖更新`, type: 'success' });
        }
      },
      ...
      exportChart, autoFitDomains, openAxisSettings, setLeftPanelMode, setAspectRatio, setTemplateSearchQuery, handleDiscoverTemplate, handleDiscoverTemplateFromImage, handleSaveDeepAnalysis, applyTemplate, setShowExportPanel, setSubplotLayout, setActiveSubplotId, updateSubplotPanel, assignSeriesToPanel, handleUndo, handleRedo,
