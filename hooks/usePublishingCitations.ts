 ...
 */
export const CITE_REGEX = /(\((?:[^)]|@@SCIFLOW_CURSOR@@)+?,?\s*\d{4}[a-z]?(?:@@CID:[\w-]+@@)?\)|\[(?!(?:Fig|FigRef|Table|Math|Ref):)(?:[^\]]|@@SCIFLOW_CURSOR@@)+?\])/gi;
  ...
  return useMemo(() => {
    ...
    uniqueRawCites.forEach(raw => {
      ...
      if (raw.startsWith('[')) {
        ...
        const match = raw.match(/\((.+?)(?:(?:\s+et\s+al\.?|等))?,?\s*(\d{4})([a-z]?)\)/i);
        if (match) {
          const name = match[1].trim().toLowerCase();
          const year = parseInt(match[2]);
          const disambigSuffix = match[3] || ''; // a, b, c...
          ...
          // 只匹配第一作者的姓（引用标签就是用第一作者姓生成的）
          const candidates = resources.filter(r => {
            if (r.projectId !== project?.id || r.year !== year) return false;
          ...
          if (disambigSuffix && candidates.length > 1) {
            // 有后缀时按字母序号精确选取
            const suffixIdx = disambigSuffix.charCodeAt(0) - 97; // a=0, b=1, c=2...
