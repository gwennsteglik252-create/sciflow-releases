...
export const CANVAS_PRESETS: { name: string; widthMM: number; heightMM: number }[] = [
  { name: 'Nature 单栏', widthMM: 89, heightMM: 89 },
  { name: 'Nature 双栏', widthMM: 183, heightMM: 150 },
  { name: 'Science 单栏', widthMM: 90, heightMM: 90 },
  { name: 'JACS 单栏', widthMM: 84, heightMM: 84 },
  { name: 'JACS 双栏', widthMM: 175, heightMM: 150 },
  { name: 'A4 横向', widthMM: 297, heightMM: 210 },
  { name: 'A4 纵向', widthMM: 210, heightMM: 297 },
  { name: '默认', widthMM: 135, heightMM: 101 },
];

/** mm → px: px = mm / 25.4 * dpi */
export const mmToPx = (mm: number, dpi: number) => Math.round(mm / 25.4 * dpi);

const DEFAULT_CANVAS_CONFIG: CanvasConfig = { widthMM: 135, heightMM: 101, dpi: 300, presetName: '默认' };
const DEFAULT_GAP_CONFIG: GapConfig = { margin: 40, gap: 50 };
...
export const useFigureAssemblyLogic = (savedLibrary: any[]) => {
  ...
  const [canvasConfig, setCanvasConfig] = useState<CanvasConfig>(() => {
    try {
      const saved = localStorage.getItem('sciflow_assembly_canvas_config');
      return saved ? JSON.parse(saved) : DEFAULT_CANVAS_CONFIG;
    } catch { return DEFAULT_CANVAS_CONFIG; }
  });

  const canvasWidth = useMemo(() => mmToPx(canvasConfig.widthMM, canvasConfig.dpi), [canvasConfig]);
  const canvasHeight = useMemo(() => mmToPx(canvasConfig.heightMM, canvasConfig.dpi), [canvasConfig]);

  // ─── 面板间距精确控制（功能6）─────────────────────────────────────────────
  const [gapConfig, setGapConfig] = useState<GapConfig>(() => {
    try {
      const saved = localStorage.getItem('sciflow_assembly_gap_config');
      return saved ? JSON.parse(saved) : DEFAULT_GAP_CONFIG;
    } catch { return DEFAULT_GAP_CONFIG; }
  });
  ...
  useEffect(() => {
    localStorage.setItem('sciflow_assembly_canvas_config', JSON.stringify(canvasConfig));
  }, [canvasConfig]);

  useEffect(() => {
    localStorage.setItem('sciflow_assembly_gap_config', JSON.stringify(gapConfig));
  }, [gapConfig]);
  ...
  const handleCategoryChange = useCallback((id: string, newCategory: string) => {
    setSavedAssemblies(prev => prev.map(s => s.id === id ? { ...s, category: newCategory } : s));
  }, []);
  ...
  return {
    ...
    handleWheel, handleLoadSaved, handleDeleteSaved, handleRenameSaved, handleCategoryChange, handleSaveConfirm, handleDeleteLocalAsset, handleSaveToLibrary,
    // Layer system
