...
import MobileNotificationCenter from './components/Mobile/NotificationCenter';

const AppContent: React.FC = () => {
  const { isOnline, userProfile, appSettings, setAppSettings, projects, mechanismSession, navigate, cloudSync, inventory } = useProjectContext();
  ...
  const smartNotifScanInput = React.useMemo(() => ({
    projectTitle: currentProject?.title || 'SciFlow Pro',
    milestones: (currentProject?.milestones || []).map(m => ({
      id: m.id, title: m.title, status: m.status, dueDate: m.dueDate
    })),
    inventory: (inventory || []).filter(i => i.status !== 'Purchasing').map(i => ({
      id: i.id, name: i.name, quantity: i.quantity, threshold: i.threshold,
      unit: i.unit, category: i.category, status: i.status
    })),
    feedItems: (currentProject?.feedItems || []).map(f => ({
      id: f.id, title: f.title, isRead: f.isRead,
      authors: f.authors, source: f.source, discoveredAt: f.discoveredAt
    }))
  }), [currentProject, inventory]);

  const smartNotifs = useSmartNotifications(smartNotifScanInput);
  const totalUnread = smartNotifs.unreadCount + (inviteNotifs.hasNew ? inviteNotifs.newInvites.length : 0);
  ...
  // 监听移动端通知中心开关事件
  React.useEffect(() => {
    const handler = () => setShowMobileNotifications(prev => !prev);
    window.addEventListener('sciflow_toggle_notifications', handler);
    return () => window.removeEventListener('sciflow_toggle_notifications', handler);
  }, []);
  ...
  const moduleLabels: Record<string, string> = {
    ...
    research_brain: '中心大脑', inception: '战略立项', industry_trends: '行业动态', market_analysis: '市场分析',
    ...
    writing: '写作工坊', assistant: '科研助理', notebook: '科研笔记', companion: '移动伴侣',
  };
  ...
  return (
    <div id="app-root" className={`flex ${isMobileView ? 'flex-col' : ''} h-screen overflow-hidden relative ${isLightMode ? 'theme-light' : 'theme-dark'}`} style={{ backgroundColor: activeTheme.colors.background }}>
      ...
      {/* 桌面端侧边栏 — 移动端隐藏 */}
      {!isMobileView && <aside className={`${isCollapsed ? 'w-20' : 'w-20 lg:w-56'} backdrop-blur-3xl border-r flex flex-col z-50 no-print shadow-2xl transition-all duration-300 ${activeTheme.colors.sidebar} ${activeTheme.colors.sidebarBorder}`}>
        ...
        <Sidebar
          ...
          hiddenNavModules={appSettings.hiddenNavModules}
          navModuleOrder={appSettings.navModuleOrder}
        />
          ...
          <div className="relative">
            <button
              ...
              className={`w-8 h-8 rounded-lg flex items-center justify-center border transition-all active:scale-95 shrink-0 shadow-md relative ${isLightMode
                ? 'bg-slate-50 border-slate-200 text-slate-500 hover:bg-amber-500 hover:text-white'
                : 'bg-slate-700/30 border-slate-600/30 text-slate-400 hover:bg-amber-500 hover:text-white'
                }`}
              title="通知中心"
            >
      ...
      {/* 科研黄历 — 从侧边栏后方平滑滑入（桌面端）/ 从底部滑入（移动端） */}
      <AnimatePresence>
        {showGlobalAlmanac && (
          <motion.div
            initial={isMobileView ? { y: '100%', opacity: 0 } : { x: '-110%', opacity: 0 }}
            animate={isMobileView ? { y: 0, opacity: 1 } : { x: 0, opacity: 1 }}
            exit={isMobileView ? { y: '100%', opacity: 0 } : { x: '-110%', opacity: 0 }}
            transition={{ type: 'spring', damping: 25, stiffness: 200 }}
            className={isMobileView
              ? 'fixed inset-x-0 bottom-0 top-16 bg-white shadow-[0_-10px_40px_rgba(0,0,0,0.15)] z-[9970] overflow-hidden rounded-t-3xl'
              : `absolute top-0 bottom-0 ${isCollapsed ? 'left-20' : 'left-20 lg:left-56'} w-[380px] bg-white shadow-[20px_0_50px_rgba(0,0,0,0.15)] border-r border-slate-200 z-[45] overflow-hidden rounded-r-2xl`
            }
          >
            <AlmanacPanel
      ...
      {/* 移动端顶部 Header — 替代侧边栏的品牌和模块名称 */}
      {isMobileView && (
        <header className={`flex items-center justify-between px-4 h-12 shrink-0 border-b backdrop-blur-xl z-40 ${isLightMode ? 'bg-white/90 border-slate-200' : 'bg-slate-950/90 border-white/10'}`}>
          <div className="flex items-center gap-2.5">
            <div className="w-7 h-7 rounded-lg bg-gradient-to-br from-indigo-600 via-purple-600 to-pink-600 flex items-center justify-center shadow-lg">
              <span className="text-white text-[10px] font-black">S</span>
            </div>
            <span className={`text-sm font-black tracking-tight ${isLightMode ? 'text-slate-800' : 'text-white'}`}>SciFlow</span>
          </div>
          <div className="flex items-center gap-2">
            {!isOnline && <span className="text-[8px] font-black uppercase px-2 py-0.5 rounded-full bg-amber-500/20 text-amber-600">离线</span>}
            <button onClick={() => setIsAiCliOpen(true)} className={`w-8 h-8 rounded-xl flex items-center justify-center transition-colors ${isLightMode ? 'bg-slate-100 text-slate-600' : 'bg-white/10 text-white/70'}`}>
      ...
      <main
        className={`flex-1 h-full min-h-0 p-2 md:p-4 transition-colors duration-500 min-w-0 overflow-hidden relative ${isMobileView ? 'pb-16 overflow-y-auto' : ''}`}
        style={{ backgroundImage: 'radial-gradient(at 0% 0%, rgba(99, 102, 241, 0.1) 0, transparent 40%)' }}
      >
        <div className={`${isMobileView ? 'min-h-full' : 'h-full min-h-0'} flex flex-col overflow-hidden`}>
          <AppRouter route={route} navigate={navigate} openAddProject={() => setModalOpen('addProject', true)} confirmDelete={confirmDelete} />
        </div>
      </main>
      ...
      {isMobileView && (
        <MobileBottomNav
          ...
          doeBadge={activeDoeRuns}
          mechanismBadge={mechanismSession.isProcessing ? 1 : 0}
          notificationBadge={totalUnread}
        />
      )}

      {toast && (
        <div className={`fixed z-[9999] animate-reveal ${isMobileView ? 'top-14 left-3 right-3' : 'bottom-6 right-6'}`}>
          <div className={`flex items-center gap-3 px-5 py-3 rounded-2xl border shadow-2xl backdrop-blur-md ${toast.type === 'error' ? 'bg-rose-500/90 border-rose-400 text-white' :
            toast.type === 'success' ? 'bg-emerald-500/90 border-emerald-400 text-white' :
              'bg-slate-800/90 border-slate-700 text-white'
            }`}>
            ...
            {toast.onAction && toast.actionLabel && (
              <button
                ...
                className="ml-2 px-3 py-1 bg-white/20 hover:bg-white/30 rounded-lg text-[9px] font-black uppercase transition-all shrink-0"
              >
                {toast.actionLabel}
              </button>
            )}
            <button onClick={hideToast} className="ml-1 opacity-70 hover:opacity-100 shrink-0"><i className="fa-solid fa-xmark"></i></button>
          </div>
        </div>
      )}

      <GlobalSearchOverlay />
      {/* VoiceLabCompanion 在移动端隐藏（悬浮窗不适合触摸交互） */}
      {!isMobileView && <VoiceLabCompanion />}
      {!isMobileView && <QuickNoteButton />}

      {/* ═══ 智能通知中心面板 ═══ */}
      {showNotificationCenter && (
        <NotificationCenter
          notifications={smartNotifs.notifications}
          unreadCount={smartNotifs.unreadCount}
          weeklyReports={smartNotifs.weeklyReports}
          currentWeekHasReport={smartNotifs.currentWeekHasReport}
          onMarkAsRead={smartNotifs.markAsRead}
          onMarkAllAsRead={smartNotifs.markAllAsRead}
          onDismiss={smartNotifs.dismissNotification}
          onClearAll={smartNotifs.clearAll}
          onClose={() => setShowNotificationCenter(false)}
          onAddWeeklyReport={smartNotifs.addWeeklyReport}
          projectTitle={currentProject?.title || 'SciFlow Pro'}
          milestones={currentProject?.milestones || []}
          recentLogs={(currentProject?.milestones || []).flatMap(m => m.logs || []).slice(0, 20)}
          inventoryAlerts={(inventory || []).filter(i => i.category !== 'Hardware' && i.status !== 'Purchasing' && i.quantity <= i.threshold)}
          literatureCount={(currentProject?.feedItems || []).filter(f => !f.isRead).length}
          language="zh"
        />
      )}
      <AiCommandCli />
      ...
      {/* PWA 安装提示 — 仅移动端 */}
      {isMobileView && <PWAInstallPrompt isLightMode={isLightMode} />}
      ...
      <MobileNotificationCenter
        ...
        isLightMode={isLightMode}
        onNavigate={(view, projectId) => navigate(view as AppView, projectId)}
      />
