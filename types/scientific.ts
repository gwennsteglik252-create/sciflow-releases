...
export interface SavedFlowchart {
  ...
  optimizedParameters?: { key: string; value: string; reason: string }[];
  controlParameters?: { key: string; value: string; reason: string }[];
  /** 全量 session 快照，保存时自动生成 */
  sessionData?: Omit<FlowchartSession, 'currentFlowchart'>;
}
...
export interface SavedDOE {
  ...
  category?: string;
  /** 全量 session 快照（新格式） */
  sessionData?: Omit<DoeSession, 'isCalculating'>;
  // ── 旧格式扁平字段（向后兼容，新保存不再使用） ──
  factors?: DOEFactor[];
  responses?: DOEResponse[];
  history?: DoeHistoryItem[];
  processDescription?: string;
...
export interface SavedChart {
  ...
  thumbnailUrl?: string;
  folderId?: string;
  sessionData: Omit<DataAnalysisSession, 'savedCharts' | 'chartFolders' | 'currentSavedChartId' | 'currentSavedChartName'>;
}
...
export interface DataAnalysisSession {
  ...
  /** 内嵌数据工作表状态 */
  spreadsheet?: import('./spreadsheet').SpreadsheetState;
  ...
  /** Origin 风格工作区（多工作表+多图表） */
  workspace?: import('./workspace').WorkspaceState;

  /** 子图布局模式 */
  subplotLayout?: import('./visuals').SubplotLayoutMode;
  /** 子图面板配置 */
  subplotPanels?: import('./visuals').SubplotPanel[];
  /** 当前选中的子图 ID */
...
export interface SavedVisionAnalysis {
  ...
  timestamp: string;
  /** 全量 session 快照（新格式） */
  sessionData?: Omit<VisionSession, 'savedArchives' | 'modeStates'>;
  // ── 旧格式扁平字段（向后兼容，新保存不再使用） ──
  imageSrc?: string | null;
  mode?: 'SEM' | 'TEM' | 'XRD';
  scaleRatio?: number | null;
