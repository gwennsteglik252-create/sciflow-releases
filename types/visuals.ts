...
export type GraphEdgeType =
  ...
  | 'extends'          // 延伸研究
  | 'cites'            // 引用关系
  | 'shared_method'    // 共享实验方法
  | 'temporal_follow'; // 时间序列后续

/* Enhanced GraphEdge with richer relationship model */
...
/** ★ Phase 3: 图谱自动推理洞察 */
export type GraphInsightType = 'contradiction' | 'gap' | 'opportunity' | 'risk' | 'cluster';
...
export interface FigureText {
  ...
  color: string;
  fontFamily: string;
  // Rich text / LaTeX mode (功能7)
  renderMode?: 'plain' | 'rich' | 'latex';
  // Layer system
  zIndex?: number;
  visible?: boolean;
  locked?: boolean;
...
export interface FigureShape {
  id: string;
  type: 'arrow' | 'line' | 'rect' | 'circle' | 'bezier' | 'polygon' | 'freehand' | 'callout';
  x1: number;
  y1: number;
  x2: number;
  y2: number;
  ...
  // Bezier/Polygon/Freehand points (功能8)
  ...
  fill?: {
    ...
    gradient?: { from: string; to: string; angle?: number };  // gradient
    pattern?: 'dots' | 'lines' | 'crosshatch' | 'diagonal';  // pattern
  };
  // 功能15: 标注连线 (callout)
  calloutText?: string;
...
export interface FigurePanel {
  ...
  // Scale bar for SEM/TEM images (功能3)
  scaleBar?: {
    text: string;
    width: number;
    height: number;
  ...
  // Rotation and flip (功能12)
  rotation?: number;
  flipH?: boolean;
  flipV?: boolean;
  // Image filters (功能10)
  filters?: { brightness: number; contrast: number; saturate: number; hueRotate: number };
  pseudoColor?: string;
  // Grouping (功能13)
  groupId?: string;
  /** 图源追溯（用于综述 Caption 自动标注） */
  sourceRef?: {
    type: 'literature' | 'local' | 'generated';
    title?: string;
    page?: number;
  };
}
...
export interface DataSeries {
  ...
  name: string;
  data: ChartDataPoint[];
  yAxisId?: 'left' | 'right';        // 双Y轴归属（默认 left）
  color?: string;
  pointColor?: string;
  strokeWidth?: number;
  visible?: boolean;
...
// --- Data Analysis Subplot Layout Types ---

/* 12. SubplotLayoutMode for multi-panel figure layouts (Fig a/b/c/d) */
export type SubplotLayoutMode = 'single' | '1x2' | '2x1' | '2x2' | '1x3' | '3x1' | '2x3' | '3x2';

/* 13. SubplotPanel for individual subplot configuration */
export interface SubplotPanel {
  id: string;
  label: string;          // e.g. "(a)", "(b)"
  seriesIds: string[];     // IDs from the main seriesList assigned to this panel
  chartType: string;
  chartTitle: string;
  xAxisLabel: string;
  yAxisLabel: string;
  ...
  pointSize?: number;
  // ── 字体设置 ──
  fontSize?: number;           // 标题字号
  axisLabelFontSize?: number;  // 轴标签字号
  tickFontSize?: number;       // 刻度字号
  labelFontFamily?: string;
