...
const ModalManager: React.FC<ModalManagerProps> = ({ modals, closeModal, onOpenConfirm, cloudSync }) => {
  ...
  return (
    <>
      {modals.addProject && (
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-md z-[5000] flex items-center justify-center p-2 sm:p-4">
          <div className="bg-white w-full max-w-lg rounded-2xl sm:rounded-[3rem] p-5 sm:p-10 animate-reveal shadow-2xl relative border-4 border-white flex flex-col max-h-[95vh] sm:max-h-[90vh]">
            <button onClick={() => closeModal('addProject')} className="absolute top-4 right-4 sm:top-8 sm:right-8 text-slate-300 hover:text-rose-500 transition-all"><i className="fa-solid fa-times text-xl sm:text-2xl"></i></button>
            <h3 className="text-lg sm:text-2xl font-black text-slate-800 mb-4 sm:mb-8 uppercase tracking-tighter italic border-l-8 border-indigo-600 pl-4 sm:pl-6 flex items-center gap-3">
              创建新课题库
              {isParsing && <i className="fa-solid fa-spinner animate-spin text-indigo-500 text-sm"></i>}
            </h3>

            <div className="flex-1 overflow-y-auto custom-scrollbar pr-2 space-y-4 sm:space-y-6">
              {/* New Proposal Upload Section */}
              <section className="bg-indigo-50/50 p-4 sm:p-6 rounded-2xl sm:rounded-[2.5rem] border-2 border-dashed border-indigo-200">
              ...
              <div>
                ...
                <input autoFocus className="w-full bg-slate-50 border-none rounded-2xl p-4 sm:p-5 text-[14px] font-bold outline-none shadow-inner mb-4 focus:ring-2 focus:ring-indigo-100 transition-all" placeholder="输入研究课题全称..." value={newProject.title} onChange={e => setNewProject({ ...newProject, title: e.target.value })} />

                <label className="text-[10px] font-black text-slate-400 uppercase mb-2 block px-2">课题描述</label>
                <textarea className="w-full bg-slate-50 border-none rounded-2xl p-4 sm:p-5 text-[14px] font-bold outline-none shadow-inner h-24 mb-4 resize-none focus:ring-2 focus:ring-indigo-100 transition-all" placeholder="简述研究背景与目标..." value={newProject.desc} onChange={e => setNewProject({ ...newProject, desc: e.target.value })} />

                <label className="text-[10px] font-black text-slate-400 uppercase mb-2 block px-2">关键词 (以逗号分隔)</label>
                <input className="w-full bg-slate-50 border-none rounded-2xl p-4 sm:p-5 text-[14px] font-bold outline-none shadow-inner focus:ring-2 focus:ring-indigo-100 transition-all" placeholder="e.g. AEM, OER, NiFe-LDH" value={newProject.kpis} onChange={e => setNewProject({ ...newProject, kpis: e.target.value })} />
              </div>
            </div>

            <div className="flex gap-3 sm:gap-4 pt-4 sm:pt-8 shrink-0">
              <button onClick={() => closeModal('addProject')} className="flex-1 py-3.5 sm:py-5 bg-slate-100 text-slate-500 rounded-2xl text-[11px] font-black uppercase hover:bg-slate-200">取消</button>
              <button onClick={handleCreateProject} disabled={!newProject.title.trim() || isParsing} className="flex-[2] py-3.5 sm:py-5 bg-indigo-600 text-white rounded-2xl text-[11px] font-black uppercase shadow-2xl disabled:opacity-30 transition-all hover:bg-black">确认开启课题库</button>
            </div>
          </div>
        </div>
      )}
