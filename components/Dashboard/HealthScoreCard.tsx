...
function getScoreStyle(score: number, t: (key: string) => string) {
    if (score >= 80) return { bar: 'bg-emerald-500', text: 'text-emerald-500', badge: 'bg-emerald-500/10 text-emerald-600', glow: 'shadow-[0_0_12px_rgba(16,185,129,0.35)]', label: t('dashboardWidgets.scoreHealthy') };
    if (score >= 60) return { bar: 'bg-amber-400', text: 'text-amber-500', badge: 'bg-amber-400/10 text-amber-600', glow: 'shadow-[0_0_12px_rgba(251,191,36,0.35)]', label: t('dashboardWidgets.scoreAttention') };
    return { bar: 'bg-rose-500', text: 'text-rose-500', badge: 'bg-rose-500/10 text-rose-600', glow: 'shadow-[0_0_12px_rgba(244,63,94,0.35)]', label: t('dashboardWidgets.scoreRisk') };
}
...
const HealthScoreCard: React.FC<HealthScoreCardProps> = ({ projects, resources, isLight, navigate }) => {
    ...
    const DIM_LABELS = [
        { key: 'taskRate', label: t('dashboardWidgets.dimTask'), icon: 'fa-list-check', color: 'bg-indigo-400' },
        { key: 'successRate', label: t('dashboardWidgets.dimExperiment'), icon: 'fa-flask', color: 'bg-emerald-400' },
        { key: 'metricRate', label: t('dashboardWidgets.dimMetric'), icon: 'fa-chart-line', color: 'bg-amber-400' },
        { key: 'litSupport', label: t('dashboardWidgets.dimLiterature'), icon: 'fa-book-open', color: 'bg-violet-400' },
    ];
    ...
    const avgScore = scores.length > 0
        ...
        : 0;
    const avgStyle = getScoreStyle(avgScore, t);
        ...
        <div className={`h-full flex-1 p-6 rounded-[2.5rem] flex flex-col min-h-0 border ${isLight ? 'bg-white border-slate-100 shadow-sm' : 'bg-slate-800/80 border-white/5 shadow-xl'}`}>
            ...
            <div className="flex items-center justify-between mb-5 shrink-0">
                <h4 className={`text-xs font-black flex items-center gap-2 ${isLight ? 'text-slate-800' : 'text-white'}`}>
                    ...
                    {t('dashboardWidgets.projectHealth')}
                </h4>
                {scores.length > 0 && (
                    <div className={`flex items-center gap-1.5 px-2.5 py-1 rounded-xl text-[9px] font-black uppercase tracking-widest ${avgStyle.badge}`}>
                        ...
                        {t('dashboardWidgets.teamAvg')} {avgScore}
                    </div>
                )}
            </div>
            ...
            <div className="flex-1 overflow-y-auto space-y-3 custom-scrollbar pr-1 min-h-0">
                {scores.length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-8 opacity-40">
                        ...
                        <p className="text-[9px] text-slate-400 italic">{t('dashboardWidgets.noActiveProjects')}</p>
                    </div>
                ) : (
                    scores.map(sc => {
                        const s = getScoreStyle(sc.total, t);
                        return (
                            <div
                                key={sc.projectId}
