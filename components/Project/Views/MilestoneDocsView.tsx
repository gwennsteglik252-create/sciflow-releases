...
const MilestoneDocsView: React.FC<MilestoneDocsViewProps> = ({
  ...
  const handleRenameDoc = (id: string, e: React.MouseEvent) => {
    ...
    if (!doc) return;
    const newTitle = window.prompt("输入新标题:", doc.title);
    ...
    <div className="flex-1 min-h-0 flex flex-col p-4 lg:p-6 overflow-hidden bg-slate-50/20">
      <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4 shrink-0">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h3 className="text-lg lg:text-2xl font-black text-slate-800 uppercase tracking-tight italic mb-0">节点文档库</h3>
        ...
        {filteredDocs.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-2.5 pb-6">
            {filteredDocs.map(doc => {
              ...
              const typeColor = typeLabel === '总结' ? 'bg-violet-100 text-violet-600' : typeLabel === 'AI审计' ? 'bg-rose-100 text-rose-600' : typeLabel === '机理' ? 'bg-amber-100 text-amber-600' : 'bg-indigo-100 text-indigo-600';
              return (
          ...
          <div className="h-full flex flex-col items-center justify-center opacity-30 gap-4">
            ...
            <p className="text-[11px] font-black uppercase tracking-[0.4rem] italic">该节点暂无生成文档记录</p>
          </div>
        )}
      </div>
