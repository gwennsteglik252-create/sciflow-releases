        ...
        <div className="flex-1 min-h-0 flex flex-col p-4 lg:p-8 overflow-y-auto bg-slate-50/20 custom-scrollbar animate-reveal">
                ...
                <div className="flex items-center gap-3 w-full sm:w-auto">
                    ...
                    {onOpenCollector && (
                        <button
                            ...
                            className={`relative px-5 py-2.5 rounded-xl text-[11px] font-black uppercase shadow-md transition-all flex items-center gap-2 active:scale-95 shrink-0 border ${
                                collectedPlanCount > 0
                                    ? 'bg-gradient-to-r from-emerald-500 to-teal-500 text-white border-emerald-400/50 hover:from-emerald-600 hover:to-teal-600 shadow-emerald-100/50'
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8 pb-20 items-start">
                {filteredPlans.map(plan => {
                    ...
                    return (
                        <div
                            ...
                            className={`bg-white p-6 rounded-2xl border transition-all relative overflow-hidden group flex flex-col ${isHighlighted
                            ...
                            <div className="flex-1 flex flex-col">
                                ...
                                <div className="grid grid-cols-2 gap-3 mb-6">
                                    {plan.matrix?.slice(0, 4).map((m, i) => (
                                        <div key={i} className="flex flex-col p-4 bg-slate-50 border border-slate-100 rounded-xl group-hover:bg-white group-hover:border-indigo-100 transition-all shadow-inner">
                                        ...
                                        {isRunListExpanded && (
                                            <div className="space-y-4 animate-reveal">
                                                {plan.runs?.map((run, rIdx) => {
                                                    ...
                                                    return (
                                                        <div key={rIdx} className={`flex flex-col p-5 rounded-xl border-2 transition-all shadow-sm relative overflow-hidden ${cardBgClass}`}>
                ...
                <div
                    ...
                    className="bg-white rounded-2xl border-2 border-slate-100 shadow-sm hover:shadow-xl hover:border-indigo-300 transition-all cursor-pointer group overflow-hidden flex flex-col min-h-[350px] relative"
                >
                    {/* 顶部装饰条 */}
                    <div className="absolute top-0 left-0 w-2 h-full bg-gradient-to-b from-indigo-300 to-violet-400 opacity-30 group-hover:opacity-80 transition-opacity"></div>
                    {/* 主体内容居中 */}
                    <div className="flex-1 flex flex-col items-center justify-center gap-5 px-8 py-10">
                        {/* 渐变图标 */}
                        <div className="relative">
                            <div className="w-20 h-20 rounded-[1.5rem] bg-gradient-to-br from-indigo-500 to-violet-600 flex items-center justify-center shadow-2xl shadow-indigo-200/60 group-hover:scale-110 group-hover:shadow-indigo-300/80 transition-all duration-300">
                                <i className="fa-solid fa-plus text-white text-3xl"></i>
                            </div>
                            {/* 光圈动效 */}
                            <div className="absolute inset-0 rounded-[1.5rem] bg-indigo-400/20 scale-110 opacity-0 group-hover:opacity-100 group-hover:scale-125 transition-all duration-500"></div>
                        </div>
                        <div className="text-center">
                            <p className="text-sm font-black text-slate-700 group-hover:text-indigo-700 uppercase tracking-widest transition-colors">新增实验设计</p>
                            <p className="text-[9px] font-bold text-slate-300 uppercase tracking-widest mt-2">CREATE NEW MATRIX TEMPLATE</p>
                        </div>
                        {/* 小提示标签 */}
                        <div className="flex flex-wrap gap-2 justify-center mt-2">
                            {['快速建模', '参数范围', '正交设计'].map(tag => (
                                <span key={tag} className="px-2 py-1 bg-slate-50 border border-slate-100 rounded-lg text-[8px] font-black text-slate-300 uppercase group-hover:bg-indigo-50 group-hover:border-indigo-100 group-hover:text-indigo-400 transition-all">{tag}</span>
                            ))}
                        </div>
                    </div>
                    {/* 底部提示 */}
                    <div className="py-3 border-t border-dashed border-slate-100 flex items-center justify-center gap-2 text-slate-300 group-hover:text-indigo-400 group-hover:bg-indigo-50/30 transition-all">
                        <i className="fa-solid fa-arrow-pointer text-[10px]"></i>
                        <span className="text-[9px] font-black uppercase tracking-widest">点击创建</span>
                    </div>
                </div>
            </div>
            <style>{`
