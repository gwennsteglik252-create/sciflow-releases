    ...
    <div className="bg-indigo-50/50 rounded-2xl p-3 mb-4 border border-indigo-100 shrink-0">
        <div className="flex justify-between items-center cursor-pointer" onClick={() => setPushToMatrix(!pushToMatrix)}>
            <div className="flex items-center gap-2">
                ...
                <span className="text-[9px] font-black text-indigo-800 uppercase tracking-widest">推送到项目性能矩阵</span>
            </div>
            ...
            <div className="mt-3 animate-reveal">
                {projectMatrices.length > 0 ? (
                    <div className="bg-white p-2.5 rounded-xl border border-indigo-100 mb-2">
                        <label className="text-[8px] font-black text-slate-400 uppercase block mb-1">Target Matrix (目标矩阵)</label>
                        <select 
                    ...
                    <div className="text-[8px] text-amber-500 italic mb-2">System will create "Default Matrix"</div>
                )}

                <div className="flex items-center gap-2 mb-2">
                    <span className="text-[8px] font-black text-slate-500 uppercase">Sample ID:</span>
                ...
                <div className="grid grid-cols-2 gap-3">
                    <div className="bg-white p-2.5 rounded-xl border border-slate-100 relative group/box">
                        <div className="flex justify-between items-center mb-1.5">
                            <p className="text-[8px] font-black text-slate-400 uppercase">Process Params (X)</p>
                        ...
                        <div className="max-h-24 overflow-y-auto custom-scrollbar">
                            {matrixProcessParams.map((p, i) => (
                                <div key={i} className="flex gap-1 mb-1 items-center group/row">
                                    <input className="flex-[2.5] min-w-0 bg-slate-50 rounded px-1.5 py-0.5 text-[8px] border-none outline-none" value={p.key} onChange={e => {const n=[...matrixProcessParams]; n[i].key=e.target.value; setMatrixProcessParams(n)}} placeholder="Name" />
                                    <input className="flex-[1.2] min-w-0 bg-slate-50 rounded px-1.5 py-0.5 text-[8px] border-none outline-none text-right" value={p.value} onChange={e => {const n=[...matrixProcessParams]; n[i].value=e.target.value; setMatrixProcessParams(n)}} placeholder="Val" />
                                    <input className="flex-1 min-w-0 bg-slate-50 rounded px-1.5 py-0.5 text-[8px] border-none outline-none text-center" value={p.unit} onChange={e => {const n=[...matrixProcessParams]; n[i].unit=e.target.value; setMatrixProcessParams(n)}} placeholder="Unit" />
                    ...
                    <div className="bg-white p-2.5 rounded-xl border border-slate-100 relative group/box">
                        <div className="flex justify-between items-center mb-1.5">
                            <p className="text-[8px] font-black text-slate-400 uppercase">Results (Y)</p>
                        ...
                        <div className="max-h-24 overflow-y-auto custom-scrollbar">
                            {matrixResults.map((r, i) => (
                                <div key={i} className="flex gap-1 mb-1 items-center group/row">
                                    <input className="flex-[2.5] min-w-0 bg-slate-900 rounded px-1.5 py-0.5 text-[8px] text-white border-none outline-none" value={r.key} onChange={e => {const n=[...matrixResults]; n[i].key=e.target.value; setMatrixResults(n)}} placeholder="Metric" />
                                    <input className="flex-[1.2] min-w-0 bg-slate-900 rounded px-1.5 py-0.5 text-[8px] text-emerald-400 border-none outline-none text-right font-bold" value={r.value} onChange={e => {const n=[...matrixResults]; n[i].value=e.target.value; setMatrixResults(n)}} placeholder="Val" />
                                    <input className="flex-1 min-w-0 bg-slate-900 rounded px-1.5 py-0.5 text-[8px] text-slate-400 border-none outline-none text-center" value={r.unit} onChange={e => {const n=[...matrixResults]; n[i].unit=e.target.value; setMatrixResults(n)}} placeholder="Unit" />
