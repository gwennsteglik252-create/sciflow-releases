...
export const LogFiles: React.FC<LogFilesProps> = ({
    ...
    return (
        <div className="border-t border-slate-100 pt-4">
            <label className="text-xs font-black text-slate-950 uppercase mb-2 block px-1">{t('logModal.files.title')}</label>
            <div className="flex gap-2 mb-3">
                <input className="flex-1 bg-slate-50 border-none rounded-xl p-3.5 text-xs font-bold outline-none shadow-inner" placeholder={t('logModal.files.descPlaceholder')} value={fileDescInput} onChange={e => setFileDescInput(e.target.value)} />
            ...
            <div className="space-y-2">
                            ...
                            <div className="flex items-start gap-3 min-w-0">
                                ...
                                <div className="flex-1 min-w-0 space-y-1.5">
                                    <input
                                        ...
                                        placeholder={t('logModal.files.fileName')}
                                    />
                                    <input
                                        ...
                                        placeholder={t('logModal.files.addFileDesc')}
                                    />
                                </div>
                                <div className="flex items-center gap-2 shrink-0">
                                    {file.localPath && <span className="text-emerald-600 text-[10px] font-black uppercase"><i className="fa-solid fa-link"></i> {t('logModal.files.linked')}</span>}
                                    <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                        {!file.localPath && (
                                            <button onClick={(e) => { e.stopPropagation(); handleLinkExistingLocalFile(idx); }} className="text-slate-400 hover:text-indigo-600 p-1" title={t('logModal.files.linkLocal')}>
                ...
                {logFiles.length === 0 && <p className="text-[11px] text-slate-300 italic text-center py-4">{t('logModal.files.noFiles')}</p>}
            </div>
        </div>
    );
};
