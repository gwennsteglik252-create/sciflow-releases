...
export const LogMetrics: React.FC<LogMetricsProps> = ({
    ...
    return (
        <div className="space-y-5">
            <div>
                <label className="text-xs font-black text-slate-950 uppercase mb-2 block px-1 italic">原子化性能指标 (KPI SNAPSHOT)</label>
                <div className={`bg-slate-50 rounded-[2.5rem] p-5 border border-slate-200 shadow-inner overflow-visible relative ${activeUnitMenu ? 'z-[500]' : 'z-10'}`}>
                    ...
                    <div className="flex gap-1.5 px-1 mb-2 text-[8px] font-black text-slate-600 uppercase tracking-widest">
                        <span className="flex-[1.5]">指标名</span>
                        <span className="flex-1 text-center">数值</span>
                    ...
                    <div className="pt-4 border-t border-slate-200 overflow-visible">
                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3 px-1 italic">添加新指标 (ADD METRIC)</p>
                        <div className={`flex gap-1.5 items-center relative overflow-visible ${activeUnitMenu === 'NEW' ? 'z-50' : 'z-10'}`}>
                            <input className="flex-[1.5] min-w-0 bg-white rounded-xl px-3 py-2.5 text-xs font-bold outline-none shadow-sm border border-slate-200 focus:border-indigo-400 h-11 transition-all" placeholder="指标名" value={newMetricKey} onChange={e => setNewMetricKey(e.target.value)} onBlur={() => setNewMetricKey(smartConvertChemistry(newMetricKey))} />
                            <input className="flex-1 min-w-0 bg-white rounded-xl px-1 py-2.5 text-xs font-black text-rose-600 font-mono outline-none border border-slate-200 focus:border-indigo-400 text-center shadow-sm h-11" placeholder="数值" value={newMetricVal} onChange={e => setNewMetricVal(e.target.value)} />
            ...
            {hasKpi && (
                <div className="animate-reveal">
                    <div className="flex items-center gap-2 mb-2 px-1">
                        <label className="text-xs font-black text-slate-950 uppercase italic">实测数据 VS 目标 (BENCHMARKING)</label>
                        {projectTargets.length > 0 && (
                            <span className="text-[8px] font-black text-indigo-600 bg-indigo-50 px-2 py-0.5 rounded-full border border-indigo-100 flex items-center gap-1">
                                <i className="fa-solid fa-crosshairs"></i> 对标引擎开启
                            </span>
                        )}
                    </div>
                            ...
                            projectTargets.filter(Boolean).map((target, i) => {
                                ...
                                if (actualVal !== null && Number.isFinite(actualVal) && targetNum !== null && targetNum !== 0) {
                                    ...
                                    if (isExcellent) {
                                        ...
                                        badge = '✓ 达标'; badgeColor = 'bg-emerald-50 text-emerald-600 border border-emerald-200';
                                    } else if (isGood) {
                                        statusColor = 'bg-amber-400'; textColor = 'text-amber-600';
                                        badge = '≈ 接近'; badgeColor = 'bg-amber-50 text-amber-600 border border-amber-200';
                                    } else {
                                        statusColor = 'bg-rose-400'; textColor = 'text-rose-600';
                                        badge = '✗ 未达'; badgeColor = 'bg-rose-50 text-rose-600 border border-rose-200';
                                    }
                                } else if (actualVal !== null && Number.isFinite(actualVal)) {
                                    statusColor = 'bg-indigo-400'; textColor = 'text-indigo-600';
                                    badge = '已录入'; badgeColor = 'bg-indigo-50 text-indigo-600 border border-indigo-200';
                                    percent = 100;
                                }
            ...
            <div>
                <label className="text-xs font-black text-slate-950 uppercase mb-3 block px-1 italic">实验结果状态 (OUTCOME)</label>
                <div className="grid grid-cols-1 gap-2.5">
                    <button onClick={() => setLogResult('success')} className={`py-4 rounded-3xl text-xs font-black uppercase transition-all flex items-center justify-center gap-3 border-2 ${logResult === 'success' ? 'bg-emerald-50 border-emerald-500 text-emerald-700 shadow-lg scale-[1.02]' : 'bg-slate-50 border-transparent text-slate-400 hover:bg-slate-100'}`}>
                        <span className={`w-2.5 h-2.5 rounded-full ${logResult === 'success' ? 'bg-emerald-500 shadow-[0_0_12px_#10b981] animate-pulse' : 'bg-slate-300'}`}></span> 达到预期
                    </button>
                    <button onClick={() => setLogResult('observation')} className={`py-4 rounded-3xl text-xs font-black uppercase transition-all flex items-center justify-center gap-3 border-2 ${logResult === 'observation' ? 'bg-amber-50 border-amber-600 text-amber-700 shadow-lg scale-[1.02]' : 'bg-slate-50 border-transparent text-slate-400 hover:bg-slate-100'}`}>
                        <span className={`w-2.5 h-2.5 rounded-full ${logResult === 'observation' ? 'bg-amber-400 shadow-[0_0_12px_#f59e0b] animate-pulse' : 'bg-slate-300'}`}></span> 观测中
                    </button>
                    <button onClick={() => setLogResult('failure')} className={`py-4 rounded-3xl text-xs font-black uppercase transition-all flex items-center justify-center gap-3 border-2 ${logResult === 'failure' ? 'bg-rose-50 border-rose-600 text-rose-700 shadow-lg scale-[1.02]' : 'bg-slate-50 border-transparent text-slate-400 hover:bg-slate-100'}`}>
                        <span className={`w-2.5 h-2.5 rounded-full ${logResult === 'failure' ? 'bg-rose-500 shadow-[0_0_12px_#f43f5e] animate-pulse' : 'bg-slate-300'}`}></span> 未达预期
                    </button>
                </div>
            </div>
        </div>
