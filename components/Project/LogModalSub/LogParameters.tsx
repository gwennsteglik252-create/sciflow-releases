  ...
  return (
    <div>
        <div className="flex justify-between items-center mb-2.5 px-1">
            <label className="text-xs font-black text-slate-950 uppercase block italic">{t('logModal.parameters.title')}</label>
            {planSnapshot && <span className="text-[8px] font-black text-indigo-600 bg-indigo-50 px-2 py-0.5 rounded-full border border-indigo-100 flex items-center gap-1"><i className="fa-solid fa-link-slash"></i> {t('logModal.parameters.engineOn')}</span>}
        </div>
        ...
        <div className="bg-slate-50 rounded-[2.5rem] p-4 border border-slate-200 shadow-inner max-h-[340px] overflow-y-auto custom-scrollbar">
                    ...
                    <div key={idx} className="flex gap-2 items-start group animate-reveal relative">
                        <div className="flex-1 min-w-0">
                            <input 
                                ...
                                placeholder={t('logModal.parameters.variableName')} 
                                value={param.key}
                        ...
                        <div className="flex-1 min-w-0 relative">
                            <input 
                                ...
                                placeholder={t('logModal.parameters.valuePlaceholder')} 
                                value={param.value}
                            ...
                            {dev && (
                                <div className={`absolute -bottom-1 left-1/2 -translate-x-1/2 translate-y-full z-[100] whitespace-nowrap px-2 py-0.5 rounded-lg text-[7px] font-black uppercase shadow-xl border ${
                                    ...
                                    {dev.type === 'alert' 
                                      ? t('logModal.parameters.deviation').replace('{target}', dev.target).replace('{percent}', (dev.percent || 0).toString()) 
                                      : t('logModal.parameters.expected').replace('{target}', dev.target)}
                                </div>
                            )}
                        </div>
                        <div className="w-16 shrink-0">
                            <input className="w-full bg-white rounded-xl px-2 py-2.5 text-[10px] font-black outline-none border border-slate-100 h-10 text-slate-400 shadow-sm text-center" placeholder={t('logModal.parameters.unit')} value={param.unit} onChange={e => updateParam(idx, 'unit', e.target.value)} />
                        </div>
            ...
            <button onClick={() => addParamRow()} className="w-full mt-6 py-3 text-[10px] font-black text-indigo-600 border-2 border-dashed border-indigo-200 rounded-2xl hover:bg-indigo-50 transition-all flex items-center justify-center gap-2"><i className="fa-solid fa-plus-circle"></i> {t('logModal.parameters.customRow')}</button>
        </div>
    </div>
  );
};
