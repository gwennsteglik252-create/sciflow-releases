...
export const LogBasicInfo: React.FC<LogBasicInfoProps> = ({
    ...
    const intentInfo = useMemo(() => {
        ...
        if (aggressiveRegex.test(logContent)) {
            return { label: t('logModal.basicInfo.aggressive'), color: 'bg-rose-600 border-rose-500', icon: 'fa-bolt-lightning' };
        }
        if (robustRegex.test(logContent)) {
            return { label: t('logModal.basicInfo.robust'), color: 'bg-indigo-600 border-indigo-500', icon: 'fa-check-double' };
        }
        if (explorerRegex.test(logContent)) {
            return { label: t('logModal.basicInfo.explorer'), color: 'bg-amber-500 border-amber-400', icon: 'fa-compass' };
        }
        return null;
    }, [logContent]);
        ...
        <div className="space-y-5">
            <div>
                <div className="flex justify-between items-end mb-2 px-1">
                    <label className="text-xs font-black text-slate-950 uppercase block italic tracking-tight">{t('logModal.basicInfo.runIdentifier')}</label>
                    {intentInfo && (
                ...
                <input
                    ...
                    placeholder={t('logModal.basicInfo.namePlaceholder')}
                    value={logContent}
            ...
            <div className="bg-slate-50 border border-slate-200 rounded-2xl p-3 shadow-inner">
                <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest">{t('logModal.basicInfo.experimentGroup')}</span>
                    {groupId && (
                ...
                {!creatingGroup ? (
                    <div className="flex gap-2">
                        ...
                        >
                            <option value="">{t('logModal.basicInfo.noGroup')}</option>
                        ...
                        <button
                            ...
                            title={t('logModal.basicInfo.newGroupTitle')}
                        >
                            <i className="fa-solid fa-plus"></i> {t('logModal.basicInfo.newGroupBtn')}
                        </button>
                    </div>
                ) : (
                    <div className="flex gap-2 animate-reveal">
                        <input
                            ...
                            placeholder={t('logModal.basicInfo.newGroupPlaceholder')}
                            value={newGroupName}
                ...
                {groupId && groupLabel && (
                    <div className="mt-2 flex items-center gap-1.5">
                        ...
                        <span className="text-[9px] font-bold text-indigo-600 truncate">{t('logModal.basicInfo.currentGroup').replace('{label}', groupLabel)}</span>
                        <button
                            type="button"
                        ...
                        >
                            <i className="fa-solid fa-xmark"></i> {t('logModal.basicInfo.unlinkGroup')}
                        </button>
                    </div>
                )}
            </div>
            <div>
                <div className="flex justify-between items-center mb-2 px-1">
                    <label className="text-xs font-black text-slate-950 uppercase block italic tracking-tight">{t('logModal.basicInfo.observation')}</label>
                    {logDescription.includes('设计意图溯源') && (
                        <span className="text-[8px] font-black text-emerald-600 uppercase flex items-center gap-1">
                            <i className="fa-solid fa-link"></i> {t('logModal.basicInfo.intentTraced')}
                        </span>
                    )}
                </div>
                <div className="relative group">
                    <textarea
                        ...
                        placeholder={t('logModal.basicInfo.obsPlaceholder')}
                ...
                <button onClick={handleAiExtract} disabled={isExtracting || !logDescription.trim()} className="mt-2 text-[10px] font-black text-indigo-700 uppercase flex items-center gap-2 hover:underline disabled:opacity-30 transition-all active:scale-95">
                    ...
                    {t('logModal.basicInfo.aiAutoCapture')}
                </button>
            </div>

            <div>
                <div className="flex justify-between items-center mb-2 px-1">
                    <label className="text-xs font-black text-slate-950 uppercase block italic tracking-tight">{t('logModal.basicInfo.samplePhoto')}</label>
                    {samplePhoto && <span className="text-[8px] font-black text-emerald-600 uppercase">{t('logModal.basicInfo.uploaded')}</span>}
                </div>
                <input
                ...
                <div className="mt-2 rounded-2xl border border-indigo-200 bg-indigo-50/50 p-2">
                    <div className="flex gap-2">
                        ...
                        >
                            <i className="fa-solid fa-image mr-1.5"></i> {t('logModal.basicInfo.uploadSample')}
                        </button>
                        <button
                            type="button"
                        ...
                        >
                            ...
                            {t('logModal.basicInfo.aiAppearance')}
                        </button>
                    </div>
                </div>
                    ...
                    <div className="mt-2 rounded-2xl border border-indigo-100 bg-white overflow-hidden">
                        ...
                        >
                            <span className="text-[9px] font-black text-indigo-600 uppercase tracking-widest flex items-center gap-1.5">
                                <i className="fa-solid fa-microscope"></i> {t('logModal.basicInfo.appearanceResult')}
                            </span>
            ...
            <div className="bg-slate-50 rounded-2xl border border-slate-200 shadow-inner">
                ...
                >
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest">{t('logModal.basicInfo.scientificKeyboard')}</span>
                    ...
                    <div className="flex items-center gap-1.5" onClick={e => e.stopPropagation()}>
                        <span className="text-[8px] font-bold text-slate-300 italic">
                            {activeTarget === 'title' ? t('logModal.basicInfo.inputToTitle') : t('logModal.basicInfo.inputToDesc')}
                        </span>
                        <button
                            type="button"
                        ...
                        >
                            ...
                            {t('logModal.basicInfo.smartConvert')}
                        </button>
                    </div>
                </div>
                ...
                {panelOpen && <div className="px-3 pb-3 space-y-2.5">
                    ...
                    <div className="flex items-center gap-2">
                        <div className="flex bg-white rounded-xl border border-slate-200 overflow-hidden shadow-sm shrink-0">
                            ...
                            >
                                {t('logModal.basicInfo.subscript')}
                            </button>
                            <button
                                type="button"
                            ...
                            >
                                {t('logModal.basicInfo.superscript')}
                            </button>
                        </div>
