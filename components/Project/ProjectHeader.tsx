...
const ProjectHeader: React.FC<ProjectHeaderProps> = ({
  ...
  const reportMenuItems = [
    { id: 'manual', label: '空白研报 (手动)', color: 'bg-indigo-600' },
    { id: 'weekly', label: '周报智能生成', color: 'bg-indigo-50' },
    { id: 'monthly', label: '月报智能生成', color: 'bg-violet-50' },
    { id: 'annual', label: '年报智能生成', color: 'bg-amber-50' }
  ];
  ...
  const handleBatchPurchase = () => {
    ...
    if (!addTaskToActivePlan) {
      showToast({ message: "无法调用采购引擎", type: 'error' });
      return;
    }

    toPurchase.forEach(res => {
      addTaskToActivePlan(project.id, `批量采购: ${res.name}`, {
        urgency: res.status === 'missing' ? 'Critical' : 'Urgent',
        quantity: 1,
        unit: res.stock?.unit || '个',
    ...
    showToast({ message: `已成功发起 ${toPurchase.length} 项物资的批量采购申请`, type: 'success' });
  ...
  // 优化：使用 fuzzy match 判定，只要包含 simulation 即视为来自虚拟实验室
      ...
      <header className="flex flex-row items-center gap-4 mb-3 bg-slate-900/80 p-3.5 rounded-2xl border border-white/10 shrink-0 shadow-xl backdrop-blur-xl z-50 relative">
        <div className="flex items-center gap-3 shrink-0">
          ...
          >
            ...
            {isFromVirtualLab && <span className="text-[6px] font-black uppercase mt-0.5">返回实验室</span>}
          </button>
          <div className="min-w-0">
            <div className="flex items-center gap-2">
              ...
              <button onClick={onEditProject} className="w-7 h-7 flex items-center justify-center rounded-lg bg-white/10 text-slate-400 hover:text-white transition-all shrink-0" title="编辑课题详情"><i className="fa-solid fa-pen text-[9px]"></i></button>
            </div>
            {isFromVirtualLab && (
              <p className="text-[7px] font-black text-amber-400 uppercase tracking-widest leading-none mt-1 animate-pulse">从仿真实验室跳转 · 智能溯源模式</p>
            )}
          </div>
        </div>
        ...
        <div className="flex flex-row gap-4 flex-1 items-center justify-between min-w-0">
          <div className="flex bg-slate-800/60 p-1.5 rounded-xl overflow-x-auto no-scrollbar">
            {[
              { id: 'logs', label: '工作流' },
              { id: 'process', label: '工艺路线' },
              { id: 'plan', label: '实验矩阵' },
              { id: 'sample_matrix', label: '样本矩阵' },
              { id: 'plan_board', label: '项目计划' },
              { id: 'reports', label: '归档库' },
              { id: 'presentation', label: '成果汇报' }
            ].map(tab => (
              <button
                key={tab.id}
          ...
          <div className="flex gap-2 shrink-0 items-center">
              ...
              <div className="relative">
                {readinessData.hasIssues && (
                  ...
                  >
                    ...
                    <span>物资准备度: {readinessData.overallStatus === 'ok' ? '就绪' : readinessData.overallStatus === 'warn' ? '库存紧缺' : '物资缺失'}</span>
                ...
                {showReadinessPanel && (
                  <div className="absolute top-full right-0 mt-3 w-80 bg-white border border-slate-200 rounded-3xl shadow-2xl p-6 overflow-hidden animate-reveal z-[100] text-slate-800">
                    <h4 className="text-[11px] font-black text-slate-400 uppercase tracking-widest mb-4 flex justify-between">
                      <span>物资供应审计</span>
                      <span className="text-indigo-600">{readinessData.checkResults.length} 项</span>
                    </h4>
                        ...
                        <div key={i} className={`p-3 rounded-2xl border flex justify-between items-center ${res.status === 'missing' ? 'bg-rose-50 border-rose-100' : res.status === 'low' ? 'bg-amber-50 border-amber-100' : res.status === 'purchasing' ? 'bg-indigo-50 border-indigo-100' : 'bg-slate-50 border-slate-100'}`}>
                          <div className="min-w-0 flex-1">
                            ...
                            <p className="text-[8px] font-bold text-slate-400 mt-0.5">
                              {res.status === 'missing' ? '🚨 仓库无此物料记录' : res.status === 'purchasing' ? '🚚 已推送到采购清单 / 路上' : `📦 库位: ${res.stock?.location || '--'} | 余量: ${res.stock?.quantity}${res.stock?.unit}`}
                            </p>
                          </div>
                          {(res.status === 'missing' || res.status === 'low') && (
                            <button
                              onClick={(e) => {
                                ...
                                const { addTaskToActivePlan } = (window as any).ProjectContextValue || {};
                                if (addTaskToActivePlan) {
                                  addTaskToActivePlan(project.id, `采购物料: ${res.name}`, {
                                    urgency: res.status === 'missing' ? 'Critical' : 'Urgent',
                                    quantity: 1,
                                    unit: res.stock?.unit || '个',
                                    deadline: new Date(Date.now() + 3 * 86400000).toISOString().split('T')[0],
                    ...
                    <div className="mt-6 pt-4 border-t border-slate-100 flex flex-col gap-3">
                      ...
                      <p className="text-[8px] text-slate-400 font-bold italic text-center">系统已自动通过项目计划书对比当前库存数据库</p>
                    </div>
                  </div>
                )}
              </div>
            ...
            {onlineUsers && onlineUsers.length > 0 && (
              <div className="flex items-center gap-2 ml-1">
                ...
                <span className="text-[9px] font-bold text-slate-400">在线协作者</span>
              </div>
            )}
            ...
            <button
              ...
              title="Generate Deep Dive Audio Overview"
            >
            ...
            <div className="relative">
              <button
                ...
                className="px-6 py-3 bg-rose-600 text-white rounded-lg text-[13px] font-black uppercase shadow-lg active:scale-95 transition-all flex items-center justify-center gap-2 border border-rose-500/50"
              >
                {isGeneratingWeekly ? <i className="fa-solid fa-spinner animate-spin"></i> : <><i className="fa-solid fa-file-pen"></i> 研报生成</>}
                <i className={`fa-solid fa-chevron-down text-[11px] transition-transform ${showReportMenu ? 'rotate-180' : ''}`}></i>
              </button>

              {showReportMenu && (
                <div className="absolute top-full right-0 mt-2 w-48 bg-white rounded-xl shadow-xl border border-slate-100 overflow-hidden animate-reveal z-50">
                  {reportMenuItems.map((item) => (
                    <button
                      key={item.id}
                      onClick={() => {
                        setShowReportMenu(false);
                        onStartWeeklyReport(item.id);
                      }}
                      className="w-full text-left px-4 py-3 hover:bg-slate-50 transition-all flex items-center gap-4 group"
                    >
                      {item.id === 'manual' ? (
                      ...
                      <span className="text-[11px] font-black text-slate-700 uppercase group-hover:text-indigo-600">
                        {item.label}
                      </span>
                    </button>
                  ))}
