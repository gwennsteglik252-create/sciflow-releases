  ...
  return (
    <div className="col-span-1 lg:col-span-3 flex flex-col gap-1 min-h-[300px] lg:h-[calc(100vh-110px)] overflow-hidden">
      ...
      <div className="flex justify-between items-center px-1 shrink-0 mt-6 mb-2">
        <h3 className="text-[11px] font-black text-slate-400 uppercase tracking-widest">{t('projectSidebar.topologyNodes')}</h3>
        <div className="flex gap-2">
          ...
          {onGenerateFromInput && (
            <button
              ...
              title={t('projectSidebar.aiGenerate')}
            >
          ...
          <button
            ...
            title={hasBlueprint ? t('projectSidebar.viewBlueprint') : t('projectSidebar.noBlueprintYet')}
          >
          ...
          <button
            ...
            title={t('projectSidebar.addNode')}
          >
          ...
          {onSortNodes && (
            <button
              ...
              title={t('projectSidebar.sortNodes')}
            >
          ...
          {onCollapse && (
            <button
              onClick={onCollapse}
              className="w-10 h-10 bg-slate-50 text-slate-400 border-2 border-slate-200 rounded-xl flex items-center justify-center shadow-lg active:scale-95 shrink-0 hover:bg-slate-700 hover:text-white hover:border-slate-600 transition-all"
      ...
      {showAiPanel && onGenerateFromInput && (
        <div className="mx-1 mb-3 rounded-2xl border border-violet-200 bg-gradient-to-b from-violet-50 to-white shadow-lg overflow-hidden shrink-0">
          ...
          <div className="flex items-center gap-2 px-4 pt-3.5 pb-2 border-b border-violet-100">
            ...
            <span className="text-[11px] font-black text-violet-700 uppercase tracking-widest">{t('projectSidebar.aiRouteTitle')}</span>
          </div>
          ...
          <div className="p-3 space-y-3">
              ...
              {uploadedImage ? (
                <div className="relative">
                  <img
                    ...
                    alt={t('projectSidebar.uploadPreview')}
                  ...
                  <div className="absolute bottom-1.5 left-1.5 bg-black/50 text-white text-[9px] font-bold rounded-md px-2 py-0.5 backdrop-blur-sm">
                    <i className="fa-solid fa-check-circle mr-1 text-emerald-400"></i>{t('projectSidebar.screenshotReady')}
                  </div>
                </div>
              ) : (
                <div className="flex flex-col items-center justify-center py-5 gap-2">
                  ...
                  <div className="text-center">
                    <p className="text-[10px] font-black text-slate-600">{t('projectSidebar.dragOrUpload')}</p>
                    <p className="text-[9px] text-slate-400 mt-0.5">
                      {t('projectSidebar.pasteHint')}
                    </p>
                  </div>
                </div>
              )}
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-[9px] font-black text-slate-300 uppercase">{t('projectSidebar.or')}</span>
              <div className="flex-1 h-px bg-slate-100"></div>
            </div>
            ...
            <textarea
              ...
              placeholder={t('projectSidebar.textPlaceholder')}
            ...
            >
              {isGeneratingNodes ? (
                <><i className="fa-solid fa-circle-notch animate-spin"></i> {t('projectSidebar.aiParsing')}</>
              ) : (
                <><i className="fa-solid fa-bolt"></i> {uploadedImage ? t('projectSidebar.parseScreenshot') : t('projectSidebar.generateRoute')}</>
              )}
            </button>
          </div>
        </div>
