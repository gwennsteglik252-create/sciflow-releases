    ...
    <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-md z-[1300] flex items-center justify-center p-4">
      <div className="bg-white w-full max-w-sm rounded-[3rem] p-10 animate-reveal shadow-2xl relative border-4 border-white">
        <h3 className="text-xl font-black text-slate-800 mb-8 uppercase italic border-l-8 border-indigo-600 pl-6">
          {data.parentId ? t('nodeModals.createBranch') : t('nodeModals.createNode')}
        </h3>
        <div className="space-y-4">
          <input 
            ...
            placeholder={t('nodeModals.nodeTitle')} 
            value={data.title} 
          ...
          <textarea 
            ...
            placeholder={t('nodeModals.hypothesis')} 
          ...
          <div className="flex gap-3">
            ...
            <button onClick={onSave} className="flex-1 py-4 bg-indigo-600 text-white rounded-2xl text-[11px] font-black uppercase shadow-xl">{t('nodeModals.confirmCreate')}</button>
          </div>
        </div>
      </div>
    </div>
