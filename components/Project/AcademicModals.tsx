...
interface AcademicModalsProps {
  ...
  onRequestExperimentFlows?: (tasks: string[]) => void;
...
const AcademicModals: React.FC<AcademicModalsProps> = ({
  ...
  projectId, onRequestExperimentFlows, isGeneratingFlows, onViewGeneratedFlows, onViewExperimentPlan
}) => {
  ...
  const [pushPhase, setPushPhase] = useState<'select' | 'confirm_flows'>('select');
  ...
  const handleOpenPushModal = useCallback(() => {
    ...
    setPushPhase('select'); // Reset phase when opening
  ...
  const handleConfirmPush = useCallback(() => {
    ...
    // 进入第二阶段：询问是否生成实验流
    if (onRequestExperimentFlows) {
      setPushedItems(itemsToPush);
      setPushPhase('confirm_flows');
    } else {
      setShowPushModal(false);
    }
  }, [projectId, addTaskToActivePlan, parsedPlanItems, selectedPlanItems, showToast, onRequestExperimentFlows]);
  ...
  const handleConfirmGenerateFlows = useCallback(() => {
    if (onRequestExperimentFlows && pushedItems.length > 0) {
      console.log('[PushPlan] 用户确认生成实验流, 任务:', pushedItems);
      onRequestExperimentFlows(pushedItems);
    }
    handleClosePushModal();
  }, [onRequestExperimentFlows, pushedItems, handleClosePushModal]);
  ...
  return (
    <>
              ...
              <div className="flex justify-between items-start">
                ...
                <div className="flex items-center gap-3 shrink-0">
                  ...
                  <div className="flex bg-white/10 p-1.5 rounded-2xl border border-white/10 shadow-inner">
                    ...
                    {isGeneratingFlows && (
                      <button disabled className="flex flex-col items-center justify-center w-20 h-12 bg-violet-500/80 rounded-xl text-white ml-1.5 border border-violet-400 shadow-lg cursor-wait animate-pulse">
                        <i className="fa-solid fa-spinner fa-spin text-sm mb-1"></i>
                        <span className="text-[8px] font-black uppercase">生成中</span>
                      </button>
                    )}
                    {!isGeneratingFlows && onViewGeneratedFlows && (
                      <button onClick={onViewGeneratedFlows} className="flex flex-col items-center justify-center w-20 h-12 bg-gradient-to-br from-violet-600 to-indigo-600 rounded-xl text-white ml-1.5 hover:from-violet-500 hover:to-indigo-500 transition-all active:scale-95 border border-violet-400 shadow-lg group/flow">
                        <i className="fa-solid fa-flask-vial text-sm mb-1 group-hover/flow:scale-110 transition-transform"></i>
                        <span className="text-[8px] font-black uppercase">实验流</span>
                      </button>
                    )}
                    {!isGeneratingFlows && onViewExperimentPlan && (
                      <button onClick={onViewExperimentPlan} className="flex flex-col items-center justify-center w-20 h-12 bg-gradient-to-br from-emerald-600 to-teal-600 rounded-xl text-white ml-1.5 hover:from-emerald-500 hover:to-teal-500 transition-all active:scale-95 border border-emerald-400 shadow-lg group/plan">
                        <i className="fa-solid fa-clipboard-list text-sm mb-1 group-hover/plan:scale-110 transition-transform"></i>
                        <span className="text-[8px] font-black uppercase">实验计划</span>
                      </button>
                    )}
                    {!weeklyReport.id && (
      ...
      {/* Push Plan to Weekly Plan Modal — Two-Phase */}
      {showPushModal && (
          ...
          <div className="bg-white w-full max-w-lg rounded-[3rem] p-10 animate-reveal shadow-2xl border-4 border-emerald-100">
            ...
            {pushPhase === 'select' ? (
              <>
                ...
                <div className="flex gap-4">
                  <button onClick={handleClosePushModal} className="flex-1 py-4 bg-slate-100 text-slate-500 rounded-2xl text-[11px] font-black uppercase">取消</button>
              ...
              <>
                {/* Phase 2: 推送成功，询问是否生成实验流 */}
                <div className="flex flex-col items-center text-center mb-8">
                  <div className="w-20 h-20 bg-emerald-100 text-emerald-600 rounded-full flex items-center justify-center text-4xl mb-5 shadow-inner animate-reveal">
                    <i className="fa-solid fa-check"></i>
                  </div>
                  <h3 className="text-xl font-black text-slate-800 uppercase italic tracking-tighter">推送成功!</h3>
                  <p className="text-[11px] text-slate-500 font-medium mt-2">
                    已推送 <span className="font-black text-emerald-600">{pushedItems.length}</span> 项任务到周计划
                  </p>
                </div>
                <div className="bg-gradient-to-r from-indigo-50 to-violet-50 rounded-2xl p-5 mb-6 border border-indigo-100">
                  <div className="flex items-center gap-3 mb-3">
                    <div className="w-10 h-10 bg-indigo-500 text-white rounded-xl flex items-center justify-center shrink-0">
                      <i className="fa-solid fa-flask-vial"></i>
                    </div>
                    <div>
                      <p className="text-[13px] font-black text-slate-800">是否同时生成实验流设计？</p>
                      <p className="text-[10px] text-slate-500 mt-0.5">AI 将识别实验类任务，自动生成含参数和正交实验组的实验流</p>
                    </div>
                  </div>
                  <div className="space-y-1.5 ml-[52px]">
                    {pushedItems.map((item, i) => (
                      <p key={i} className="text-[10px] text-slate-600 flex items-center gap-2">
                ...
                <div className="flex gap-4">
                  <button onClick={handleClosePushModal} className="flex-1 py-4 bg-slate-100 text-slate-500 rounded-2xl text-[11px] font-black uppercase hover:bg-slate-200 transition-all">
                    跳过
                  </button>
                  <button onClick={handleConfirmGenerateFlows} className="flex-[2] py-4 bg-gradient-to-r from-indigo-600 to-violet-600 text-white rounded-2xl text-[11px] font-black uppercase shadow-xl hover:shadow-2xl transition-all active:scale-[0.98] flex items-center justify-center gap-2">
                    <i className="fa-solid fa-flask-vial"></i>
                    AI 生成实验流
                  </button>
                </div>
