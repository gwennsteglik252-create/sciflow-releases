      ...
      <div className="bg-white w-full max-w-7xl rounded-[3rem] p-6 lg:p-8 animate-reveal shadow-2xl relative border-4 border-white max-h-[95vh] flex flex-col overflow-hidden">
        ...
        <div className="shrink-0 flex items-center gap-3 mb-4 px-2 overflow-x-auto custom-scrollbar pb-2">
          <div className="flex items-center gap-2 bg-slate-50 border border-slate-200 rounded-xl px-4 py-2 shadow-inner shrink-0">
            ...
            <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">{t('logModal.experimentDate')}</span>
            <input
              type="datetime-local"
          ...
          {state.linkedAnalysis && (
            <div className="flex items-center gap-2 bg-rose-50 border border-rose-200 rounded-xl px-4 py-2 shadow-sm animate-reveal shrink-0">
              ...
              <div className="flex flex-col">
                <span className="text-[8px] font-black text-rose-400 uppercase tracking-widest leading-none">{t('logModal.linkedEvidence')}</span>
              ...
              <button
                ...
                title={t('logModal.unlinkEvidence')}
              >
        ...
        <footer className="shrink-0 flex gap-4 pt-4 border-t border-slate-100">
          <button onClick={onClose} className="flex-1 py-4 bg-slate-100 text-slate-500 rounded-2xl text-[11px] font-black uppercase hover:bg-slate-200">{t('logModal.cancel')}</button>
          <button onClick={actions.handleConfirmSave} disabled={!state.logContent.trim()} className="flex-[2] py-4 bg-indigo-600 text-white rounded-2xl text-[11px] font-black uppercase shadow-[0_10px_25px_rgba(99,102,241,0.4)] hover:bg-black transition-all">{t('logModal.saveLog')}</button>
        </footer>
        ...
        {state.showNamingModal && (
          <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[1500] flex items-center justify-center p-4">
            <div className="bg-white w-full max-w-sm rounded-[2.5rem] p-8 animate-reveal shadow-2xl border-4 border-white text-center">
              ...
              <h3 className="text-xl font-black text-slate-800 mb-2 uppercase italic">{t('logModal.saveAsTemplate')}</h3>
              <input autoFocus className="w-full bg-slate-50 border-2 border-slate-100 rounded-2xl p-4 text-sm font-black outline-none mb-6 text-center focus:border-indigo-400" placeholder={t('logModal.templatePlaceholder')} value={state.tempTemplateName} onChange={e => actions.setTempTemplateName(e.target.value)} onKeyDown={e => e.key === 'Enter' && actions.handleSaveTemplate()} />
              <div className="flex gap-4"><button onClick={() => actions.setShowNamingModal(false)} className="flex-1 py-4 bg-slate-100 text-slate-500 rounded-2xl text-[11px] font-black uppercase">{t('logModal.cancelBtn')}</button><button onClick={actions.handleSaveTemplate} disabled={!state.tempTemplateName.trim()} className="flex-[2] py-4 bg-indigo-600 text-white rounded-2xl text-[11px] font-black uppercase shadow-xl">{t('logModal.confirmSave')}</button></div>
            </div>
          </div>
        )}
      </div>
