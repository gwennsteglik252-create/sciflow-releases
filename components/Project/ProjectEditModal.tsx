...
const ProjectEditModal: React.FC<ProjectEditModalProps> = ({ show, onClose, data, setData, onSave }) => {
  ...
  const handleProposalUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (isWord) {
      showToast({ message: t('projectEditModal.wordNotSupported'), type: 'info' });
      return;
    }

    if (!isPdf && !isTxt) {
      showToast({ message: t('projectEditModal.pdfOnly'), type: 'info' });
      return;
    }

    setIsParsing(true);
    showToast({ message: t('projectEditModal.parsingPlan'), type: 'info' });
    setAiStatus?.(t('projectEditModal.parsingStatus'));
    ...
    try {
      ...
      showToast({ message: t('projectEditModal.parseFailed'), type: 'error' });
      setIsParsing(false);
      setAiStatus?.(null);
    }
  };
  ...
  const handleParsingResult = (result: any, file: File) => {
    if (result) {
      ...
      if (window.confirm(t('projectEditModal.confirmAutoFill'))) {
        ...
        showToast({ message: t('projectEditModal.dataAligned'), type: 'success' });
      }
    }
    setIsParsing(false);
    setAiStatus?.(null);
  ...
  const ingestBibliography = () => {
    ...
    setPendingBib([]);
    showToast({ message: t('projectEditModal.importedRefs', { count: String(newResources.length) }), type: 'success' });
  };
  ...
  const ingestMasterRoute = () => {
    ...
    showToast({ message: t('projectEditModal.routeSynced'), type: 'success' });
  };
      ...
      <div className="bg-white w-full max-w-4xl rounded-[3rem] p-8 lg:p-12 animate-reveal shadow-2xl relative border-4 border-white flex flex-col max-h-[95vh]">
        ...
        <header className="flex justify-between items-center mb-8 shrink-0">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h3 className="text-2xl font-black text-slate-800 uppercase italic tracking-tighter leading-none">{t('projectEditModal.title')}</h3>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar pr-3 space-y-8 pb-4">
          ...
          <section className="bg-indigo-50/50 p-6 rounded-[2.5rem] border-2 border-dashed border-indigo-200 group hover:bg-indigo-50 hover:border-indigo-400 transition-all">
            <div className="flex justify-between items-center mb-5">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h4 className="text-[11px] font-black text-indigo-800 uppercase tracking-widest leading-none">{t('projectEditModal.masterPlan')}</h4>
              ...
              >
                <i className="fa-solid fa-file-upload"></i> {data.proposalDoc ? t('projectEditModal.updatePlan') : t('projectEditModal.uploadPdf')}
              </button>
            ...
            {data.proposalDoc ? (
              <div className="flex flex-col gap-4 animate-reveal">
                <div className="flex items-center gap-4 bg-white p-4 rounded-2xl border border-indigo-100 shadow-sm">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[8px] font-bold text-slate-400 uppercase mt-1">{t('projectEditModal.uploadTime')}: {data.proposalDoc.timestamp}</p>
                  </div>
                ...
                {pendingBib.length > 0 && (
                  <div className="bg-slate-900 p-5 rounded-[2rem] shadow-xl animate-reveal border border-white/5">
                    <div className="flex justify-between items-center mb-4">
                      <div>
                        <p className="text-[9px] font-black text-indigo-400 uppercase tracking-widest">{t('projectEditModal.detectedRefs')} ({pendingBib.length})</p>
                        <p className="text-[7px] text-slate-500 uppercase mt-0.5">Reference Data Ready for Ingestion</p>
                      </div>
                      <button onClick={ingestBibliography} className="px-4 py-2 bg-indigo-600 text-white rounded-xl text-[9px] font-black uppercase hover:bg-emerald-500 transition-all shadow-lg border border-indigo-400">{t('projectEditModal.syncToArchive')}</button>
                    </div>
              ...
              >
                ...
                <p className="text-[10px] font-black uppercase tracking-[0.2rem]">{t('projectEditModal.uploadPlanPrompt')}</p>
                <p className="text-[8px] mt-1 font-bold">{t('projectEditModal.uploadPlanHint')}</p>
              </div>
            )}
              ...
              <div className="mt-6 bg-slate-900 p-6 rounded-[2.5rem] shadow-2xl animate-reveal border border-white/10">
                <div className="flex justify-between items-center mb-6">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h5 className="text-[11px] font-black text-white uppercase tracking-widest">{t('projectEditModal.masterRoutePreview')}</h5>
                  ...
                  >
                    <i className="fa-solid fa-link"></i> {t('projectEditModal.syncToTopology')}
                  </button>
                </div>

                <div className="space-y-4">
                  <div className="bg-white/5 p-4 rounded-2xl border border-white/5">
                    <p className="text-[8px] font-black text-slate-500 uppercase mb-2 tracking-tighter">{t('projectEditModal.coreHypothesis')}</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-4">
              <div>
                <label className="text-[10px] font-black text-slate-400 uppercase mb-2 block px-1">{t('projectEditModal.projectTitle')}</label>
                <input
              ...
              <div>
                <label className="text-[10px] font-black text-slate-400 uppercase mb-2 block px-1">{t('projectEditModal.projectCycle')}</label>
            ...
            <div>
              <label className="text-[10px] font-black text-slate-400 uppercase mb-2 block px-1">{t('projectEditModal.objectives')}</label>
              <textarea
                ...
                placeholder={t('projectEditModal.objectivesPlaceholder')}
          ...
          <div>
            <div className="flex justify-between items-center mb-4 px-1">
              <h4 className="text-[11px] font-black text-slate-800 uppercase tracking-widest flex items-center gap-2 italic">
                <i className="fa-solid fa-chart-line text-rose-500"></i> {t('projectEditModal.performanceMatrix')}
              </h4>
              <button onClick={handleAddMetric} className="text-[9px] font-black text-indigo-600 uppercase bg-indigo-50 px-3 py-1.5 rounded-lg border border-indigo-100 hover:bg-indigo-600 hover:text-white transition-all">{t('projectEditModal.addMetric')}</button>
            </div>
            ...
            <div className="rounded-[2.5rem] p-6 bg-slate-50/50 border border-slate-100 shadow-inner space-y-4">
              <div className="grid grid-cols-12 gap-3 px-3 text-[8px] font-black text-slate-400 uppercase tracking-widest mb-1">
                <div className="col-span-5">{t('projectEditModal.metricName')}</div>
                <div className="col-span-2 text-center">{t('projectEditModal.targetValue')}</div>
                <div className="col-span-1 text-center">{t('projectEditModal.unit')}</div>
                <div className="col-span-2 text-center">{t('projectEditModal.weight')}</div>
                <div className="col-span-2 text-right">{t('projectEditModal.extremeStrategy')}</div>
              </div>
              ...
              <div className="space-y-2">
                  ...
                  <div key={idx} className="grid grid-cols-12 gap-3 items-center group animate-reveal bg-white p-2 rounded-2xl border border-slate-100 shadow-sm hover:border-indigo-200 transition-all">
                    <div className="col-span-5">
                      <input
                        ...
                        placeholder={t('projectEditModal.metricPlaceholder')}
                        value={metric.label}
                    ...
                    <div className="col-span-2">
                      <input
                        ...
                        placeholder={t('projectEditModal.valuePlaceholder')}
                        value={metric.value}
                ...
                {(!data.targetMetrics || data.targetMetrics.length === 0) && (
                  <div className="py-8 text-center text-slate-300 italic text-[10px] uppercase font-bold tracking-widest border-2 border-dashed border-slate-200 rounded-3xl">{t('projectEditModal.noMetrics')}</div>
                )}
              </div>
            </div>
          </div>
          ...
          {data.requiredMaterials && data.requiredMaterials.length > 0 && (
            <section className="animate-reveal">
              <h4 className="text-[10px] font-black text-slate-800 uppercase tracking-widest mb-4 flex items-center gap-2 italic">
                <i className="fa-solid fa-box-open text-amber-500"></i> {t('projectEditModal.materialsList')}
              </h4>
        ...
        <footer className="shrink-0 flex gap-4 pt-6 border-t border-slate-100 mt-2">
          <button onClick={onClose} className="flex-1 py-4 bg-slate-100 text-slate-500 rounded-2xl text-[11px] font-black uppercase hover:bg-slate-200 transition-all active:scale-95">{t('projectEditModal.cancel')}</button>
          <button
            onClick={onSave}
          ...
          >
            <i className="fa-solid fa-floppy-disk"></i> {t('projectEditModal.save')}
          </button>
        </footer>
      </div>
    </div>
