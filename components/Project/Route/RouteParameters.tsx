    ...
    return (
        <div className="flex flex-col h-full">
            ...
            <div className="grid grid-cols-1 gap-3">
                {(params || []).map((p, idx) => {
                    ...
                    return (
                        <div
                            key={`param-${idx}-${p.key}`}
                            className="rounded-lg border-2 border-indigo-100 flex items-stretch overflow-hidden group/param hover:scale-[1.03] transition-all duration-300 shadow-sm hover:shadow-lg"
                                ...
                                {isEditing ? (
                                    <div className="space-y-3">
                                        <div className="flex gap-3">
                                            ...
                                            <input className="w-20 bg-white/95 border border-indigo-100 rounded-lg px-3 py-1 text-[11px] font-black font-mono text-indigo-700 outline-none focus:border-indigo-500" value={p.value} onChange={(e) => handleUpdateParam(idx, { value: e.target.value })} />
            ...
            {(!params || params.length === 0) && (
                <div className="py-12 border-2 border-dashed border-slate-100 rounded-xl flex flex-col items-center justify-center text-slate-300 opacity-40">
