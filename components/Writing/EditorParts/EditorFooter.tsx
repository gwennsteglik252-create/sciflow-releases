    ...
    <div className="p-3 border-t border-slate-100 text-right text-[10px] text-slate-400 font-mono px-4 sm:px-6 flex justify-between items-center bg-white shrink-0">
        <div className="flex items-center gap-4">
            <div className="flex items-center gap-2">
                ...
                <span className="uppercase font-bold">{saveStatus === 'saved' ? `已存 (${lastSavedTime})` : saveStatus === 'saving' ? '保存中...' : '未保存'}</span>
            </div>
            ...
            >
                 ...
                 {saveStatus === 'unsaved' ? '立即保存' : '已保存'}
            </button>
        </div>
        <span className="font-black tracking-widest uppercase opacity-60">{charCount} Characters</span>
    </div>
  );
};
