    ...
    >
        ...
        >
            <h5 className="text-[10px] font-black text-indigo-500 uppercase tracking-widest flex items-center gap-2 pointer-events-none">
                <i className="fa-solid fa-magnifying-glass"></i> 查找与替换
            </h5>
            <button 
        ...
        <div className="space-y-3">
            ...
            <div className="flex gap-2">
                <button onPointerDown={(e) => e.stopPropagation()} onClick={() => onFind('prev')} className="flex-1 py-2 bg-white border border-slate-200 rounded-lg text-[9px] font-bold text-slate-600 hover:bg-slate-50 transition-all"><i className="fa-solid fa-arrow-up mr-1"></i> 上一个</button>
                <button onPointerDown={(e) => e.stopPropagation()} onClick={() => onFind('next')} className="flex-1 py-2 bg-white border border-slate-200 rounded-lg text-[9px] font-bold text-slate-600 hover:bg-slate-50 transition-all"><i className="fa-solid fa-arrow-down mr-1"></i> 下一个</button>
            </div>
            <div className="flex gap-2 pt-3 border-t border-slate-100">
                <button onPointerDown={(e) => e.stopPropagation()} onClick={onReplace} className="flex-1 py-2 bg-indigo-50 text-indigo-600 rounded-lg text-[9px] font-black hover:bg-indigo-100 transition-all">替换</button>
                <button onPointerDown={(e) => e.stopPropagation()} onClick={onReplaceAll} className="flex-1 py-2 bg-indigo-600 text-white rounded-lg text-[9px] font-black hover:bg-indigo-700 transition-all shadow-sm">全部替换</button>
            </div>
        </div>
    </div>
  );
