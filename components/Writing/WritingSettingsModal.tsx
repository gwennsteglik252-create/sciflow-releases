...
interface WritingSettingsModalProps {
    ...
    editorMode: 'classic' | 'word' | 'online' | 'onlyoffice';
    onEditorModeChange: (mode: 'classic' | 'word' | 'online' | 'onlyoffice') => void;
}
        ...
        <div className="fixed inset-0 bg-slate-950/60 backdrop-blur-md z-[5000] flex items-center justify-center p-4">
            <div className="bg-white w-full max-w-xl rounded-[2.5rem] p-6 lg:p-10 animate-reveal shadow-2xl relative border border-slate-200 flex flex-col max-h-[90vh]">
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar pr-2 space-y-8">
                    ...
                    {/* 2.5 Editor Mode — 编辑模式选择 */}
                    <section>
                        <h4 className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-3 px-1 flex items-center gap-2">
                            <i className="fa-solid fa-desktop"></i> 编辑模式
                        </h4>
                        <div className="grid grid-cols-2 gap-3">
                            <button
                                onClick={() => onEditorModeChange('classic')}
                                className={`p-4 rounded-2xl border-2 transition-all flex flex-col items-center justify-center text-center gap-3 relative overflow-hidden ${
                                    editorMode === 'classic'
                                        ? 'border-indigo-500 shadow-md bg-indigo-600 text-white'
                                        : 'bg-slate-50 border-slate-100 text-slate-600 hover:border-indigo-200 hover:bg-white'
                                }`}
                            >
                                <div className={`w-10 h-10 rounded-xl flex items-center justify-center shadow-sm ${
                                    editorMode === 'classic' ? 'bg-white/20' : 'bg-white text-indigo-600 border border-slate-100'
                                }`}>
                                    <i className="fa-solid fa-pen-to-square text-lg"></i>
                                </div>
                                <div>
                                    <h5 className="text-[10px] font-black uppercase">经典模式</h5>
                                    <p className={`text-[7px] mt-1 ${editorMode === 'classic' ? 'text-white/70' : 'text-slate-400'}`}>编辑框 + 发布预览</p>
                                </div>
                                {editorMode === 'classic' && (
                                    <div className="absolute top-2 right-2">
                                        <i className="fa-solid fa-circle-check text-white text-[10px]"></i>
                            ...
                            <button
                                ...
                                className={`p-4 rounded-2xl border-2 transition-all flex flex-col items-center justify-center text-center gap-3 relative overflow-hidden ${
                                    editorMode === 'word'
                                        ? 'border-blue-500 shadow-md bg-blue-600 text-white'
                                        : 'bg-slate-50 border-slate-100 text-slate-600 hover:border-blue-200 hover:bg-white'
                                }`}
                            >
                                <div className={`w-10 h-10 rounded-xl flex items-center justify-center shadow-sm ${
                                    editorMode === 'word' ? 'bg-white/20' : 'bg-white text-blue-600 border border-slate-100'
                                }`}>
                                    <i className="fa-solid fa-file-word text-lg"></i>
                                </div>
                                <div>
                                    <h5 className="text-[10px] font-black uppercase">Word 模式</h5>
                                    <p className={`text-[7px] mt-1 ${editorMode === 'word' ? 'text-white/70' : 'text-slate-400'}`}>A4 分页编辑</p>
                                </div>
                                {editorMode === 'word' && (
                                    <div className="absolute top-2 right-2">
                                        <i className="fa-solid fa-circle-check text-white text-[10px]"></i>
                            ...
                            <button
                                onClick={() => onEditorModeChange('online')}
                                className={`p-4 rounded-2xl border-2 transition-all flex flex-col items-center justify-center text-center gap-3 relative overflow-hidden ${
                                    editorMode === 'online'
                                        ? 'border-green-500 shadow-md bg-green-600 text-white'
                                        : 'bg-slate-50 border-slate-100 text-slate-600 hover:border-green-200 hover:bg-white'
                                }`}
                            >
                                <div className={`w-10 h-10 rounded-xl flex items-center justify-center shadow-sm ${
                                    editorMode === 'online' ? 'bg-white/20' : 'bg-white text-green-600 border border-slate-100'
                                }`}>
                                    <i className="fa-solid fa-globe text-lg"></i>
                                </div>
                                <div>
                                    <h5 className="text-[10px] font-black uppercase">在线文档</h5>
                                    <p className={`text-[7px] mt-1 ${editorMode === 'online' ? 'text-white/70' : 'text-slate-400'}`}>WPS / 腾讯文档</p>
                                </div>
                                {editorMode === 'online' && (
                                    <div className="absolute top-2 right-2">
                                        <i className="fa-solid fa-circle-check text-white text-[10px]"></i>
                            ...
                            <button
                                onClick={() => onEditorModeChange('onlyoffice')}
                                className={`p-4 rounded-2xl border-2 transition-all flex flex-col items-center justify-center text-center gap-3 relative overflow-hidden ${
                                    editorMode === 'onlyoffice'
                                        ? 'border-orange-500 shadow-md bg-orange-600 text-white'
                                        : 'bg-slate-50 border-slate-100 text-slate-600 hover:border-orange-200 hover:bg-white'
                                }`}
                            >
                                <div className={`w-10 h-10 rounded-xl flex items-center justify-center shadow-sm ${
                                    editorMode === 'onlyoffice' ? 'bg-white/20' : 'bg-white text-orange-600 border border-slate-100'
                                }`}>
                                    <i className="fa-solid fa-server text-lg"></i>
                                </div>
                                <div>
                                    <h5 className="text-[10px] font-black uppercase">OnlyOffice</h5>
                                    <p className={`text-[7px] mt-1 ${editorMode === 'onlyoffice' ? 'text-white/70' : 'text-slate-400'}`}>本地 Word 引擎</p>
                                </div>
                                {editorMode === 'onlyoffice' && (
                                    <div className="absolute top-2 right-2">
                                        <i className="fa-solid fa-circle-check text-white text-[10px]"></i>
                    ...
                    {/* 2.6 Review Workshop — 综述工坊提示 */}
                    <section>
                        <h4 className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-3 px-1 flex items-center gap-2">
                            <i className="fa-solid fa-dna"></i> AI 综述工坊
                        </h4>
                        <div className="p-4 rounded-2xl border-2 border-dashed border-purple-200 bg-gradient-to-r from-purple-50 to-indigo-50 flex items-center gap-4">
                            <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-purple-500 to-indigo-600 flex items-center justify-center shadow-md flex-shrink-0">
                                <span className="text-white text-sm">🧬</span>
                            </div>
                            <div className="text-left flex-1">
                                <h5 className="text-xs font-black text-slate-700">综述工坊已集成到侧边栏</h5>
                                <p className="text-[9px] text-slate-400 mt-0.5">点击侧边栏「综述」 tab 即可启动 AI Agent 管线</p>
                            </div>
                        </div>
                    </section>
