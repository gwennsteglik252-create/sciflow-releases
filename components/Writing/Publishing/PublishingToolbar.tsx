    ...
    <div className="no-print p-2 sm:p-4 flex items-center gap-2 sm:gap-4 shrink-0 bg-slate-800/40 backdrop-blur-md border-b border-white/5 z-50">
      {!isTriple && (
        <button
          ...
          title={isSidebarOpen ? t('writing.publishingToolbar.collapseNav') : t('writing.publishingToolbar.expandNav')}
        >
      ...
      {onBack && (
        ...
        >
          <i className="fa-solid fa-arrow-left"></i> {t('writing.publishingToolbar.backToEditor')}
        </button>
      )}
      ...
      <button
        ...
        title={t('writing.publishingToolbar.exportPdf')}
      >
        <i className="fa-solid fa-print"></i> {t('writing.publishingToolbar.export')}
      </button>
      ...
      <div className="flex items-center gap-1 bg-black/40 p-1 rounded-full border border-white/10 ml-1 sm:ml-2">
        <button
          ...
          title={t('writing.publishingToolbar.zoomOut')}
        >
        ...
        <div className="flex items-center gap-1.5 px-1 sm:px-2 relative">
          ...
          <button
            ...
            title={isAutoZoom ? t('writing.publishingToolbar.manualZoomTip') : t('writing.publishingToolbar.autoZoomTip')}
          >
            AUTO
          </button>
        </div>
        ...
        <button
          onClick={() => handleManualZoom(1)}
          className="w-6 h-6 sm:w-8 sm:h-8 rounded-full flex items-center justify-center text-white hover:bg-white/20 transition-colors"
          title={t('writing.publishingToolbar.zoomIn')}
        >
      ...
      <div className="ml-auto flex items-center gap-2 sm:gap-4 min-w-0">
        {onToggleRightSidebar && viewMode === 'standard' && (
          <button
            ...
            title={isRightSidebarVisible ? t('writing.publishingToolbar.hideRight') : t('writing.publishingToolbar.showRight')}
          >
        ...
        <div className="flex items-center gap-2 sm:gap-3 px-3 sm:px-4 py-1.5 bg-black/30 rounded-2xl border border-white/10 shadow-inner group min-w-0">
          <span className="text-[8px] sm:text-[9px] font-black text-indigo-300 uppercase tracking-widest italic shrink-0 hidden sm:inline">{t('writing.publishingToolbar.template')}</span>
          <select
          ...
          >
            ...
            {filteredTemplates.length === 0 && <option disabled>{t('writing.publishingToolbar.noTemplate')}</option>}
          </select>
        </div>
      </div>
    </div>
