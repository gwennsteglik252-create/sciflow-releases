    ...
    <div className="space-y-6 animate-reveal">
      <div className="p-4 border-2 border-dashed border-indigo-200 rounded-2xl flex flex-col items-center justify-center gap-3 hover:border-indigo-400 hover:bg-indigo-50/30 transition-all group bg-slate-50/50 cursor-pointer" onClick={onRunMirrorAnalysis}>
        ...
        <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest text-center">
          {isProcessing ? t('writing.mirrorPanel.analyzing') : t('writing.mirrorPanel.runMirror')}
        </p>
      </div>
      ...
      {mirrorInsight ? (
        <div className="space-y-4">
          <div className="grid grid-cols-1 gap-3">
            <div className="p-4 bg-indigo-50 border border-indigo-100 rounded-2xl relative overflow-hidden">
              <h5 className="text-[8px] font-black text-indigo-400 uppercase tracking-widest mb-2">{t('writing.mirrorPanel.planIntent')}</h5>
            ...
            <div className="p-4 bg-emerald-50 border border-emerald-100 rounded-2xl relative overflow-hidden">
              <h5 className="text-[8px] font-black text-emerald-400 uppercase tracking-widest mb-2">{t('writing.mirrorPanel.evidence')}</h5>
          ...
          <div className="p-5 bg-slate-900 rounded-[2rem] border border-white/5 shadow-xl relative group">
            <div className="flex justify-between items-center mb-3">
              <h5 className="text-[9px] font-black text-indigo-300 uppercase tracking-widest flex items-center gap-2"><i className="fa-solid fa-wand-magic-sparkles text-amber-300"></i> {t('writing.mirrorPanel.responsiveConclusion')}</h5>
              <button onClick={() => onInsertText(mirrorInsight.synthesisConclusion)} className="text-[8px] font-black text-white px-2 py-1 bg-white/10 rounded-lg hover:bg-indigo-600 transition-all">{t('writing.mirrorPanel.insertToBody')}</button>
            </div>
        ...
        <div className="text-center py-20 opacity-30 flex flex-col items-center gap-4">
          ...
          <p className="text-[10px] font-black uppercase tracking-widest">{t('writing.mirrorPanel.startEngine')}</p>
        </div>
      )}
    </div>
  );
