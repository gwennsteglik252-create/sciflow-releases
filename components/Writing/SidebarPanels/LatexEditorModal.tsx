...
const SNIPPETS = {
  ...
  chem: [
    { label: '反应箭头', code: '\\rightarrow' },
    { label: '可逆符号', code: '\\rightleftharpoons' },
    { label: '气体生成', code: '\\uparrow' },
    { label: '沉淀生成', code: '\\downarrow' },
    { label: 'OER 反应', code: '4OH^- \\rightarrow O_2 + 2H_2O + 4e^-' },
    { label: 'HER 反应', code: '2H^+ + 2e^- \\rightarrow H_2' }
  ]
};
...
export const LatexEditorModal: React.FC<LatexEditorModalProps> = ({ show, onClose, onSave, initialSnippet }) => {
  ...
  const renderPreview = () => {
    try {
      return katex.renderToString(content || '\\text{等待输入...}', {
        displayMode: isBlock,
        throwOnError: false
      });
    } catch (e) {
      return '<span class="text-rose-500">代码语法错误</span>';
    }
  };
      ...
      <div className="bg-white w-full max-w-5xl h-[85vh] rounded-[3rem] p-0 animate-reveal shadow-2xl border border-slate-200 flex flex-col overflow-hidden">
        <header className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h3 className="text-xl font-black text-slate-900 uppercase italic">LaTeX 动力学解析器</h3>
          ...
          <div className="flex items-center gap-3">
            ...
            >
              {isRecognizing ? (
                <><i className="fa-solid fa-circle-notch animate-spin"></i> 正在识别中...</>
              ) : (
                <><i className="fa-solid fa-camera"></i> 智能识图</>
              )}
            </button>
        ...
        <div className="flex-1 flex flex-row min-h-0">
          ...
          <div className="w-56 bg-slate-50 border-r border-slate-100 p-4 flex flex-col gap-6 overflow-y-auto custom-scrollbar shrink-0">
            <section>
              <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-3">数学符号</p>
            ...
            <section>
              <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-3">化学组件</p>
              <div className="grid grid-cols-1 gap-2">
                {SNIPPETS.chem.map(s => (
          ...
          <div className="flex-1 flex flex-col p-6 gap-4 bg-white min-w-0">
            <div className="flex gap-4">
              <div className="flex-1">
                <label className="text-[10px] font-black text-slate-400 uppercase block mb-2 px-1">片段名称 (Title)</label>
                <input
                  ...
                  placeholder="例如: Butler-Volmer 基准公式..."
                  value={title}
            ...
            <div className="relative">
              <div className="relative flex items-center gap-2 bg-white rounded-2xl p-2 border border-slate-200 overflow-hidden">
                <input
                  ...
                  placeholder="AI 助手：描述公式内容（如：OER反应、热力学第二定律...）"
                  value={aiInput}
            ...
            <div className="flex-1 flex flex-col min-h-0 mt-2">
              <div className="flex justify-between items-center mb-2 px-1">
                <label className="text-[10px] font-black text-slate-400 uppercase block">LaTeX 代码编辑器</label>
                <div className="relative">
                  <button
                    type="button"
                  ...
                  {showHelp && (
                    <div className="absolute top-full right-0 mt-2 w-72 bg-white rounded-2xl shadow-2xl border border-slate-200 p-4 z-50 animate-reveal overflow-hidden flex flex-col">
                      <div className="flex justify-between items-center mb-3 shrink-0">
                        <span className="text-[10px] font-black text-slate-800 uppercase tracking-widest">LaTeX 参考手册</span>
                      ...
                      <div className="space-y-4 max-h-64 overflow-y-auto custom-scrollbar pr-1">
                        <section>
                          <p className="text-[8px] font-black text-slate-400 uppercase mb-2">数学基础</p>
                          <div className="grid grid-cols-1 gap-1.5">
                            <div className="flex justify-between bg-slate-50 p-1.5 rounded-lg text-[9px] font-mono"><span className="text-slate-400">分式:</span> {"\\frac{a}{b}"}</div>
                            <div className="flex justify-between bg-slate-50 p-1.5 rounded-lg text-[9px] font-mono"><span className="text-slate-400">根号:</span> {"\\sqrt{x}"}</div>
                            <div className="flex justify-between bg-slate-50 p-1.5 rounded-lg text-[9px] font-mono"><span className="text-slate-400">上下标:</span> {"x^{n}_{i}"}</div>
                            <div className="flex justify-between bg-slate-50 p-1.5 rounded-lg text-[9px] font-mono"><span className="text-slate-400">希腊字母:</span> {"\\alpha, \\beta, \\gamma, \\Delta"}</div>
                          </div>
                        </section>
                        <section>
                          <p className="text-[8px] font-black text-slate-400 uppercase mb-2">化学方程式</p>
                          <div className="grid grid-cols-1 gap-1.5">
                            <div className="flex justify-between bg-slate-50 p-1.5 rounded-lg text-[9px] font-mono"><span className="text-slate-400">电荷:</span> {"OH^{-}"}</div>
                            <div className="flex justify-between bg-slate-50 p-1.5 rounded-lg text-[9px] font-mono"><span className="text-slate-400">反应箭:</span> {"\\rightarrow, \\rightleftharpoons"}</div>
                            <div className="flex justify-between bg-slate-50 p-1.5 rounded-lg text-[9px] font-mono"><span className="text-slate-400">状态:</span> {"\\uparrow, \\downarrow"}</div>
                          </div>
                        </section>
                      </div>
                      <div className="mt-3 pt-2 border-t border-slate-100 text-center">
                        <p className="text-[8px] text-slate-400 italic">提示：在编辑器中直接输入相应代码</p>
                      </div>
                    </div>
                  )}
                </div>
              ...
              <textarea
                ...
                placeholder="使用上方 AI 指令自动生成或直接在此输入代码..."
                value={content}
            ...
            <div className="flex items-center gap-6 p-4 bg-slate-50 rounded-2xl border border-slate-100">
              <div className="flex items-center gap-2">
                <span className="text-[10px] font-black text-slate-500 uppercase">模式:</span>
                <div className="flex bg-white p-1 rounded-lg border border-slate-200">
                  <button onClick={() => setType('math')} className={`px-4 py-1.5 rounded-md text-[9px] font-black uppercase transition-all ${type === 'math' ? 'bg-indigo-600 text-white' : 'text-slate-400'}`}>数学</button>
                  <button onClick={() => setType('chem')} className={`px-4 py-1.5 rounded-md text-[9px] font-black uppercase transition-all ${type === 'chem' ? 'bg-emerald-600 text-white' : 'text-slate-400'}`}>化学</button>
                </div>
              </div>
              <div className="flex items-center gap-2">
                <span className="text-[10px] font-black text-slate-500 uppercase">排版:</span>
                <label className="flex items-center gap-2 cursor-pointer group">
                  ...
                  <span className="text-[9px] font-bold text-slate-600 group-hover:text-indigo-600">块状渲染 (Block Mode)</span>
                </label>
              </div>
            </div>
          </div>
          ...
          <div className="w-96 bg-slate-50 border-l border-slate-100 flex flex-col overflow-hidden shrink-0">
            <div className="p-4 border-b border-slate-200 bg-white flex justify-between items-center shrink-0">
              <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">实时预览 (Live View)</span>
            </div>
        ...
        <footer className="p-6 border-t border-slate-100 flex gap-4 bg-white shrink-0">
          ...
          <button onClick={handleSave} disabled={!content.trim()} className="flex-[2] py-4 bg-indigo-600 text-white rounded-2xl text-[11px] font-black uppercase shadow-xl hover:bg-black transition-all active:scale-95 disabled:opacity-50">保存公式至项目库</button>
        </footer>
      </div>
    </div>,
    document.body
