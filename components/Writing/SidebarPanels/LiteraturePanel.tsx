...
const LiteraturePanel: React.FC<LiteraturePanelProps> = ({
    ...
    const getCitingSections = (resId: string) => {
        // 优先使用 orderedCitations 判断（精确且高效）
        if (orderedCitations && orderedCitations.list.length > 0
            && !orderedCitations.list.some(r => r.id === resId)) return [];
        if (!project?.paperSections) return [];
        const res = (resources || []).find(r => r && r.id === resId);
        if (!res) return [];

        // ① 优先使用 @@CID:resId@@ 精确匹配
        const cidStr = `@@CID:${resId}@@`;

        // ② 向后兼容：surname+year 正则 fallback
        let surnamePattern: RegExp | null = null;
        if (res.type === '专利') {
            const patentTag = `[${res.title}, ${res.source}]`;
            surnamePattern = new RegExp(patentTag.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'i');
        } else {
            const authors = Array.isArray(res.authors) ? res.authors : [];
            if (authors.length > 0 && res.year) {
                ...
                const lastName = firstAuthorName.includes(',')
                    ...
                    : /^[\u4e00-\u9fff]+$/.test(firstAuthorName.trim())
                        ...
                        : (() => { const parts = firstAuthorName.trim().split(/\s+/); return parts[parts.length - 1] || firstAuthorName; })();
                const safeName = lastName.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
                surnamePattern = new RegExp(`${safeName}(?:\\s+et\\s+al\\.?)?,?\\s*${res.year}`, 'i');
            }
        }

        const locations: string[] = [];
        project.paperSections.forEach(s => {
            if (s.content && (s.content.includes(cidStr) || (surnamePattern && surnamePattern.test(s.content)))) {
                locations.push(s.title || s.id);
            }
        });
        if (project.tables) {
            project.tables.forEach(table => {
                const allText = (table.title || '') + ' ' + table.rows.flat().join(' ');
                if (allText.includes(cidStr) || (surnamePattern && surnamePattern.test(allText))) {
