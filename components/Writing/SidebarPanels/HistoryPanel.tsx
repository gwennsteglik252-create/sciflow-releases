    ...
    <div className="flex flex-col gap-4 animate-reveal h-full">
      <div className="bg-indigo-50/50 p-5 rounded-[2rem] border border-indigo-100 shrink-0">
        <p className="text-[9px] font-black text-indigo-500 uppercase tracking-widest mb-3 flex items-center gap-2">
          <i className="fa-solid fa-camera-retro"></i> {t('writing.historyPanel.createSnapshot')}
        </p>
        <div className="flex gap-2">
          <input
            ...
            placeholder={t('writing.historyPanel.snapshotPlaceholder')}
            value={newName}
        ...
        <p className="text-[7px] text-slate-400 mt-2 italic px-1">{t('writing.historyPanel.snapshotNote')}</p>
      </div>

      <div className="flex-1 overflow-y-auto custom-scrollbar space-y-3 pr-1">
        <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest px-1">{t('writing.historyPanel.historyRecords')}</p>
        {snapshots.map((snap) => (
          <div key={snap.id} className="p-4 bg-white rounded-2xl border border-slate-100 shadow-sm hover:shadow-md transition-all group relative overflow-hidden">
            ...
            <div className="grid grid-cols-2 gap-2 mb-3">
              <div className="bg-slate-50 px-2 py-1.5 rounded-lg border border-slate-100">
                <span className="text-[7px] text-slate-400 uppercase block font-black">{t('writing.historyPanel.timestamp')}</span>
                <span className="text-[8px] font-mono text-slate-600 font-bold">{snap.timestamp}</span>
              </div>
              <div className="bg-slate-50 px-2 py-1.5 rounded-lg border border-slate-100">
                <span className="text-[7px] text-slate-400 uppercase block font-black">{t('writing.historyPanel.assetOverview')}</span>
                <div className="flex gap-2 mt-0.5">
                  <span className="text-[8px] font-black text-indigo-500" title={t('writing.historyPanel.sectionCount')}><i className="fa-solid fa-file-lines mr-1"></i>{snap.paperSections.length}</span>
                  <span className="text-[8px] font-black text-emerald-500" title={t('writing.historyPanel.citationCount')}><i className="fa-solid fa-quote-right mr-1"></i>{snap.citedLiteratureIds?.length || 0}</span>
                </div>
              </div>
            </div>
            ...
            <div className="flex gap-2 mt-2 pt-2 border-t border-slate-50 opacity-100 sm:opacity-40 sm:group-hover:opacity-100 transition-opacity">
              ...
              >
                <i className="fa-solid fa-clock-rotate-left"></i> {t('writing.historyPanel.fullRestore')}
              </button>
              <button
                ...
                title={t('writing.historyPanel.deleteSnapshot')}
              >
        ...
        {snapshots.length === 0 && (
          <div className="text-center py-20 opacity-30 flex flex-col items-center gap-3">
            ...
            <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 italic">{t('writing.historyPanel.noSnapshots')}</p>
          </div>
        )}
      </div>
    </div>
