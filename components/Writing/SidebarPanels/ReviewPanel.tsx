    ...
    <div className="space-y-5 animate-reveal">
      <div className="grid grid-cols-1 gap-2 mb-6">
        <div
          ...
          title={t('writing.reviewPanel.runReviewTitle')}
        >
          ...
          <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest group-hover:text-indigo-500">
            {isProcessing ? t('writing.reviewPanel.reviewing') : t('writing.reviewPanel.runReview')}
          </p>
        </div>
        ...
        >
          ...
          {t('writing.reviewPanel.submissionSimulator')}
        </button>
      </div>
      ...
      <div className="flex items-center gap-3 mb-2 px-1">
        ...
        <div>
          <h4 className="text-[10px] font-black text-slate-800 uppercase leading-none">{t('writing.reviewPanel.reviewScore')}</h4>
      ...
      {reviewResult?.critiques && reviewResult.critiques.length > 0 ? (
          ...
          {reviewResult.critiques.map((critique: any, i: number) => {
            ...
            return (
              <div key={i} className={`p-4 rounded-[2rem] border transition-all shadow-sm relative overflow-hidden ${isApplied ? 'bg-emerald-50 border-emerald-200' : (critique.severity === 'critical' ? 'bg-rose-50 border-rose-100' : 'bg-amber-50 border-amber-100')}`}>
                <div className="flex justify-between items-center mb-3">
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      title={t('writing.reviewPanel.locateQuote')}
                    >
                ...
                <div className="space-y-3">
                  <div>
                    <p className="text-[8px] font-black text-slate-400 uppercase mb-1">{t('writing.reviewPanel.originalQuote')}</p>
                  ...
                  {!isApplied && (
                    <div>
                      <p className="text-[8px] font-black text-slate-400 uppercase mb-1">{t('writing.reviewPanel.reviewerComment')}</p>
                  ...
                  {critique.suggestedRevision && !isApplied && (
                    <div className="pt-3 border-t border-slate-200/50">
                      <p className="text-[8px] font-black text-emerald-600 uppercase mb-1">{t('writing.reviewPanel.suggestedRevision')}</p>
                      ...
                      >
                        ...
                        {t('writing.reviewPanel.smartApply')}
                      </button>
                    </div>
                  )}
                  ...
                  {isApplied && (
                    <div className="mt-2 text-[9px] font-black text-emerald-600 italic uppercase">
                      <i className="fa-solid fa-check-double mr-1"></i> {t('writing.reviewPanel.applied')}
                    </div>
                  )}
                </div>
              </div>
        ...
        <div className="text-center py-20 opacity-50 flex flex-col items-center gap-3">
          ...
          <p className="text-[10px] font-black uppercase tracking-[0.2rem] text-slate-400">{t('writing.reviewPanel.noReviews')}</p>
        </div>
      )}
    </div>
  );
