...
export const TableEditorModal: React.FC<TableEditorModalProps> = ({
  ...
  const handleInsertLit = (lit: Literature) => {
    ...
    // 消歧：检查同项目中是否有同姓同年的其他文献
    const projectRes = allResources.filter(r => r.projectId === project?.id);
    const lastNameLower = lastName.toLowerCase();
    const sameGroup = projectRes.filter(r => {
      if (r.type === '专利' || !r.year || !r.authors?.length) return false;
      return getAuthorLastName(r.authors[0]).toLowerCase() === lastNameLower && r.year === lit.year;
    });
    let suffix = '';
    if (sameGroup.length > 1) {
      const idx = sameGroup.findIndex(r => r.id === lit.id);
      if (idx >= 0) suffix = String.fromCharCode(97 + idx);
    }
    const tag = lit.type === '专利' ? ` [${lit.source}] ` : ` (${lastName} et al., ${lit.year}${suffix}) `;
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar space-y-2 pr-1">
                  {filteredLit.map(lit => (
                    <div key={lit.id} onMouseDown={(e) => e.preventDefault()} onClick={() => handleInsertLit(lit)} className="p-3 bg-white rounded-xl border border-slate-100 hover:border-indigo-400 hover:shadow-md transition-all cursor-pointer group">
                      ...
                      <span className="text-[8px] font-bold text-slate-400 uppercase">{(() => { const a = lit.authors?.[0]?.trim() || ''; const parts = a.split(/\s+/); return a.includes(',') ? a.split(',')[0].trim() : /^[\u4e00-\u9fff]+$/.test(a) ? a : parts[parts.length - 1] || a; })()} et al., {lit.year}</span>
                    </div>
                  ))}
                </div>
              </div>
