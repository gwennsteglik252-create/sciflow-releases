...
const PolishingPanel: React.FC<PolishingPanelProps> = ({ 
  ...
  const handlePolishWithSuggestion = async (mode: string) => {
    ...
    if (!selection.trim()) {
        showToast({ message: "请先在左侧输入或选中一段文字", type: 'info' });
        return;
    }
    ...
    try {
        ...
        if (result && result.polishedText) {
            ...
            showToast({ message: '已生成润色建议', type: 'success' });
        }
    } catch (e) {
        showToast({ message: '服务异常', type: 'error' });
    } finally {
  ...
  return (
    <div className="flex flex-col gap-4 animate-reveal">
      <div className="p-3 bg-amber-50 rounded-xl border border-amber-100 text-[9px] text-amber-800 leading-relaxed italic">
        ...
        提示：点击下方模式可生成对应的润色建议卡片，您可以预览并选择性采纳。
      </div>

      {/* 模式选择网格 */}
      <div className="grid grid-cols-2 gap-2">
        {[
            { id: 'academic', label: '学术升级', icon: 'fa-wand-magic-sparkles', color: 'indigo' },
            { id: 'objective', label: '客观修正', icon: 'fa-scale-balanced', color: 'emerald' },
            { id: 'logic', label: '逻辑增强', icon: 'fa-link', color: 'violet' },
            { id: 'formula', label: '公式化', icon: 'fa-square-root-variable', color: 'blue' }
        ].map(m => (
            <button 
                key={m.id}
      ...
      <div className="space-y-4 pt-4 border-t border-slate-100">
          <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest px-1 flex justify-between items-center">
              <span>待处理建议 ({suggestions.length})</span>
              {suggestions.length > 0 && <button onClick={() => setSuggestions([])} className="text-rose-500 hover:underline">清空</button>}
          </p>
          ...
          <div className="space-y-3">
                  ...
                  <div key={s.id} className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden animate-reveal group">
                      ...
                      <div className="p-4 space-y-3">
                          <div className="space-y-1">
                              <p className="text-[7px] font-black text-slate-300 uppercase">润色后 (Polished)</p>
                          ...
                          <div className="space-y-1.5">
                              <p className="text-[7px] font-black text-slate-300 uppercase">改进点 (Reasoning)</p>
              ...
              {suggestions.length === 0 && !internalLoading && (
                  <div className="py-10 text-center opacity-30 flex flex-col items-center gap-2">
                      ...
                      <p className="text-[9px] font-black uppercase">暂无待处理建议</p>
                  </div>
              )}
              ...
              {internalLoading && (
                  <div className="py-10 text-center animate-pulse flex flex-col items-center gap-3">
                      ...
                      <p className="text-[9px] font-black text-indigo-400 uppercase">AI 正在进行多维语义分析...</p>
                  </div>
              )}
          </div>
      </div>
