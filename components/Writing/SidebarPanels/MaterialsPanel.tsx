    ...
    <div className="space-y-5 animate-reveal">
      ...
      {docType === 'report' && (
        <button onClick={onGeneratePlanActual} disabled={isProcessing} className="w-full py-2.5 bg-gradient-to-r from-emerald-600 to-teal-600 text-white rounded-xl text-[9px] font-black uppercase shadow-lg flex items-center justify-center gap-2 hover:scale-[1.01] transition-transform">
          {isProcessing ? <i className="fa-solid fa-spinner animate-spin"></i> : <><i className="fa-solid fa-table-list"></i> {t('writing.materialsPanel.generateCompareTable')}</>}
        </button>
      )}
      {docType === 'patent' && (
        <button onClick={onFlowchartToEmbodiment} disabled={isProcessing} className="w-full py-2.5 bg-gradient-to-r from-violet-600 to-purple-600 text-white rounded-xl text-[9px] font-black uppercase shadow-lg flex items-center justify-center gap-2 hover:scale-[1.01] transition-transform">
          {isProcessing ? <i className="fa-solid fa-spinner animate-spin"></i> : <><i className="fa-solid fa-wand-magic-sparkles"></i> {t('writing.materialsPanel.flowchartToEmbodiment')}</>}
        </button>
      )}
      {selectedLogIds.size > 1 && (
        <button onClick={onSynthesizeResults} disabled={isProcessing} className="w-full py-2.5 bg-gradient-to-r from-indigo-600 to-violet-600 text-white rounded-xl text-[9px] font-black uppercase shadow-lg flex items-center justify-center gap-2 hover:scale-[1.01] transition-transform">
          {isProcessing ? <i className="fa-solid fa-spinner animate-spin"></i> : <><i className="fa-solid fa-layer-group"></i> {t('writing.materialsPanel.compareSelected')} ({selectedLogIds.size})</>}
        </button>
      )}

      <div className="space-y-3 pt-2 border-b border-slate-100 pb-4">
        <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest px-1">{t('writing.materialsPanel.diagnosticArchive')}</p>
      ...
      <div className="space-y-2 pt-2">
        <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest px-1">{t('writing.materialsPanel.researchMaterials')}</p>
                      ...
                      >
                        <div className="flex items-start gap-2.5">
                          ...
                          <div className="flex-1 min-w-0">
                              ...
                              <div className="flex gap-1.5 items-center">
                                {isInserted && (
                                  <button
                                    ...
                                    title={t('writing.materialsPanel.locateCitation')}
                                  >
                            ...
                            <div className="text-[9px] text-slate-600 italic line-clamp-1 leading-tight mb-1.5 cursor-pointer" onClick={() => onInsertText(`[Log:${log.id}]`)} title={t('writing.materialsPanel.insertTag')}>{log.content}</div>
