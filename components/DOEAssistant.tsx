...
const DOEAssistant: React.FC<DOEAssistantProps> = (props) => {
  ...
  const handleExportPDF = async () => {
    if (!state.suggestion || !exportAreaRef.current) {
      showToast({ message: '请先生成或加载推演方案', type: 'info' });
      return;
    }
    showToast({ message: '正在构建高精矢量层...', type: 'info' });
  ...
  const handleBulkSubmit = (validEntries: any[]) => {
    ...
    if (showToast) showToast({ message: `数据集已同步，共计 ${validEntries.length}组观测数据`, type: 'success' });
  };
  ...
  return (
    <div className="h-full flex flex-col gap-4 animate-reveal overflow-hidden px-4 py-2 relative">
      ...
      <div className="flex-1 flex flex-col lg:flex-row gap-4 min-h-0 overflow-hidden relative">
        ...
        <div ref={exportAreaRef} className="flex-1 flex flex-col min-h-0 overflow-y-auto custom-scrollbar relative pr-1 pb-10 bg-white rounded-[2.5rem] border border-slate-100 z-10">
      ...
      {state.diagnosisResult && (
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-md z-[5000] flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-lg rounded-[3rem] p-8 lg:p-10 animate-reveal shadow-2xl relative border-4 border-white flex flex-col overflow-hidden">
            ...
            <div className="flex items-center gap-4 mb-8 shrink-0">
              <div className={`w-14 h-14 rounded-2xl flex items-center justify-center text-2xl text-white shadow-lg ${state.diagnosisResult.diagnosis === 'Synergy' ? 'bg-indigo-600' : 'bg-rose-600'}`}>
                <i className={`fa-solid ${state.diagnosisResult.diagnosis === 'Synergy' ? 'fa-wand-magic-sparkles' : 'fa-circle-exclamation'}`}></i>
              </div>
              <div>
                <h3 className="text-2xl font-black text-slate-800 uppercase italic">离群点科学诊断</h3>
              </div>
            </div>

            <div className="flex-1 overflow-y-auto custom-scrollbar pr-1 space-y-6">
              <div className="bg-slate-50 p-6 rounded-[2rem] border border-slate-100 shadow-inner relative">
                <p className="text-[10px] font-black text-indigo-500 uppercase tracking-widest mb-3">专家推演逻辑 (REASONING)</p>
            ...
            <button
              ...
              className="mt-8 w-full py-4 bg-slate-900 text-white rounded-2xl text-[11px] font-black uppercase shadow-xl hover:bg-indigo-600 transition-all active:scale-95"
            >
              已阅读诊断结论
            </button>
          </div>
      ...
      {state.showSaveModal && (
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-md z-[1200] flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-sm rounded-[2rem] p-8 animate-reveal shadow-2xl border-4 border-white text-center">
            <h3 className="text-xl font-black text-slate-800 mb-4 uppercase italic tracking-tighter">存入推演档案库</h3>
            <input
              autoFocus
              className="w-full bg-slate-50 border border-slate-200 rounded-xl p-4 text-sm font-black outline-none shadow-inner mb-6 text-center focus:border-indigo-400 transition-all"
              placeholder="为推演结果命名..."
              value={state.saveTitle}
              onChange={e => actions.setSaveTitle(e.target.value)}
            />
            <div className="flex gap-3">
              <button onClick={() => actions.setShowSaveModal(false)} className="flex-1 py-3.5 bg-slate-100 text-slate-500 rounded-xl text-[11px] font-black uppercase hover:bg-slate-200 transition-all active:scale-95">取消</button>
              <button onClick={actions.handleSaveResult} disabled={!state.saveTitle.trim()} className="flex-[2] py-3.5 bg-indigo-600 text-white rounded-xl text-[11px] font-black uppercase shadow-xl disabled:opacity-30 hover:bg-black transition-colors">确认保存</button>
            </div>
          </div>
        </div>
      )}
      ...
      {state.showSyncModal && (
        <div className="fixed inset-0 bg-slate-950/40 backdrop-blur-md z-[1200] flex items-center justify-center p-4 no-print">
          <div className="bg-white w-full max-w-sm rounded-[2.2rem] p-8 animate-reveal shadow-2xl border-4 border-white flex flex-col">
            <h3 className="text-lg font-black text-slate-800 mb-6 uppercase italic border-l-8 border-indigo-600 pl-4">同步方案至课题</h3>
            <div className="space-y-4 flex-1 overflow-y-auto custom-scrollbar pr-2">
              <div>
                <label className="text-[10px] font-black text-slate-400 uppercase mb-1.5 block px-1">关联课题</label>
                <select className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3.5 text-xs font-bold text-slate-800 outline-none appearance-none cursor-pointer hover:border-indigo-300 transition-colors shadow-inner" value={state.targetProjectId} onChange={e => { actions.setTargetProjectId(e.target.value); const a = getAutoSelections(props.projects, e.target.value); actions.setTargetMilestoneId(a.milestoneId); }}>
              ...
              {state.targetProjectId && (
                <div className="animate-reveal">
                  <label className="text-[10px] font-black text-slate-400 uppercase mb-1.5 block px-1">指定实验节点</label>
                  <select className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3.5 text-xs font-bold text-slate-800 outline-none appearance-none cursor-pointer hover:border-indigo-300 transition-colors shadow-inner" value={state.targetMilestoneId} onChange={e => actions.setTargetMilestoneId(e.target.value)}>
            ...
            <div className="flex gap-3 pt-6 shrink-0 border-t border-slate-50 mt-4">
              <button onClick={() => actions.setShowSyncModal(false)} className="flex-1 py-3 bg-slate-100 text-slate-500 rounded-xl text-[10px] font-black uppercase hover:bg-slate-200 transition-all">取消</button>
              <button onClick={actions.handleSyncToProject} disabled={!state.targetProjectId || !state.targetMilestoneId} className={`flex-1 py-3 text-white rounded-xl text-[10px] font-black uppercase shadow-xl active:scale-95 transition-all bg-indigo-600 hover:bg-black`}>
                确认同步
              </button>
            </div>
          </div>
