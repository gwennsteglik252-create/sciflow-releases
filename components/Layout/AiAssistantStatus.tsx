           ...
           <div className="flex flex-col gap-1.5">
              <div className="flex items-center gap-2">
                 ...
                 <span className={`text-[9px] font-black uppercase tracking-widest animate-pulse leading-none ${pacingInfo?.active ? 'text-rose-200' : 'text-indigo-200'}`}>
                    {pacingInfo?.active ? `API 繁忙：排队中 (${pacingInfo.delay}s)` : message}
                 </span>
              </div>
      ...
      <div className="mt-2 rounded-xl border border-white/10 bg-slate-900/45 overflow-hidden">
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] font-black uppercase tracking-wider text-cyan-200/90">AI 缓存命中率日志</span>
          </div>
          ...
          <div className="px-3 pb-3 space-y-2.5 border-t border-white/10">
            <div className="grid grid-cols-2 gap-2 pt-2">
              <div className="rounded-lg border border-cyan-500/20 bg-cyan-500/10 p-2">
                <div className="text-[9px] text-cyan-100/70">总请求</div>
                <div className="text-[13px] font-black text-cyan-200">{metrics.requests}</div>
              </div>
              <div className="rounded-lg border border-cyan-500/20 bg-cyan-500/10 p-2">
                <div className="text-[9px] text-cyan-100/70">命中率(缓存+去重)</div>
              ...
              <div className="rounded-lg border border-indigo-500/20 bg-indigo-500/10 p-2">
                <div className="text-[9px] text-indigo-100/70">平均网络耗时</div>
                <div className="text-[13px] font-black text-indigo-200">{avgLatencyMs.toFixed(0)} ms</div>
              </div>
              <div className="rounded-lg border border-emerald-500/20 bg-emerald-500/10 p-2">
                <div className="text-[9px] text-emerald-100/70">成功率(网络尝试)</div>
            ...
            {topTaskTypes.length > 0 && (
              <div className="rounded-lg border border-white/10 bg-black/20 p-2">
                <div className="text-[9px] font-black uppercase tracking-wider text-slate-300 mb-1.5">按任务类型（Top 5）</div>
            ...
            <div className="rounded-lg border border-white/10 bg-black/20 p-2">
              <div className="text-[9px] font-black uppercase tracking-wider text-slate-300 mb-1.5">最近 20 条请求明细时间线</div>
              {metrics.recentRequests && metrics.recentRequests.length > 0 ? (
                ...
                <div className="text-[10px] text-slate-500">暂无请求记录</div>
              )}
            </div>

            <div className="flex items-center justify-between">
              <span className="text-[9px] text-slate-400">
                更新于 {metrics.updatedAt ? new Date(metrics.updatedAt).toLocaleTimeString() : '--:--:--'}
              </span>
              <button
