...
const COMMANDS = [
    { cmd: "Start Timer", action: "启动 30 分钟反应计时" },
...
export const VoiceLabCompanion: React.FC = () => {
    ...
    useEffect(() => {
        ...
        if (isTimerRunning && timer > 0) {
            ...
            showToast({ message: "实验计时结束！", type: 'info' });
        }
    ...
    const handleVoiceCommand = useCallback(async (text: string) => {
        ...
        try {
            ...
            setLastAction(`辅助执行失败: ${e.message}`);
        }
    }, [processCommand]);
        ...
        recognition.onresult = (event: any) => {
            ...
            for (let i = event.resultIndex; i < event.results.length; ++i) {
                if (event.results[i].isFinal) {
                    ...
                    // Wake word detection: "Hey SciFlow" or "嘿 SciFlow"
                    const wakeWords = ["hey sciflow", "嘿 sciflow", "嘿sciflow", "开机", "启动"]; // Adding some common start words
                    ...
                    if (foundWakeWord) {
                        ...
                        // For demonstration or if specifically in a 'listening' state, we might process directly.
                        // But per requirement, we implement wake-word.
                        // Let's allow direct commands if the user just spoke the wake word recently, 
                        // but for simplicity, let's stick to wake word + command in same sentence or just wake word to 'activate'.
                    }
                } else {
    ...
    }, [isVoiceMode, handleVoiceCommand, showToast]);
        ...
        <div className="fixed inset-0 bg-slate-950/95 z-[9000] flex flex-col items-center justify-center p-8 backdrop-blur-xl animate-in fade-in zoom-in duration-300">
            ...
            <div className="w-full max-w-4xl flex flex-col items-center gap-10">
                ...
                <div className="text-center space-y-2">
                    ...
                    <h2 className="text-5xl font-black text-white uppercase italic tracking-tighter">湿实验语音伴侣</h2>
                    <p className="text-slate-400 font-bold text-lg max-w-xl mx-auto">专为通风橱操作设计。大屏显示关键数据，语音控制实验流程。</p>
                </div>
                ...
                <div className="w-full bg-black/40 rounded-[3rem] border border-white/10 p-10 flex flex-col items-center gap-8 relative overflow-hidden shadow-2xl">
                    ...
                    <div className="text-center space-y-4 relative z-10">
                        <p className={`text-4xl font-bold transition-all ${transcript || interimTranscript ? 'text-white' : 'text-slate-600'}`}>
                            "{transcript || interimTranscript || '等待语音指令...'}"
                        </p>
                        {lastAction && (
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-6 w-full">
                    <div className="bg-slate-800/50 p-6 rounded-3xl border border-white/5 flex flex-col items-center justify-center gap-3 text-center group hover:bg-indigo-600/20 hover:border-indigo-500/50 transition-all cursor-pointer h-40">
                        ...
                        <div>
                            ...
                            <p className="text-slate-400 text-xs font-bold uppercase mt-1">倒计时器</p>
                        </div>
                    </div>
                    ...
                    >
                        ...
                        <span className="text-rose-200 font-bold text-lg leading-tight">退出模式</span>
                    </button>
                </div>
