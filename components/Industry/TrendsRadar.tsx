...
const MOCK_TRENDS: TrendItem[] = [
    {
        ...
        category: 'Policy',
        content: '欧盟委员会宣布 CBAM 过渡期启动，涉及钢铁、水泥、铝、化肥、电力 and 氢气六大行业。要求进口商按季度报告碳排放量。该政策将显著增加高碳产品的出口成本，推动企业加速低碳转型。',
        impactScore: 5,
...
const TrendsRadar: React.FC = () => {
    ...
    useEffect(() => {
        ...
        if (selectedIds.size === 0 && trends.length > 0) {
    ...
    const handleManualSearch = async () => {
        ...
        await startGlobalTask(
            { id: `trend_search_${Date.now()}`, type: 'trend_analysis', status: 'running', title: `情报探测: ${radarSearch.substring(0, 10)}...` },
            async () => {
                try {
                    const result = await searchGlobalTrends(radarSearch, timeRange);
                    if (result.items && result.items.length > 0) {
                        setTrends(result.items);
                        setSelectedIds(new Set(result.items.map((t: any) => t.id)));
    ...
    return (
        <div className="h-full flex flex-col animate-reveal p-6 gap-6 bg-slate-50/50">
                ...
                <div className="flex-1 flex flex-col min-h-0">
                    <div className="flex-1 overflow-y-auto custom-scrollbar grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6 pb-20 content-start auto-rows-max">
                        {filteredTrends.map(trend => {
                            ...
                            const isShowingOriginal = showOriginalSet.has(trend.id);
                            const hasSummary = !!trend.summary;

                            return (
                                <div key={trend.id} className={`bg-white rounded-[2.5rem] border-2 transition-all shadow-sm hover:shadow-xl p-6 flex flex-col group relative ${selectedIds.has(trend.id) ? 'border-indigo-600 ring-4 ring-indigo-50 shadow-indigo-100' : 'border-slate-100'}`}>
                                    <div className="flex justify-between items-start mb-4">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <span className={`px-3 py-1 rounded-full text-[8px] font-black uppercase border flex items-center gap-1.5 shadow-sm ${getCategoryColor(trend.category)}`}>
                                                <i className={`fa-solid ${getCategoryIcon(trend.category)}`}></i>
                                                {getCategoryLabel(trend.category)}
                                            </span>
                                        </div>
                                        <div className="flex gap-0.5 text-amber-400 text-[10px]">
                                            {Array.from({ length: 5 }).map((_, i) => (
                                                <i key={i} className={`fa-solid fa-star ${i < trend.impactScore ? 'opacity-100' : 'opacity-20'}`}></i>
                                            ))}
                                        </div>
                                    </div>

                                    <div className="flex-1 min-h-0">
                                        <h4 className="text-sm font-black text-slate-800 leading-tight mb-3 italic group-hover:text-indigo-600 transition-colors">{trend.title}</h4>
                                        ...
                                        <div className="flex items-center gap-3 mb-4 bg-slate-50 px-3 py-2 rounded-xl border border-slate-100">
                                            ...
                                            <div className="flex flex-col">
                                                ...
                                                <span className="text-[9px] font-black text-indigo-600 font-mono">{new Date().toISOString().split('T')[0]}</span>
                                            </div>
                                            <div className="w-px h-5 bg-slate-200"></div>
                                            <div className="flex flex-col">
                                        ...
                                        <div className="p-5 bg-slate-50/50 rounded-3xl border border-slate-100 mb-4 max-h-[280px] flex flex-col relative group/content shadow-inner">
                                            {hasSummary && !isShowingOriginal ? (
                                                <div className="animate-reveal flex flex-col gap-4 h-full min-h-0 overflow-hidden">
                                                    <div className="flex items-center justify-between border-b border-indigo-100 pb-2">
                                                        <div className="flex items-center gap-2">
                                                            <i className="fa-solid fa-wand-magic-sparkles text-indigo-500 text-[10px]"></i>
                                                            <span className="text-[9px] font-black text-indigo-600 uppercase tracking-[0.1rem] italic leading-none">AI 内参摘要 (EXECUTIVE INSIGHT)</span>
                                                        </div>
                                                        <button
                                                            onClick={() => toggleShowOriginal(trend.id)}
                                                            className="text-[8px] font-black text-slate-400 hover:text-indigo-600 uppercase tracking-tighter transition-colors"
                                                        >
                                                            查看原文
                                                        </button>
                                                    </div>
                                                    <p className="text-[11px] font-bold text-slate-800 leading-relaxed italic text-justify flex-1 overflow-y-auto custom-scrollbar pr-1">
                                                        {trend.summary}
                                                    </p>
                                                </div>
                                            ) : (
                                                <div className="flex flex-col gap-4 h-full min-h-0 overflow-hidden">
                                                    {hasSummary && (
                                                        <div className="flex items-center justify-between border-b border-slate-200 pb-2">
                                                            <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest leading-none">原文摘要 (ORIGINAL SOURCE)</span>
                                                            <button
                                                                onClick={() => toggleShowOriginal(trend.id)}
                                    ...
                                    <div className="flex items-center justify-between pt-4 border-t border-slate-100">
                                        <div className="flex items-center gap-2">
                                            {!trend.summary && (
                                                ...
                                                >
                                                    {isProcessingThis ? <i className="fa-solid fa-spinner animate-spin"></i> : <i className="fa-solid fa-bolt"></i>}
                                                    解析内参
                                                </button>
                                            )}
                                            {trend.summary && (
                                                <span className="text-[9px] font-black text-emerald-600 flex items-center gap-1.5 bg-emerald-50 px-3 py-1.5 rounded-xl border border-emerald-100 shadow-sm animate-reveal">
                                                    <i className="fa-solid fa-check-circle"></i>
                                                    内参就绪
                                                </span>
                                            )}
                                        </div>
                                        <span className="text-[9px] font-black text-slate-400 bg-slate-50 px-3 py-1.5 rounded-xl border border-slate-200">ID: {trend.id.slice(-4).toUpperCase()}</span>
                                    </div>
                                </div>
                            );
                        })}
