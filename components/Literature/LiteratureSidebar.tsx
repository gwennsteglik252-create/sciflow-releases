...
import { ResourceType, Literature, MatrixReport, TransformationProposal, AiTask } from '../../types';
import ResourceCard from './ResourceCard';
import VirtualList from '../Common/VirtualList';
import { SortOption } from '../../hooks/useLiteratureLogic';

const SORT_OPTIONS: { value: SortOption; label: string; icon: string }[] = [
  { value: 'default', label: '导入顺序', icon: 'fa-clock-rotate-left' },
  { value: 'year_desc', label: '年份 ↓ 新→旧', icon: 'fa-arrow-down-wide-short' },
  { value: 'year_asc', label: '年份 ↑ 旧→新', icon: 'fa-arrow-up-wide-short' },
  { value: 'title_asc', label: '标题 A→Z', icon: 'fa-arrow-down-a-z' },
  { value: 'title_desc', label: '标题 Z→A', icon: 'fa-arrow-up-z-a' },
  { value: 'author_asc', label: '作者排序', icon: 'fa-user' },
];
...
interface LiteratureSidebarProps {
  ...
  viewMode: 'list' | 'reports' | 'proposals' | 'canvas' | 'benchmarking';
...
const LiteratureSidebar: React.FC<LiteratureSidebarProps> = ({
  ...
  return (
    <div className="col-span-1 lg:col-span-3 flex flex-col gap-3 min-h-[300px] lg:h-full overflow-hidden">
      <div className="flex gap-1 p-0.5 bg-slate-50 rounded-xl shrink-0 shadow-sm border border-slate-200/60">
        <button onClick={() => onTypeChange('文献')} className={`flex-1 py-1.5 rounded-lg text-[10px] font-black uppercase transition-all ${activeType === '文献' ? 'bg-indigo-600 text-white shadow-md' : 'bg-transparent text-slate-400 hover:text-indigo-600'}`}>文献</button>
        <button onClick={() => onTypeChange('专利')} className={`flex-1 py-1.5 rounded-lg text-[10px] font-black uppercase transition-all ${activeType === '专利' ? 'bg-emerald-600 text-white shadow-md' : 'bg-transparent text-slate-400 hover:text-emerald-600'}`}>专利</button>
        <button onClick={() => onTypeChange('商业竞品')} className={`flex-1 py-1.5 rounded-lg text-[10px] font-black uppercase transition-all ${activeType === '商业竞品' ? 'bg-amber-600 text-white shadow-md' : 'bg-transparent text-slate-400 hover:text-amber-600'}`}>商业竞品</button>
      </div>

      {/* 2. Classification & Tag Filter Panel (Collapsible) */}
      {viewMode === 'list' && (
        <div className={`bg-white/60 rounded-2xl border border-slate-200/50 shadow-sm shrink-0 backdrop-blur-sm transition-all duration-300 overflow-hidden ${isPanelExpanded ? 'p-3' : 'p-2'}`}>
          <div className={`flex justify-between items-center cursor-pointer ${isPanelExpanded ? 'mb-2' : 'mb-0'}`} onClick={() => setIsPanelExpanded(!isPanelExpanded)}>
            <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest px-1 flex items-center gap-1">
              <i className="fa-solid fa-layer-group"></i> 分类筛选
            </p>
              ...
              {allTags.length > 0 && (
                <div>
                  <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest px-1 mb-2 flex items-center gap-1">
                    <i className="fa-solid fa-tags"></i> 智能标签云
                  </p>
      ...
      <div className="flex-1 flex flex-col overflow-hidden min-h-0">
        <div className="flex gap-2 shrink-0 mb-3">
          <div className="relative flex-1">
            ...
            <input
              type="text"
              placeholder="搜索标题/作者/摘要/DOI/标签..."
              className="w-full bg-slate-50/50 border border-slate-200/60 rounded-lg pl-8 pr-3 py-1.5 text-[9px] font-bold outline-none shadow-inner focus:ring-1 focus:ring-indigo-200 transition-all"
              value={searchQuery}
              ...
              {showSortMenu && (
                <>
                  ...
                  <div className="absolute right-0 top-full mt-1 z-50 bg-white rounded-2xl shadow-2xl border border-slate-100 py-2 w-40 animate-reveal">
                    <p className="px-3 py-1 text-[7px] font-black text-slate-300 uppercase tracking-widest">排序方式</p>
        ...
        {viewMode === 'list' ? (
          <VirtualList<Literature>
            ...
            emptyContent={
              <div className="text-center py-10 opacity-40">
                ...
                <p className="text-[9px] font-black uppercase text-slate-400">暂无相关资源</p>
              </div>
            }
            renderItem={(item) => (
              <ResourceCard
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar h-[calc(100vh-180px)] pr-1">
            <div className="space-y-3">
                ...
                <div key={rep.id} onClick={() => onSelectReport(rep.id)} className={`p-3 rounded-xl border-2 transition-all cursor-pointer shadow-sm group relative overflow-hidden hover:shadow-md ${selectedReportId === rep.id ? 'bg-violet-600 text-white border-violet-600' : 'bg-white border-slate-100 hover:border-violet-200'}`}>
                  <div className="flex justify-between items-start mb-2">
                    ...
                    <div className="flex gap-1 sm:opacity-0 group-hover:opacity-100 transition-opacity z-20">
                      <button onClick={(e) => { e.stopPropagation(); onRenameReport?.(rep.id); }} className={`p-1.5 rounded-lg transition-all active:scale-90 ${selectedReportId === rep.id ? 'bg-white/20 hover:bg-white/40' : 'bg-slate-100 text-slate-500 hover:bg-indigo-500 hover:text-white'}`} title="重命名">
                        <i className="fa-solid fa-pen text-[8px]"></i>
                      </button>
                      <button onClick={(e) => { e.stopPropagation(); onDeleteReport?.(rep.id); }} className={`p-1.5 rounded-lg transition-all active:scale-90 ${selectedReportId === rep.id ? 'bg-white/20 hover:bg-rose-500' : 'bg-rose-50 text-rose-400 hover:bg-rose-500 hover:text-white'}`} title="删除">
              ...
              {reports.length === 0 && <div className="text-center py-8 text-[9px] text-slate-400 font-bold uppercase">暂无分析报告</div>}
            </div>
          </div>
        ) : (
          <div className="flex-1 overflow-y-auto custom-scrollbar h-[calc(100vh-180px)] pr-1">
            <div className="space-y-3">
              {proposals.map(prop => (
                <div key={prop.id} onClick={() => onSelectProposal(prop.id)} className={`p-3 rounded-xl border-2 transition-all cursor-pointer shadow-sm group relative overflow-hidden hover:shadow-md ${selectedProposalId === prop.id ? `bg-emerald-600 text-white border-emerald-600` : `bg-white border-slate-100 hover:border-emerald-200`}`}>
                  <div className="flex justify-between items-start mb-2">
                    ...
                    <div className="flex gap-1 sm:opacity-0 group-hover:opacity-100 transition-opacity z-20">
                      <button onClick={(e) => { e.stopPropagation(); onRenameProposal?.(prop.id); }} className={`p-1.5 rounded-lg transition-all active:scale-90 ${selectedProposalId === prop.id ? 'bg-white/20 hover:bg-white/40' : 'bg-slate-100 text-slate-500 hover:bg-indigo-500 hover:text-white'}`} title="重命名">
                        <i className="fa-solid fa-pen text-[8px]"></i>
                      </button>
                      <button onClick={(e) => { e.stopPropagation(); onDeleteProposal?.(prop.id); }} className={`p-1.5 rounded-lg transition-all active:scale-90 ${selectedProposalId === prop.id ? 'bg-white/20 hover:bg-rose-500' : 'bg-rose-50 text-rose-400 hover:bg-rose-500 hover:text-white'}`} title="删除">
                        <i className="fa-solid fa-trash-can text-[8px]"></i>
                      </button>
                    </div>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className={`text-[7px] font-bold uppercase ${selectedProposalId === prop.id ? 'text-emerald-100' : 'text-slate-400'}`}>基于: {prop.literatureTitle?.substring(0, 10)}...</span>
              ...
              {proposals.length === 0 && <div className="text-center py-8 text-[9px] text-slate-400 font-bold uppercase">暂无转化建议</div>}
            </div>
          </div>
        )}
      </div>
