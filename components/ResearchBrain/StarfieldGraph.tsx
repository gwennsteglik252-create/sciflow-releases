...
function layoutForceDirected3D(
  ...
  const n3d: N3D[] = nodes.map((n) => ({
    ...
    pos: new THREE.Vector3(
      ...
      (Math.random() - 0.5) * spread,
      (Math.random() - 0.5) * spread * 0.6 // Z轴压缩，让整体更扁平像星盘
    ),
    vel: new THREE.Vector3(0, 0, 0),
  }));

  const projectNode = n3d.find(n => n.type === 'Project');
  if (projectNode) projectNode.pos.set(0, 0, 0);
  ...
  for (let iter = 0; iter < iterations; iter++) {
    for (let i = 0; i < n3d.length; i++) {
      for (let j = i + 1; j < n3d.length; j++) {
        const diff = new THREE.Vector3().subVectors(n3d[i].pos, n3d[j].pos);
        const dist = Math.max(diff.length(), 0.15);
        const force = diff.normalize().multiplyScalar(repulsion / (dist * dist));
        n3d[i].vel.add(force.clone().multiplyScalar(dt));
        n3d[j].vel.sub(force.clone().multiplyScalar(dt));
      }
    }
    for (const edge of edges) {
      const s = n3d.find(n => n.id === edge.source);
      const t = n3d.find(n => n.id === edge.target);
      if (!s || !t) continue;
      const diff = new THREE.Vector3().subVectors(t.pos, s.pos);
      const dist = diff.length();
      const force = diff.normalize().multiplyScalar((dist - linkDist) * attraction * dt);
      s.vel.add(force);
      t.vel.sub(force);
    }
    for (const n of n3d) {
...
function CosmicBackground() {
  ...
  const dustData = useMemo(() => {
    ...
    return { positions, sizes, alphas, count };
  }, []);
  ...
  const twinklePhases = useMemo(() => {
    const arr = new Float32Array(dustData.count);
    for (let i = 0; i < dustData.count; i++) arr[i] = Math.random() * Math.PI * 2;
    return arr;
  }, [dustData.count]);
  ...
  useFrame(({ clock }) => {
    ...
    if (dustRef.current) {
      ...
      // 随机闪烁动画
      const alphaAttr = dustRef.current.geometry.getAttribute('alpha') as THREE.BufferAttribute;
      for (let i = 0; i < dustData.count; i++) {
        const base = dustData.alphas[i];
        const twinkle = Math.sin(t * (1.5 + (i % 7) * 0.3) + twinklePhases[i]) * 0.5 + 0.5;
        alphaAttr.array[i] = base * (0.3 + twinkle * 0.7);
      }
  ...
  return (
    <>
      {nodes.map(node => {
        ...
        // 标签大小层级：Project > 选中 > 高亮/悬停 > 里程碑 > 其他
        let fontSize = '9px';
