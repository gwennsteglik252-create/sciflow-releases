...
export const ElectroResultPanel: React.FC<ElectroResultPanelProps> = ({
    ...
    return (
        <div className="flex flex-col gap-4">
            {/* 主要指标面板 */}
            <div className="bg-white border-2 border-indigo-500/20 shadow-2xl p-5 lg:p-6 rounded-[0.8rem] animate-reveal border-l-[10px] border-l-indigo-600 flex flex-col gap-4 relative overflow-hidden">
                    ...
                    <div className="grid grid-cols-2 gap-2.5">
                        ...
                        {activeMode === 'OER' && (
                            <>
                                ...
                                {/* 理论准确性信息 */}
                                {(analysisResult.oerUncertainty !== undefined || analysisResult.eqPotentialUsed !== undefined) && (
                                    <div className="col-span-2 mt-1 p-2.5 bg-violet-50/50 rounded-2xl border border-violet-100 flex flex-col gap-1">
                                        <p className="text-[7px] font-black text-violet-500 uppercase tracking-widest flex items-center gap-1.5">
                                            <i className="fa-solid fa-atom text-[8px]"></i> 理论精度指示
                                        </p>
                                        <div className="grid grid-cols-2 gap-2 mt-1">
                                            {analysisResult.eqPotentialUsed !== undefined && (
                                                <div className="bg-white rounded-xl p-2 border border-violet-100 text-center">
                                                    <p className="text-[6px] font-black text-slate-400 uppercase">E_eq(RHE, pH修正)</p>
                                                    <p className="text-xs font-black text-violet-700 font-mono">{analysisResult.eqPotentialUsed} V</p>
                                                </div>
                                            )}
                                            {analysisResult.oerUncertainty !== undefined && (
                                                <div className="bg-white rounded-xl p-2 border border-violet-100 text-center">
                                                    <p className="text-[6px] font-black text-slate-400 uppercase">η 不确定度</p>
                                                    <p className="text-xs font-black text-amber-600 font-mono">±{analysisResult.oerUncertainty} mV</p>
                                                </div>
                                            )}
                                            {analysisResult.tafelSlopeUncertainty !== undefined && (
                                                <div className="col-span-2 bg-white rounded-xl p-2 border border-violet-100 text-center">
                                                    <p className="text-[6px] font-black text-slate-400 uppercase">Tafel 斜率不确定度</p>
                                                    <p className="text-xs font-black text-amber-600 font-mono">±{analysisResult.tafelSlopeUncertainty?.toFixed(1)} mV/dec</p>
                                                </div>
                                            )}
                                        </div>
                                    </div>
                ...
                {analysisResult.tafelFit && (
                    <section className="relative z-10 animate-reveal">
                        <div className="bg-gradient-to-br from-violet-50 to-indigo-50 rounded-[0.8rem] p-4 border border-violet-100">
                ...
                {aiConclusion && (
                    <section className="relative z-10">
                        <div className="bg-slate-900 rounded-[1rem] p-4 text-white shadow-2xl relative overflow-hidden group/insight border border-white/5">
                ...
                {(isDeepAnalysing || aiDeepAnalysis) && (
                    <section className="relative z-10">
                        <div className="bg-gradient-to-br from-indigo-50 to-slate-50 rounded-[1rem] p-4 border border-indigo-100 shadow-inner">
            ...
            {sensitivityGrid.length > 0 && (activeMode === 'LSV' || activeMode === 'CV') && (
                <div className="bg-white border border-slate-200 shadow-xl p-5 rounded-[0.8rem] animate-reveal">
