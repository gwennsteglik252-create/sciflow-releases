...
export interface AnalysisResult {
  ...
  oerUncertainty?: number;           // OER η@10 不确定度 (mV)，来自 DFT 系统误差
  tafelSlopeUncertainty?: number;    // Tafel 斜率不确定度 (mV/dec)，来自 R² 反推
  eqPotentialUsed?: number;          // 实际使用的平衡电位 E_eq (V vs RHE)，含 pH 修正
  dftFreeEnergySteps?: DFTFreeEnergyResult; // 四步 CHE 自由能分解
}
...
export interface ElectroParams {
  ...
  // === 理论精度修正参数 ===
  pH: number;                 // 溶液 pH (0-14)，影响平衡电位 E_eq
  temperature: number;        // 温度 K（默认 298.15，影响 K-L 方程 B 系数）
  dftUValue: number;          // DFT+U Hubbard U 值 eV（0 = 纯 GGA，Fe≈4.0, Ni≈6.2）
  zpeCorrectionEv: number;    // ZPE 修正 eV（手动输入，来自 DFT 频率计算）
}
...
export const DEFAULT_ELECTRO_PARAMS: ElectroParams = {
  ...
  // 理论精度修正默认值
  pH: 0,                      // 酸性标准态（RHE = 1.229V）
  temperature: 298.15,        // 25°C 标准温度
  dftUValue: 0,               // 纯 GGA 默认
  zpeCorrectionEv: 0,         // 不修正
};

// ==================== DFT 自由能接口 ====================

/**
 * CHE（计算氢电极）框架下的 OER 四步自由能结果
 * ΔG = ΔE_DFT + ΔZPE - TΔS + ΔG_pH + ΔG_U
 */
export interface DFTFreeEnergyResult {
  dg1: number;                // OH* 形成：H₂O → OH* + H⁺ + e⁻ (eV)
  dg2: number;                // O* 形成：OH* → O* + H⁺ + e⁻ (eV)
  dg3: number;                // OOH* 形成：O* + H₂O → OOH* + H⁺ + e⁻ (eV)
  dg4: number;                // O₂ 脱附：OOH* → O₂ + H⁺ + e⁻ (eV)
  limitingPotential: number;  // 限制电位 U_L = max(ΔG1..4)/e (V)
  overpotentialTheory: number;// 理论过电位 η = U_L - 1.23 (V)
  rds: 1 | 2 | 3 | 4;        // 限速步骤编号
  phCorrectionApplied: number;// 实际使用的 pH 修正 (eV)
  zpeCorrectionApplied: number;// 实际使用的 ZPE 修正 (eV)
}
