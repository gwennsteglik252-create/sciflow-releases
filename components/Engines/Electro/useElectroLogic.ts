...
export const useElectroLogic = ({ projects, onSave, defaultProjectId, defaultMilestoneId, traceRecordId, show }: UseElectroLogicProps) => {
  ...
  const [chartType, setChartType] = useState<'line' | 'bar' | 'scatter' | 'area' | 'box' | 'histogram'>('line');
    ...
    setTimeout(async () => {
      ...
      if (result) {
        ...
        if (activeMode === 'LSV' || activeMode === 'CV') {
          const grid = computeElectroSensitivityGrid(displayData, electroParams);
  ...
  const handleSaveToLog = (thumbnailUrl?: string) => {
    console.log('[Electro:SaveToLog] CALLED', { targetProjectId, targetMilestoneId, targetLogId, activeMode, hasAnalysis: !!analysisResult, hasAiConclusion: !!aiConclusion });
    if (!targetProjectId || !targetMilestoneId) {
      console.warn('[Electro:SaveToLog] ABORTED: missing projectId or milestoneId');
      return;
    }
    ...
    console.log('[Electro:SaveToLog] SENDING to onSave', { dataKeys: Object.keys(deepAnalysisData), mode: deepAnalysisData.mode });
