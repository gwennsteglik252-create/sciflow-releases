...
export const ElectroInputPanel: React.FC<ElectroInputPanelProps> = ({
    ...
    return (
        <div className="bg-white border border-slate-200 shadow-xl p-6 rounded-[0.8rem] flex flex-col gap-5 group">
            {/* 数据输入区 */}
            <section>
                ...
                <div className="relative group/textarea">
                    <textarea
                        className="w-full h-36 bg-slate-50 rounded-[0.8rem] p-5 text-xs font-mono text-emerald-700 outline-none border border-slate-200 focus:border-indigo-400 focus:bg-white transition-all resize-none shadow-inner custom-scrollbar leading-relaxed"
            ...
            <section className="border border-slate-100 rounded-2xl overflow-hidden">
                ...
                {showParams && (
                    <div className="p-4 space-y-3 animate-reveal border-t border-slate-100">
                        ...
                        {/* 理论精度修正（新增） */}
                        <div className="mt-1 p-3 rounded-2xl border-2 border-violet-100 bg-gradient-to-br from-violet-50/60 to-indigo-50/40">
                            <div className="flex items-center gap-2 mb-3">
                                <i className="fa-solid fa-atom text-violet-500 text-[10px]"></i>
                                <span className="text-[9px] font-black text-violet-700 uppercase tracking-widest">🔬 理论精度修正</span>
                                {/* 实时显示修正后的 E_eq */}
                                <span className="ml-auto text-[8px] font-mono font-black text-violet-600 bg-violet-100 px-2 py-0.5 rounded-full">
                                    E_eq = {(1.229 - 0.05916 * electroParams.pH).toFixed(3)} V
                                </span>
                            </div>
                            <div className="grid grid-cols-2 gap-3">
                                <div>
                                    <label className="text-[7px] font-black text-violet-500 uppercase block mb-1">溶液 pH (0-14)</label>
                                    <input
                                        ...
                                        value={electroParams.pH}
                                        onChange={e => setElectroParams({ ...electroParams, pH: Math.max(0, Math.min(14, Number(e.target.value))) })}
                                    />
                                    <p className="text-[6px] text-slate-400 mt-1 italic">影响 E_eq (Nernst)</p>
                                </div>
                                <div>
                                    <label className="text-[7px] font-black text-violet-500 uppercase block mb-1">温度 T (K)</label>
                                    <input
                                        type="number" step="1" min="273" max="373"
                                        className="w-full bg-white border border-violet-200 rounded-xl px-3 py-2 text-xs font-mono text-violet-700 outline-none focus:border-violet-400"
                                        value={electroParams.temperature}
                                        onChange={e => setElectroParams({ ...electroParams, temperature: Number(e.target.value) })}
                                    />
                                    <p className="text-[6px] text-slate-400 mt-1 italic">影响 K-L 方程 B 系数</p>
                                </div>
                                <div>
                                    <label className="text-[7px] font-black text-violet-500 uppercase block mb-1">DFT+U 值 (eV)</label>
                                    <input
                                        type="number" step="0.1" min="0" max="8"
                                        className="w-full bg-white border border-violet-200 rounded-xl px-3 py-2 text-xs font-mono text-violet-700 outline-none focus:border-violet-400"
                                        value={electroParams.dftUValue}
                                        onChange={e => setElectroParams({ ...electroParams, dftUValue: Number(e.target.value) })}
                                    />
                                    <p className="text-[6px] text-slate-400 mt-1 italic">Fe≈4.0, Ni≈6.2, 0=纯GGA</p>
                                </div>
                                <div>
                                    <label className="text-[7px] font-black text-violet-500 uppercase block mb-1">ZPE 修正 (eV)</label>
                                    <input
                                        type="number" step="0.01" min="-1" max="1"
                                        className="w-full bg-white border border-violet-200 rounded-xl px-3 py-2 text-xs font-mono text-violet-700 outline-none focus:border-violet-400"
                                        value={electroParams.zpeCorrectionEv}
                                        onChange={e => setElectroParams({ ...electroParams, zpeCorrectionEv: Number(e.target.value) })}
                                    />
                                    <p className="text-[6px] text-slate-400 mt-1 italic">来自 DFT 频率计算</p>
                                </div>
                            </div>
                            <div className="mt-2 p-2 bg-violet-100/40 rounded-xl border border-violet-100">
                                <p className="text-[7px] text-violet-600 font-bold italic">
                                    pH={electroParams.pH} → ΔG_pH = {(-0.05916 * electroParams.pH).toFixed(4)} eV/步 · T={electroParams.temperature}K
                                    {electroParams.dftUValue > 0 ? ` · U修正 −${(0.10 * electroParams.dftUValue / 4.0).toFixed(3)} eV` : ' · 纯GGA'}
                                </p>
                            </div>
                        </div>
                    </div>
            ...
            <button
                ...
                className="w-full py-5 bg-slate-900 text-white rounded-[0.8rem] text-xs font-black uppercase tracking-[0.2rem] shadow-2xl shadow-indigo-100 hover:bg-indigo-600 transition-all active:scale-95 disabled:opacity-50 flex items-center justify-center gap-3"
            >
