...
export const InventoryHeader: React.FC<InventoryHeaderProps> = React.memo(({ stats, onAdd, onAddPurchase, onShowLowStock, onShowPurchaseList }) => {
  ...
  const handleClearAll = () => {
    setModalOpen('confirm', {
      show: true,
      title: t('inventory.messages.confirmClear'),
      desc: t('inventory.messages.clearDesc'),
      onConfirm: () => {
        setInventory([]);
        showToast({ message: t('inventory.messages.cleared'), type: 'success' });
  ...
  const handleExcelUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    await startGlobalTask(
      { id: 'excel_import', type: 'transformation', status: 'running', title: t('inventory.messages.importingExcel') },
      async () => {
        try {
          ...
          if (allItems.length === 0) {
            showToast({ message: t('inventory.messages.noDataInExcel'), type: 'error' });
            return;
          }
          ...
          showToast({ message: t('inventory.messages.importComplete', { count: allItems.length, name: labName || 'Excel' }), type: 'success' });

        } catch (error) {
          showToast({ message: t('inventory.messages.importError'), type: 'error' });
  ...
  const handleEnrichFormula = async () => {
    ...
    const msgs: string[] = [];
    if (enrichedCount > 0) msgs.push(t('inventory.messages.enrichComplete', { msg: `补全/修正 ${enrichedCount} 条` }));
    if (clearedCount > 0) msgs.push(t('inventory.messages.enrichComplete', { msg: `清除 ${clearedCount} 条旧错误数据` }));
    if (msgs.length > 0) {
      showToast({ message: msgs.join('，'), type: 'success' });
    } else {
      showToast({ message: t('inventory.messages.alreadyEnriched'), type: 'info' });
    }
  };

  return (
    <header className="flex flex-col xl:flex-row justify-between items-stretch gap-4 shrink-0">
      <div className="flex items-center gap-5">
        <div>
          <h2 className="text-2xl font-black text-slate-800 uppercase tracking-tighter italic flex items-center gap-3">
            <i className="fa-solid fa-box-archive text-indigo-600"></i> {t('inventory.title')}
          </h2>
          <p className="text-[9px] font-black text-slate-400 uppercase tracking-[0.2rem] mt-1 pl-10">{t('inventory.subtitle')}</p>
        </div>
      </div>

      <div className="flex flex-col xl:flex-row bg-white/90 p-2 border border-slate-200 shadow-sm rounded-[2rem] gap-4 w-full xl:w-auto">
        {/* Stats */}
        <div className="flex gap-4 px-4 text-center shrink-0 items-center">
          <div className="flex flex-col items-center">
            <span className="text-lg font-black text-slate-800 leading-none">{stats.hardware}</span>
            <span className="text-[7px] font-black text-slate-400 uppercase">{t('inventory.stats.instrument')}</span>
          </div>
          <div className="flex flex-col items-center">
            <span className="text-lg font-black text-slate-800 leading-none">{stats.reagents}</span>
            <span className="text-[7px] font-black text-slate-400 uppercase">{t('inventory.stats.reagent')}</span>
          </div>
          ...
          <div onClick={onShowLowStock} className="flex flex-col items-center cursor-pointer group">
            ...
            <span className={`text-[7px] font-black uppercase ${stats.lowStock > 0 ? 'text-rose-400' : 'text-slate-400'}`}>{t('inventory.stats.lowStock')}</span>
          </div>
          <div className="w-px h-6 bg-slate-100"></div>
          <div onClick={onShowPurchaseList} className="flex flex-col items-center cursor-pointer group">
            ...
            <span className="text-[7px] font-black text-indigo-400 uppercase group-hover:text-indigo-600">{t('inventory.stats.purchaseList')}</span>
          </div>
        </div>
        ...
        <div className="flex gap-2">
          <button
            ...
            title={t('inventory.actions.clearAll')}
          >
            <i className="fa-solid fa-trash-can"></i>
            <span className="ml-2">{t('inventory.actions.clearAll')}</span>
          </button>
          <button
          ...
          >
            {isImporting ? <i className="fa-solid fa-spinner animate-spin"></i> : <><i className="fa-solid fa-file-excel"></i> <span className="ml-2">{t('inventory.actions.import')}</span></>}
          </button>
          {lastImportIds.length > 0 && (
            <button
              onClick={() => {
                ...
                showToast({ message: t('inventory.messages.withdrawComplete', { count: ids.size }), type: 'success' });
                setLastImportIds([]);
              }}
              ...
              title={t('inventory.actions.withdraw')}
            >
              <i className="fa-solid fa-rotate-left"></i>
              <span className="ml-2">{t('inventory.actions.withdraw')}</span>
            </button>
          )}
          {needsEnrichCount > 0 && (
            <button
              ...
              title={isEnriching ? t('common.cancel') : t('inventory.modal.mw')}
            >
              {isEnriching ? <><i className="fa-solid fa-stop mr-2"></i> {t('inventory.actions.cancelEnrich')}</> : <><i className="fa-solid fa-flask mr-2"></i> {t('inventory.actions.enrich')}</>}
            </button>
          )}
          <button
          ...
          >
            <i className="fa-solid fa-cart-plus"></i> {t('inventory.actions.addPurchase')}
          </button>
          <button
            onClick={onAdd}
          ...
          >
            <i className="fa-solid fa-plus"></i> {t('inventory.actions.addAsset')}
          </button>
        </div>
