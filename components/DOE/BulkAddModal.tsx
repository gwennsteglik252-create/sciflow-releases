...
const BulkAddModal: React.FC<BulkAddModalProps> = ({ show, onClose, factors, responses, onSubmit, initialData = [] }) => {
  ...
  return (
    <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-md z-[1300] flex items-center justify-center p-4">
      <div className="bg-white w-full max-w-7xl rounded-[3rem] p-8 lg:p-12 animate-reveal shadow-2xl relative border-4 border-white flex flex-col max-h-[90vh]">
        ...
        <h3 className="text-2xl font-black text-slate-800 mb-8 uppercase italic border-l-8 border-indigo-600 pl-6 shrink-0">批量录入观测数据</h3>
        
        <div className="flex-1 overflow-auto custom-scrollbar mb-8 border border-slate-100 rounded-3xl bg-slate-50/50">
          <table className="w-full text-left border-collapse min-w-[800px]">
            <thead className="sticky top-0 bg-slate-700 text-white z-10">
              <tr>
                ...
                <th className="p-4 text-[10px] font-black uppercase w-24 text-center bg-slate-800">操作</th>
              </tr>
            </thead>
                ...
                <tr key={idx} className="border-b border-slate-100 hover:bg-slate-50 transition-colors group">
                  ...
                  {responses.map(r => (
                    <td key={r.name} className="p-2 border-r border-slate-100">
                      <input type="number" className="w-full bg-transparent p-2 text-xs font-black text-center outline-none text-emerald-600" value={entry.responses[r.name]} onChange={e => updateValue(idx, 'responses', r.name, e.target.value)} placeholder="数值" />
                    </td>
                  ))}
                  <td className="p-2 text-center">
                    <button 
                      ...
                      className="w-9 h-9 rounded-xl bg-rose-50 text-rose-400 transition-all hover:bg-rose-500 hover:text-white disabled:opacity-0 flex items-center justify-center mx-auto shadow-sm active:scale-90 border border-rose-100"
                    >
        ...
        <div className="flex justify-between items-center shrink-0 pt-4 border-t border-slate-100">
          <div className="flex items-center gap-2">
             <div className="text-[10px] font-bold text-slate-400 uppercase tracking-widest bg-slate-50 px-3 py-1.5 rounded-lg border border-slate-100">
                当前准备录入 ({entries.length})
             </div>
             <button onClick={addRow} className="px-5 py-2.5 bg-indigo-50 text-indigo-600 border border-indigo-200 rounded-xl text-[10px] font-black uppercase hover:bg-indigo-600 hover:text-white transition-all flex items-center gap-2 active:scale-95 shadow-sm">
                <i className="fa-solid fa-plus"></i> 添加新数据行
             </button>
          </div>
          <div className="flex gap-4">
            <button onClick={onClose} className="px-8 py-3 bg-slate-100 text-slate-500 rounded-2xl text-[10px] font-black uppercase hover:bg-slate-200 transition-all">取消</button>
            <button onClick={handleConfirm} className="px-10 py-3 bg-indigo-600 text-white rounded-2xl text-[10px] font-black uppercase shadow-xl hover:bg-black transition-all active:scale-95">存入数据集</button>
          </div>
        </div>
      </div>
    </div>
