...
interface XrdDataset {
    ...
    name: string;
    rawPoints: DataPoint[];
    originalPoints?: DataPoint[]; // 存储原始数据用于重置
    peaks: any[];
    matchedPhases: any[];
    color: string;
    offsetY?: number; // 瀑布图垂直偏移量
    // 数据集级关联实验记录
...
interface PhaseEvolutionResult {
...
interface MultiDatasetXrdResult {
...
interface XrdWorkspaceSnapshot {
    datasets: XrdDataset[];
...
const XRD_GLOBAL_CARD_LIBRARY_KEY = 'sciflow_xrd_global_card_library';

interface PeakDetectSettings {
...
const DEFAULT_PEAK_SETTINGS: PeakDetectSettings = {
    mode: 'balanced',
    maxPeaksMode: 'auto',
    maxPeaks: 8,
...
const normalizeAndSortPeaks = (peaks: any[]): PanelPeak[] => {
    return [...(peaks || [])]
...
const formatFixed = (value: any, digits: number, fallback = '--'): string => {
    const n = toOptionalNumber(value);
    return n === null ? fallback : n.toFixed(digits);
};

const localizePhaseConfidence = (confidence: string): string => {
...
const normalizePhaseKey = (value: any): string => {
...
const collectPhaseKeysFromGroups = (groups: any[]): Set<string> => {
...
const buildXrdPostMatchDeepReport = (postMatchAnalysis: any, matchRationale: any[] | null): string | undefined => {
...
const normalizePostMatchAnalysis = (value: any) => {
...
// 示例数据生成器 - 模拟不同浓度梯度的 XRD 衍射数据
const generateXrdDemoData = (): XrdDataset[] => {
    ...
    return concentrations.map((name, idx) => {
        ...
        const shift = idx * 0.02; // 模拟由于掺杂引起的微小峰位漂移
        const heightScale = 1 - idx * 0.05; // 模拟强度衰减

        for (let x = 20; x <= 80; x += 0.1) {
            let y = 10 + Math.random() * 5; // 背景
