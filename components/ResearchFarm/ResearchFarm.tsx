...
// ─── Types ──────────────────────────────────────────────────────────────────
type PlotStatus = 'empty' | 'planted' | 'growing' | 'ready' | 'withered';
type TimeOfDay = 'dawn' | 'day' | 'sunset' | 'night';
type WeatherType = 'clear' | 'rain' | 'snow' | 'thunder' | null;
interface Seed { id: string; name: string; emoji: string; color: string; glowColor: string; growthTime: number; yield: number; description: string; stages: string[]; icon?: string; unlockLevel?: number; }
interface Plot { id: number; status: PlotStatus; seed: Seed | null; growthTick: number; waterLevel: number; fertLevel: number; shielded?: boolean; }
interface ShopItem { id: string; name: string; emoji: string; description: string; price: number; effect: string; }
interface InventoryItem { id: string; name: string; emoji: string; seedId: string; harvestedAt: number; yield: number; }
interface DailyCheckin { lastDate: string; streak: number; totalDays: number; }
interface FarmExpansion { rows: number; cols: number; }
interface LabUpgrades { yieldBoost: number; waterRetention: number; autoHarvest: boolean; }
interface Challenge { id: string; description: string; target: number; current: number; reward: number; type: 'harvest' | 'papers'; timeLeft: number; active: boolean; }
interface JournalEntry { id: string; ts: number; text: string; emoji: string; }
interface FarmState { plots: Plot[]; papers: number; patents: number; citations: number; coins: number; totalHarvests: number; events: GameEvent[]; level: number; xp: number; goals: ResearchGoal[]; achievements: Achievement[]; inventory: InventoryItem[]; boosts: { speedUp: number; shields: number; autoWater: number }; checkin: DailyCheckin; expansion: FarmExpansion; petActive: boolean; prestige: number; lastSeen: number; labUpgrades: LabUpgrades; mutations: number; journal: JournalEntry[]; plantedSeedIds: string[]; farmTheme: string; gachaCount: number; }
interface GameEvent { id: string; type: 'success' | 'warning' | 'info' | 'danger'; message: string; ts: number; }
interface ResearchGoal { id: string; description: string; target: number; current: number; reward: number; type: 'harvest' | 'papers' | 'coins'; completed: boolean; }
interface Achievement { id: string; name: string; emoji: string; description: string; unlocked: boolean; unlockedAt?: number; }
type ActionType = 'water' | 'fertilize' | 'weed' | 'harvest' | 'plant' | null;
interface HarvestParticle { id: string; x: number; y: number; emoji: string; delay: number; }

// ─── Random Event Definitions ─────────────────────────────────────────────
interface RandomEventDef { id: string; emoji: string; type: GameEvent['type']; message: string; probability: number; effect: (s: FarmState) => FarmState; }
...
const INITIAL_ACHIEVEMENTS: Achievement[] = ACHIEVEMENT_DEFS.map(a => ({ ...a, unlocked: false }));

// ─── Time of Day ─────────────────────────────────────────────────────────
const getTimeOfDay = (): TimeOfDay => { const h = new Date().getHours(); if (h >= 5 && h < 10) return 'dawn'; if (h >= 10 && h < 17) return 'day'; if (h >= 17 && h < 20) return 'sunset'; return 'night'; };
const TIME_OF_DAY_CONFIG: Record<TimeOfDay, { bg: string; label: string; emoji: string; overlay: string }> = {
...
const STORAGE_KEY = 'sciflow_research_farm_v16';
const DEFAULT_COLS = 4; const DEFAULT_ROWS = 3; const TICK_MS = 2500;
const makePlot = (id: number): Plot => ({ id, status: 'empty', seed: null, growthTick: 0, waterLevel: 60, fertLevel: 60, shielded: false });
...
const CHECKIN_REWARDS = [5, 8, 12, 15, 20, 25, 35]; // Day 1-7 rewards
const PRESTIGE_BONUS = 0.15; // +15% growth per prestige level
const PRESTIGE_MIN_LEVEL = 5; // Min level to prestige
const MUTATION_CHANCE = 0.1; // 10% chance

const LAB_UPGRADES_SHOP = [
...
const INITIAL_GOALS: ResearchGoal[] = [
        ...
        <div className="h-full flex flex-col text-slate-900 relative overflow-hidden" style={{ backgroundColor: timeOfDay === 'night' ? '#101828' : '#95B182' }}>
            ...
            <div className="relative z-10 flex flex-col h-full pointer-events-none">
                ...
                <div className="flex-1 flex items-center justify-center relative translate-y-[60px] -translate-x-[30px]">
                    ...
                    {/* Mission Panel */}
                    <div className="absolute right-8 top-1/2 -translate-y-1/2 w-72 bg-white/20 border border-white/60 rounded-[3rem] p-6 backdrop-blur-3xl shadow-2xl flex flex-col gap-6 pointer-events-auto">
                        <h3 className="text-[10px] font-black text-slate-500 tracking-[0.4em] text-center border-b border-black/5 pb-4">TARGETS</h3>
                        <div className="space-y-3">{state.goals.map(g => (
                            <div key={g.id} className={`p-4 rounded-3xl border transition-all ${g.completed ? 'bg-emerald-500/10 border-emerald-500/20' : 'bg-white/40 border-white/40'}`}>
                                <div className="flex items-center justify-between">
                                    <p className={`text-[10px] font-black tracking-tight ${g.completed ? 'text-emerald-700 line-through' : 'text-slate-600'}`}>{g.description}</p>
                                    {g.completed && <span className="text-sm">✅</span>}</div>
                                <div className="flex items-center gap-2 mt-2">
                                    ...
                                    <span className="text-[8px] font-bold text-slate-500">{Math.min(g.current, g.target)}/{g.target}</span></div>
                            </div>))}</div>
                        <div className="h-28 overflow-y-auto scrollbar-hide pt-2 border-t border-black/5">
                            <p className="text-[8px] font-black text-slate-400 tracking-[0.3em] mb-2">EVENT LOG</p>
                            {state.events.slice(0, 5).map(ev => (
                                <div key={ev.id} className={`text-[9px] font-bold py-1.5 border-b border-black/5 ${ev.type === 'danger' ? 'text-red-400' : ev.type === 'warning' ? 'text-amber-500' : ev.type === 'success' ? 'text-emerald-500' : 'text-slate-400'}`}>{ev.message}</div>
                            ))}</div>
                        {challenge && (
                            <div className="mt-2 p-3 bg-red-50 border border-red-200 rounded-2xl animate-pulse">
                                <div className="flex justify-between items-center mb-1">
                                    <p className="text-[9px] font-black text-red-600">🎯 {challenge.description}</p>
                                    <span className="text-[10px] font-black text-red-500">{challenge.timeLeft}s</span>
                                </div>
                                ...
                                <p className="text-[8px] text-red-400 mt-1 text-right">{challenge.current}/{challenge.target} · 奖励 {challenge.reward}💰</p>
                            </div>
                        )}
                    </div>
                </div>
