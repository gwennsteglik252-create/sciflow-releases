...
const ResearchFarm: React.FC = () => {
    ...
    const buyShopItem = (item: ShopItem) => {
        ...
        if (item.effect === 'speedUp') { setSpeedBoostActive(true); setActiveToast({ id: Date.now().toString(), type: 'info', message: '⚡ 加速器启动！生长速度翻倍 60 秒！', ts: Date.now() }); addTimer(() => setSpeedBoostActive(false), 60_000); }
