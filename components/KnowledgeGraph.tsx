...
const KnowledgeGraph: React.FC<KnowledgeGraphProps> = ({ projects, resources }) => {
  ...
  const handleGenerateReport = async () => {
    ...
    if (!targetId) {
        if (showToast) showToast({ message: t('knowledgeGraph.selectNodeFirst'), type: 'info' });
        return;
    }

    const pathIds = highlightedPath.size > 0 ? highlightedPath : findUpstreamPath(targetId, edges);
    if (showToast) showToast({ message: t('knowledgeGraph.generatingReport'), type: 'info' });
    try {
        ...
        if (showToast) showToast({ message: t('knowledgeGraph.reportExported'), type: 'success' });
    } catch (error) {
        if (showToast) showToast({ message: t('knowledgeGraph.reportFailed'), type: 'error' });
    }
  };
  ...
  const handleTraceLogClick = (logId: string | undefined) => {
    ...
    if (project) {
        ...
        if (showToast) showToast({ message: t('knowledgeGraph.projectNotFound'), type: 'error' });
    }
  };
  ...
  useEffect(() => {
    ...
    activeProject.milestones.forEach((ms, i) => {
        ...
        ms.logs.forEach((log, j) => {
            ...
            nodesList.push({
                id: logId,
                label: `${t('knowledgeGraph.logPrefix')}${log.content.substring(0, 8)}`,
                ...
                charData: { linkedLogId: log.id, analysisText: log.aiInsight || t('knowledgeGraph.noAiAnalysis') }
            });
  ...
  const getTypeDisplay = (type: string) => {
      switch(type) {
          case 'Project': return t('knowledgeGraph.nodeTypes.project');
          case 'Literature': return t('knowledgeGraph.nodeTypes.literature');
          case 'Patent': return t('knowledgeGraph.nodeTypes.patent');
          case 'Characterization': return t('knowledgeGraph.nodeTypes.characterization');
          case 'Cost': return t('knowledgeGraph.nodeTypes.cost');
          case 'TRL_Milestone': return t('knowledgeGraph.nodeTypes.trlMilestone');
          case 'Metric': return t('knowledgeGraph.nodeTypes.metric');
          default: return type;
      }
  };
      ...
      <header className="absolute top-0 left-0 right-0 p-6 flex flex-col md:flex-row justify-between items-start z-10 pointer-events-none gap-4">
         <div className="pointer-events-auto flex flex-col">
            <div className="flex flex-col">
                <h2 className={`text-2xl font-black italic uppercase tracking-tighter mb-1 border-l-4 border-indigo-500 pl-4 ${isLightMode ? 'text-slate-800' : 'text-white'}`}>{t('knowledgeGraph.title')}</h2>
         ...
         <div className="flex gap-3 pointer-events-auto items-start">
            ...
            <div className={`p-1.5 h-12 rounded-2xl border flex items-center gap-3 backdrop-blur-md shadow-lg ${isLightMode ? 'bg-white/80 border-slate-200' : 'bg-slate-900/80 border-white/10'}`}>
                ...
                <div className="flex flex-col min-w-0 pr-4">
                    <span className="text-[7px] font-black text-slate-400 uppercase tracking-widest leading-none mb-1">{t('knowledgeGraph.switchProject')}</span>
            ...
            {pathCost && showSuccessPath && (
                <div className={`px-4 py-2 h-12 rounded-xl backdrop-blur-md border animate-reveal flex flex-col justify-center ${isLightMode ? 'bg-emerald-50/80 border-emerald-200' : 'bg-emerald-900/50 border-emerald-500/30'}`}>
                    <p className="text-[8px] font-black text-emerald-500 uppercase tracking-widest leading-none mb-1">{t('knowledgeGraph.pathCost')}</p>
            ...
            >
                <i className="fa-solid fa-code-branch mr-2"></i> {t('knowledgeGraph.traceSuccessPath')}
            </button>
            <button 
            ...
            >
                <i className="fa-solid fa-file-export mr-2"></i> {t('knowledgeGraph.exportReport')}
            </button>
         </div>
      </header>
              ...
              <div className="flex justify-between items-start mb-6">
                  <div>
                      <div className="flex items-center gap-2 mb-1">
                          ...
                          <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{getTypeDisplay(selectedNode.type)} {t('knowledgeGraph.nodeLabel')}</span>
                      </div>
              ...
              <div className="flex-1 overflow-y-auto custom-scrollbar space-y-6 pr-1">
                  {selectedNode.meta && (
                      <div className={`p-4 rounded-2xl ${isLightMode ? 'bg-slate-50' : 'bg-white/5'} border border-white/10`}>
                          <p className="text-[10px] font-black text-slate-400 uppercase mb-3 tracking-widest border-b border-white/5 pb-1">{t('knowledgeGraph.nodeAttributes')}</p>
                  ...
                  {selectedNode.type === 'Characterization' && selectedNode.charData && (
                      <div className="space-y-3">
                          <div className={`p-4 rounded-2xl ${isLightMode ? 'bg-slate-50' : 'bg-slate-800'} border border-indigo-100/30`}>
                              <p className="text-[9px] text-slate-400 uppercase mb-2 font-black tracking-widest">{t('knowledgeGraph.aiDeepAnalysis')}</p>
                          ...
                          {selectedNode.charData.linkedLogId && (
                              ...
                              >
                                  <i className="fa-solid fa-eye mr-2"></i> {t('knowledgeGraph.viewOriginalLog')}
                              </button>
                          )}
                      </div>
                  )}
                  ...
                  {selectedNode.type === 'Cost' && selectedNode.costData && (
                      <div className={`border p-6 rounded-[2rem] text-center ${isLightMode ? 'bg-emerald-50 border-emerald-200' : 'bg-emerald-900/20 border-emerald-500/30'}`}>
                          <p className="text-[10px] font-black text-emerald-500 uppercase tracking-widest mb-3">{t('knowledgeGraph.unitCostAssessment')}</p>
