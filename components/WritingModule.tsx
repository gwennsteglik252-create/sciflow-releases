...
import { useReviewWorkshop, ReviewWorkshopBridge } from '../hooks/useReviewWorkshop';
import { useWritingLogic } from '../hooks/useWritingLogic';
import { useProjectContext } from '../context/ProjectContext';
import { SECTION_CONFIG, DocType } from './Writing/WritingConfig';
import WysiwygEditor from './Writing/EditorParts/WysiwygEditor';
import { useDocxMode } from '../hooks/useDocxMode';
import WordOnlineEditor from './Writing/WordOnlineEditor';
import OnlyOfficeLocalEditor from './Writing/OnlyOfficeLocalEditor';
...
const WritingModule: React.FC<WritingModuleProps> = ({
  ...
  const { activeTheme, showToast, setAppSettings, setResources } = useProjectContext();
  ...
  const [editorMode, setEditorMode] = useState<'classic' | 'word' | 'online' | 'onlyoffice'>('classic');
  ...
  // ═══ Word 文件导入/导出 ═══
  const { fileState: docxFileState, openDocx, saveDocx, markDirty, closeDocx } = useDocxMode();

  const handleOpenDocx = useCallback(async () => {
    const html = await openDocx();
    if (html && wysiwygRef.current) {
      // 将 mammoth 输出的 HTML 直接注入 TipTap 编辑器
      wysiwygRef.current.insertHtml?.(html) || wysiwygRef.current.insertText?.(html);
    }
  }, [openDocx]);
  ...
  const handleSaveDocx = useCallback(async () => {
    ...
    const ok = await saveDocx(html, false);
    if (ok) showToast({ message: `Word 文档已保存`, type: 'success' });
  }, [saveDocx, showToast]);

  const handleSaveDocxAs = useCallback(async () => {
    const editor = wysiwygRef.current?.getEditor?.();
    if (!editor) return;
    const html = editor.getHTML();
    const ok = await saveDocx(html, true);
    if (ok) showToast({ message: `Word 文档已另存`, type: 'success' });
  }, [saveDocx, showToast]);

  const handleCloseDocx = useCallback(() => {
    closeDocx();
    showToast({ message: '已关闭 Word 文档', type: 'info' });
  }, [closeDocx, showToast]);
  ...
  const handleUniversalInsertText = useCallback((text: string) => {
    if (editorMode === 'word' && wysiwygRef.current) {
      wysiwygRef.current.insertText(text);
    } else {
      actions.handleInsertText(text);
    }
  }, [editorMode, actions]);

  // ═══ 综述工坊 Hook（桥接数据到侧边栏各面板）═══
  const reviewWorkshopBridge = useMemo<ReviewWorkshopBridge>(() => ({
    onLiteratureFound: (newLiteratures, topic) => {
      if (!selectedProject) return;
      // 用综述主题简写作为分组标签（截取前12字 + "综述"后缀）
      const shortTopic = topic.length > 12 ? topic.slice(0, 12) + '…' : topic;
      const categoryLabel = `综述:${shortTopic}`;

      const existingIds = new Set(resources.map(r => r.id));
      const toAdd = newLiteratures
        .filter(lit => !existingIds.has(lit.id))
        .map(lit => ({
          ...lit,
          projectId: selectedProject.id,
          categories: [...new Set([...(lit.categories || []), categoryLabel])],
          tags: [...(lit.tags || []), '综述工坊']
        }));
      if (toAdd.length > 0) {
        setResources(prev => [...prev, ...toAdd]);
        showToast({ message: `已导入 ${toAdd.length} 篇文献到「${categoryLabel}」分组`, type: 'success' });
      } else {
        // 已存在的文献也追加分组标签
        setResources(prev => prev.map(r => {
          if (newLiteratures.some(nl => nl.id === r.id) && !(r.categories || []).includes(categoryLabel)) {
            return { ...r, categories: [...(r.categories || []), categoryLabel] };
          }
          return r;
        }));
        showToast({ message: `文献已存在，已追加「${categoryLabel}」分组标签`, type: 'info' });
      }
    },
    onOutlineGenerated: (sections) => {
      if (!selectedProject || !onUpdateProject) return;
      actions.handleAddSection(sections);
      showToast({ message: `已同步 ${sections.length} 个章节到大纲`, type: 'success' });
    },
    onSectionContentUpdate: (sectionId, title, content) => {
      if (!selectedProject || !onUpdateProject) return;
      const updatedSections = (selectedProject.paperSections || []).map(s =>
        s.id === sectionId ? { ...s, content, title: title || s.title } : s
      );
      onUpdateProject({ ...selectedProject, paperSections: updatedSections });
    },
    onAuditComplete: (report) => {
      // 审计报告将通过 reviewResult 显示在审阅 tab
      showToast({ message: '综述审计完成，请查看审阅 tab', type: 'success' });
    },
    onFigureGenerated: (figure) => {
      // ★ 将综述工坊生成的图表自动存入科研绘图中心的库
      try {
        const now = new Date().toLocaleString();
        const reviewTag = '[综述工坊]';

        if (figure.figureType === 'sankey_chart') {
          const key = 'sciflow_sankey_library';
          const existing = JSON.parse(localStorage.getItem(key) || '[]');
          const newSave = {
            id: `review_sankey_${Date.now()}`,
            title: `${reviewTag} ${figure.sectionTitle}`,
            timestamp: now,
            data: figure.sankeyData,
          };
          localStorage.setItem(key, JSON.stringify([newSave, ...existing]));
          showToast({ message: `桑基图已同步到科研绘图中心`, type: 'success' });

        } else if (figure.figureType === 'timeline_chart') {
          const key = 'sciflow_timelines_library';
          const existing = JSON.parse(localStorage.getItem(key) || '[]');
          const newSave = {
            id: `review_timeline_${Date.now()}`,
            title: `${reviewTag} ${figure.sectionTitle}`,
            timestamp: now,
            data: figure.timelineData,
          };
          localStorage.setItem(key, JSON.stringify([newSave, ...existing]));
          showToast({ message: `时间线已同步到科研绘图中心`, type: 'success' });

        } else if (figure.figureType === 'structural_diagram') {
          // 结构图存入结构设计器 session（覆盖当前画布）
          const key = 'sciflow_structural_library';
          const existing = JSON.parse(localStorage.getItem(key) || '[]');
          const newSave = {
            id: `review_structural_${Date.now()}`,
            title: `${reviewTag} ${figure.sectionTitle}`,
            timestamp: now,
            data: figure.structuralData,
          };
          localStorage.setItem(key, JSON.stringify([newSave, ...existing]));
          showToast({ message: `机理图已同步到科研绘图中心`, type: 'success' });

        } else if (figure.figureType === 'summary_infographic') {
          const key = 'sciflow_summary_library';
          const existing = JSON.parse(localStorage.getItem(key) || '[]');
          const newSave = {
            id: `review_infographic_${Date.now()}`,
            title: `${reviewTag} ${figure.sectionTitle}`,
            timestamp: now,
            data: figure.infographicData,
          };
          localStorage.setItem(key, JSON.stringify([newSave, ...existing]));
          showToast({ message: `综述圆环图已同步到科研绘图中心`, type: 'success' });
        }
      } catch (err) {
        console.warn('[ReviewWorkshop → FigureCenter] 图表同步失败:', err);
      }
    }
  }), [selectedProject, resources, onUpdateProject, actions, showToast, setResources]);

  const workshop = useReviewWorkshop(resources, handleUniversalInsertText, undefined, reviewWorkshopBridge);
  ...
  const handleUpdateMediaAsset = (media: any, newName: string, newCaption: string, subFigures?: { label: string; desc: string }[]) => {
    ...
    if (media.logId === 'PROJECT_LEVEL') {
      ...
      showToast({ message: t('writingModule.mediaUpdated'), type: 'success' });
      return;
    }
    ...
    showToast({ message: t('writingModule.assetUpdated'), type: 'success' });
  };
  ...
  const commonSidebarProps = useMemo(() => ({
    ...
    onInsertText: handleUniversalInsertText,
    onCiteLiterature: (res: Literature) => handleUniversalInsertText(actions.getCitationTag(res)),
    ...
    onDeleteMedia: (logId: string, fileIndex: number) => {
      setConfirmConfig({
        show: true,
        title: t('writingModule.confirmDeleteMedia'),
        desc: t('writingModule.confirmDeleteMediaDesc'),
        onConfirm: () => {
    ...
    onInsertFigure: (refId: string) => handleUniversalInsertText(`[Fig:${refId}]`),
    ...
    onDeleteTable: (id: string) => {
      setConfirmConfig({
        show: true,
        title: t('writingModule.confirmDeleteTable'),
        desc: t('writingModule.confirmDeleteTableDesc'),
        onConfirm: () => {
    ...
    onDeleteSnippet: (id: string) => {
      setConfirmConfig({
        show: true,
        title: t('writingModule.confirmDeleteFormula'),
        desc: t('writingModule.confirmDeleteFormulaDesc'),
        onConfirm: () => {
    ...
    onApplyPolished: (text: string) => {
      ...
      showToast({ message: t('writingModule.polishAccepted'), type: 'success' });
    },
    ...
    actions.handleFindToken, actions.getCitationTag, workshop
  ]);
  ...
  return (
    <div className="h-full flex flex-col gap-4 animate-reveal relative px-4 sm:px-6 lg:px-8 overflow-hidden">
      ...
      <div className="flex-1 h-[calc(100vh-120px)] min-h-0 py-1 overflow-hidden">
        {editorMode === 'word' ? (
          /* ─── Word 模式：增强版 TipTap A4 编辑器 ─── */
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 h-full overflow-hidden">
            {isRightSidebarVisible && (
              <div className="lg:col-span-4 lg:order-2 flex flex-col gap-4 overflow-hidden h-auto lg:h-full min-w-0 sm:min-w-[300px]">
                <WritingSidebar {...commonSidebarProps} />
              </div>
            )}
            <div className={`col-span-1 ${isRightSidebarVisible ? 'lg:col-span-8' : 'lg:col-span-12'} lg:order-1 flex flex-col overflow-hidden h-full rounded-[1.25rem] shadow-xl border border-slate-200`} style={panelStyle}>
              <WysiwygEditor
                ref={wysiwygRef}
                ...
                onSaveDocxAs={handleSaveDocxAs}
                onCloseDocx={handleCloseDocx}
          ...
          /* ─── OnlyOffice 本地模式：WebAssembly 文档编辑器 ─── */
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 h-full overflow-hidden">
            {isRightSidebarVisible && (
              <div className="lg:col-span-4 lg:order-2 flex flex-col gap-4 overflow-hidden h-auto lg:h-full min-w-0 sm:min-w-[300px]">
                <WritingSidebar {...commonSidebarProps} />
          ...
          <div className="grid grid-cols-12 gap-4 h-full overflow-hidden">
            ...
            <div className="col-span-4 flex flex-col overflow-hidden h-full shadow-lg rounded-2xl border border-slate-200/60 bg-white relative" style={panelStyle}>
              <MemoizedEditor {...commonEditorProps} isRightSidebarVisible={false} onToggleRightSidebar={noopToggle} />
            </div>
            <div className="col-span-5 flex flex-col overflow-hidden h-full shadow-lg rounded-2xl border border-slate-200/60 bg-slate-50 relative" style={panelStyle}>
              <MemoizedPublishing {...commonPublishingProps} isRightSidebarVisible={false} onToggleRightSidebar={noopToggle} />
            </div>
          </div>
        ) : viewMode === 'dual' ? (
          <div className="grid grid-cols-12 gap-4 h-full overflow-hidden">
            <div className="col-span-6 flex flex-col overflow-hidden h-full shadow-lg rounded-2xl border border-slate-200/60 bg-white relative" style={panelStyle}>
              <MemoizedEditor {...commonEditorProps} isRightSidebarVisible={false} onToggleRightSidebar={noopToggle} />
            </div>
            <div className="col-span-6 flex flex-col overflow-hidden h-full shadow-lg rounded-2xl border border-slate-200/60 bg-slate-50 relative" style={panelStyle}>
              <MemoizedPublishing {...commonPublishingProps} isRightSidebarVisible={false} onToggleRightSidebar={noopToggle} />
            </div>
          </div>
        ) : (
      ...
      />
      <ScientificFigureStudio projectMedia={projectMedia} onUpdateAsset={handleUpdateMediaAsset} onClose={() => setShowFigureStudio(false)} isActive={showFigureStudio} initialRefId={figureStudioTarget} onInsert={handleUniversalInsertText} language={language} project={selectedProject} />
