...
const VisionAnalysisPanel: React.FC<VisionAnalysisPanelProps> = ({ projects, onUpdateProject, selectedProjectId, traceRecordId }) => {
    ...
    const handleSaveToArchive = (targetLogId?: string, targetLogTitle?: string): string | undefined => {
        ...
        const visionSessionSnapshot: any = {
            imageSrc,
            mode,
            semMode,
            temMode,
        ...
        const newArchive: SavedVisionAnalysis = {
            ...
            sessionData: visionSessionSnapshot,
            // 保留少量关键字段便于列表显示和向后兼容
            mode,
    ...
    const handleLoadFromArchive = (archive: SavedVisionAnalysis) => {
        ...
        if (sd) {
            // 新格式：从 sessionData 恢复
            setMode(sd.mode);
            setImageSrc(sd.imageSrc);
            setParticles(sd.particles || []);
            setXrdPeaks(sd.xrdPeaks || []);
            setLatticeResult(sd.latticeResult || null);
            setDefectStats(sd.defectStats || null);
            setReport(sd.report || null);
            setAiReport(sd.aiReport || null);
            setScaleRatio(sd.scaleRatio || null);
            setSelectedXrdPeak(sd.selectedXrdPeak || null);
            setShowStandardLine(sd.showStandardLine || false);
            setRawXrdData(sd.rawXrdData || []);
            setLinkedLogId(sd.linkedLogId);
            setLinkedLogTitle(sd.linkedLogTitle);
            setLinkedProjectId(sd.linkedProjectId);
            setLinkedMilestoneId(sd.linkedMilestoneId);
            // 额外恢复 sessionData 中的新字段
            if (sd.semMode) setSemMode(sd.semMode);
            if (sd.temMode) setTemMode(sd.temMode);
            if (sd.realLengthInput) setRealLengthInput(sd.realLengthInput);
            if (sd.xrdConfig) setXrdConfig(sd.xrdConfig);
            if (sd.sheetStats) setSheetStats(sd.sheetStats);
        } else {
            // 向后兼容旧格式
            setMode(archive.mode || 'SEM');
            setImageSrc(archive.imageSrc || null);
