...
interface AxisSettingsPanelProps {
  ...
  setRightYAxisLabel?: (v: string) => void;
  rightYAxisDivision?: number;
  setRightYAxisDivision?: (v: number) => void;
  rightYAxisColor?: string;
  setRightYAxisColor?: (v: string) => void;
...
const AxisSettingsPanel: React.FC<AxisSettingsPanelProps> = ({
  ...
  autoFitDomains,
  hasRightAxis = false,
  rightYDomain = ['auto', 'auto'], setRightYDomain,
  rightYScale = 'auto', setRightYScale,
  rightYTickCount = 5, setRightYTickCount,
  rightYAxisLabel = '', setRightYAxisLabel,
  rightYAxisDivision = 2, setRightYAxisDivision,
  rightYAxisColor = '#334155', setRightYAxisColor,
  documentColors
}) => {
  ...
  // 右轴本地输入状态
  const [localRYMin, setLocalRYMin] = useState('');
  const [localRYMax, setLocalRYMax] = useState('');
  ...
  useEffect(() => {
    setLocalRYMin(rightYDomain[0] === 'auto' ? 'auto' : rightYDomain[0].toString());
    setLocalRYMax(rightYDomain[1] === 'auto' ? 'auto' : rightYDomain[1].toString());
  }, [rightYDomain]);
  ...
  const handleApplyRightYChanges = () => {
    const parseValue = (val: string) => {
        const lower = val.toLowerCase().trim();
        if (lower === 'auto' || lower === '') return 'auto';
        const num = parseFloat(lower);
    ...
    setRightYDomain?.([parseValue(localRYMin), parseValue(localRYMax)]);
  };
  ...
  return (
    <div className="bg-white p-5 rounded-xl border border-slate-200 space-y-6 shadow-md animate-reveal">
      <header className="mb-4 flex items-center gap-2 border-b border-slate-100 pb-3">
          <button onClick={onClose} className="w-8 h-8 rounded-lg bg-indigo-600 text-white hover:bg-black transition-all flex items-center justify-center shadow-sm active:scale-95 shrink-0">
              <i className="fa-solid fa-arrow-left text-xs"></i>
          </button>
          <div className="min-w-0 flex-1">
              <h3 className="text-xs font-black text-slate-800 uppercase italic tracking-tight leading-none">排版与坐标体系</h3>
              <p className="text-[7px] font-bold text-slate-400 uppercase tracking-widest">AXIS & TITLE TYPOGRAPHY</p>
          </div>
          <button 
              onClick={onSwitchToSeries}
              className="px-3 py-1.5 rounded-lg bg-amber-50 text-amber-600 border border-amber-200 text-[9px] font-black uppercase hover:bg-amber-600 hover:text-white transition-all shadow-sm flex items-center gap-1.5 active:scale-95 whitespace-nowrap"
          >
          ...
          <button 
              ...
              className="px-3 py-1.5 rounded-lg bg-slate-50 text-slate-600 border border-slate-200 text-[9px] font-black uppercase hover:bg-indigo-600 hover:text-white transition-all flex items-center gap-1.5 shadow-sm active:scale-95 whitespace-nowrap"
          >
      ...
      <div className="space-y-4">
          <section className="bg-indigo-50/50 p-3 rounded-lg border border-indigo-100 shadow-inner">
          ...
          <section className="bg-violet-50/40 p-3 rounded-lg border border-violet-100 shadow-inner">
          ...
          {/* ── 右 Y 轴量程（仅双轴激活时显示） ── */}
          {hasRightAxis && (
            <section className="bg-rose-100/60 p-3 rounded-xl border border-rose-200 shadow-sm transition-all hover:bg-rose-100 animate-reveal">
              <div className="flex justify-between items-center mb-2">
                  <h4 className="text-[11px] font-black text-rose-600 uppercase tracking-widest flex items-center gap-1.5">
                    <i className="fa-solid fa-arrows-left-right text-[9px]" /> 右Y轴量程
                  </h4>
                  <button 
                      onClick={() => setRightYScale?.(rightYScale === 'log' ? 'auto' : 'log')}
                      className={`px-2 py-0.5 rounded text-[8px] font-black uppercase transition-all border ${rightYScale === 'log' ? 'bg-rose-600 text-white border-rose-400 shadow-md' : 'bg-white text-rose-400 border-rose-200 hover:bg-rose-50'}`}
                  >
                      {rightYScale === 'log' ? 'LOG ON' : 'LOG OFF'}
                  </button>
              </div>
              <div className="space-y-1.5">
                <div className="flex items-center gap-2 bg-white/60 p-1 rounded-lg">
                  <span className="text-[9px] font-black text-rose-600 w-8">MIN</span>
                  <input className="flex-1 bg-transparent border-none text-[11px] font-black outline-none text-slate-700 min-w-0" value={localRYMin} onChange={(e) => setLocalRYMin(e.target.value)} onBlur={handleApplyRightYChanges} placeholder="auto" />
                </div>
                <div className="flex items-center gap-2 bg-white/60 p-1 rounded-lg">
                  <span className="text-[9px] font-black text-rose-600 w-8">MAX</span>
                  <input className="flex-1 bg-transparent border-none text-[11px] font-black outline-none text-slate-700 min-w-0" value={localRYMax} onChange={(e) => setLocalRYMax(e.target.value)} onBlur={handleApplyRightYChanges} placeholder="auto" />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-2 mt-2">
                <div className="flex flex-col gap-1">
                  <label className="text-[8px] font-black text-rose-500 uppercase px-1">刻度数</label>
                  <input 
                    type="number" 
                    className="w-full bg-white border border-slate-200 rounded-lg p-1.5 text-[11px] font-black text-center outline-none focus:border-rose-400 transition-all shadow-sm" 
                    value={rightYTickCount === 0 ? '' : rightYTickCount} 
                    onChange={(e) => setRightYTickCount?.(e.target.value === '' ? 0 : (parseInt(e.target.value) || 0))} 
                  />
                </div>
                <div className="flex flex-col gap-1">
                  <label className="text-[8px] font-black text-rose-500 uppercase px-1">轴标签</label>
                  <input 
                    className="w-full bg-white border border-slate-200 rounded-lg p-1.5 text-[11px] font-bold outline-none focus:border-rose-400 transition-all shadow-sm" 
                    value={rightYAxisLabel} 
                    onChange={(e) => setRightYAxisLabel?.(e.target.value)}
                    placeholder="右Y轴"
                  />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-2 mt-2">
                <div className="flex flex-col gap-1">
                  <label className="text-[8px] font-black text-rose-500 uppercase px-1">分格数</label>
                  <input 
                    type="number" 
                    className="w-full bg-white border border-slate-200 rounded-lg p-1.5 text-[11px] font-black text-center outline-none focus:border-rose-400 transition-all shadow-sm" 
                    value={rightYAxisDivision} 
                    onChange={(e) => setRightYAxisDivision?.(Math.max(1, parseInt(e.target.value) || 1))} 
                  />
                </div>
                <div className="flex flex-col gap-1">
                  <label className="text-[8px] font-black text-rose-500 uppercase px-1">轴颜色</label>
                  <div className="flex items-center gap-1.5">
                    <input 
                      type="color" 
                      className="w-7 h-7 rounded-lg border border-slate-200 cursor-pointer p-0.5" 
                      value={rightYAxisColor} 
                      onChange={(e) => setRightYAxisColor?.(e.target.value)} 
                    />
                    <input 
                      className="flex-1 bg-white border border-slate-200 rounded-lg p-1.5 text-[10px] font-mono font-bold outline-none focus:border-rose-400 min-w-0" 
                      value={rightYAxisColor} 
                      onChange={(e) => setRightYAxisColor?.(e.target.value)} 
                    />
                  </div>
      ...
      <footer className="mt-4 flex gap-3 shrink-0">
          <button 
              ...
              className="flex-1 py-3 bg-slate-100 text-slate-500 rounded-lg text-[10px] font-black uppercase transition-all hover:bg-slate-200 shadow-sm"
          >
              恢复默认
          </button>
          <button 
              onClick={onClose} 
              className="flex-[1.5] py-3 bg-indigo-600 text-white rounded-lg text-[10px] font-black uppercase shadow-md hover:bg-indigo-700 transition-all active:scale-95"
          >
              完成退出
          </button>
      </footer>
