...
const ChartLibraryModal: React.FC<ChartLibraryModalProps> = ({
    ...
    return (
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-md z-[5000] flex items-center justify-center p-4 lg:p-8">
            <div className="bg-white w-full max-w-5xl h-[85vh] rounded-[3rem] flex flex-col overflow-hidden shadow-2xl animate-reveal border-4 border-white/20">
                ...
                <header className="px-10 py-8 bg-slate-50 border-b border-slate-100 flex justify-between items-center shrink-0">
                    <div className="flex items-center gap-5">
                        ...
                        <div>
                            <h2 className="text-2xl font-black text-slate-800 uppercase italic tracking-tight">项目图表存档库</h2>
                            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">Project Chart Library & States</p>
                        </div>
                    </div>
                    <button onClick={onClose} className="w-12 h-12 bg-white border border-slate-200 rounded-xl flex items-center justify-center text-slate-400 hover:text-white hover:bg-rose-500 transition-all active:scale-95 shadow-sm">
                ...
                <div className="flex-1 flex flex-col md:flex-row overflow-hidden">
                    ...
                    <div className="w-full md:w-80 bg-slate-50/50 border-r border-slate-100 p-8 flex flex-col shrink-0">
                        ...
                        <div className="flex flex-col gap-4">
                            <div className="flex flex-col gap-2">
                                <label className="text-[10px] font-bold text-slate-500 uppercase">图表存档名称</label>
                                <input
                                    type="text"
                                    placeholder="如: OER 初步表征结果..."
                                    className="w-full px-4 py-3 bg-white border border-slate-200 rounded-xl text-sm font-bold text-slate-700 outline-none focus:border-indigo-400 transition-colors shadow-sm"
                                    value={saveName}
                            ...
                            <div className="flex flex-col gap-2 relative">
                                ...
                                <select
                                    className="w-full px-4 py-3 bg-white border border-slate-200 rounded-xl text-sm font-bold text-slate-700 outline-none focus:border-indigo-400 transition-colors shadow-sm appearance-none cursor-pointer"
                            ...
                            <button
                                ...
                                className="w-full py-3.5 bg-indigo-600 text-white rounded-xl text-xs font-black uppercase tracking-wider shadow-lg hover:bg-indigo-500 transition-all active:scale-95 disabled:opacity-50 flex items-center justify-center gap-2"
                            >
                            ...
                            <div className="mt-8 p-4 bg-indigo-50/50 rounded-2xl border border-indigo-100">
                                <p className="text-[10px] text-indigo-400/80 font-bold leading-relaxed">
                                    ...
                                    保存存档不仅会保存当前的图表样式配置，还会一并保存当前加载的所有数据系列和状态。您随时可以从右侧存档列表中一键恢复。
                                </p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="w-full md:w-64 bg-white border-r border-slate-100 p-6 flex flex-col shrink-0 overflow-y-auto">
                        ...
                        {isCreatingFolder && (
                            <div className="mb-4 bg-indigo-50 p-3 rounded-xl border border-indigo-100 animate-reveal">
                                <input
                                    autoFocus
                                    placeholder="输入文件夹名"
                        ...
                        <div className="flex flex-col gap-1">
                            <button
                                ...
                                className={`flex items-center justify-between px-3 py-2.5 rounded-xl text-xs font-bold transition-all ${activeFolderId === 'all' ? 'bg-indigo-50 text-indigo-700' : 'text-slate-600 hover:bg-slate-50'}`}
                            >
                            ...
                            <button
                                ...
                                className={`flex items-center justify-between px-3 py-2.5 rounded-xl text-xs font-bold transition-all ${activeFolderId === 'uncategorized' ? 'bg-indigo-50 text-indigo-700' : 'text-slate-600 hover:bg-slate-50'}`}
                            >
                                ...
                                return (
                                    <div key={fId} className="group relative">
                                        <button
                                            ...
                                            className={`w-full flex items-center justify-between px-3 py-2.5 rounded-xl text-xs font-bold transition-all ${activeFolderId === fId ? 'bg-indigo-50 text-indigo-700' : 'text-slate-600 hover:bg-slate-50'}`}
                                        >
                        ...
                        {displayedCharts.length === 0 ? (
                            ...
                            <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
                                {displayedCharts.map(chart => (
                                    <div key={chart?.id || Math.random().toString()} className="group relative bg-white border border-slate-200 rounded-[1.5rem] p-3 flex flex-col hover:border-indigo-300 hover:shadow-xl transition-all cursor-default">
                                        {/* Thumbnail */}
                                        <div className="w-full aspect-[16/9] bg-slate-50 rounded-xl mb-3 overflow-hidden border border-slate-100 flex items-center justify-center relative">
                                            {chart?.thumbnailUrl ? (
                                            ...
                                            <div className="absolute inset-0 bg-slate-900/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center backdrop-blur-sm gap-4">
                                                <button
                                                    ...
                                                    className="px-6 py-3 bg-indigo-500 text-white rounded-xl text-[11px] font-black uppercase tracking-wider hover:bg-indigo-400 transition-all shadow-xl hover:scale-105 active:scale-95"
                                                >
