...
const SeriesSettingsPanel: React.FC<SeriesSettingsPanelProps> = ({
  ...
  return (
    <div className="bg-white p-5 rounded-xl border border-slate-200 space-y-6 shadow-md animate-reveal">
      <header className="mb-2 flex flex-col gap-3 border-b border-slate-100 pb-4">
        <div className="flex items-center gap-2">
          ...
          <div className="min-w-0 flex-1">
            <h3 className="text-xs font-black text-slate-800 uppercase italic tracking-tight truncate">系列样式编辑</h3>
            <p className="text-[7px] font-bold text-slate-400 uppercase tracking-widest">Series Styling & Dynamic Presets</p>
          </div>
          <button
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-[9px] font-black uppercase transition-all flex items-center gap-1.5 border shadow-sm active:scale-95 whitespace-nowrap ${applyToAll ? 'bg-indigo-600 text-white border-indigo-500 shadow-md' : 'bg-slate-50 text-slate-400 border-slate-100 hover:border-amber-300'}`}
          >
            <i className={`fa-solid ${applyToAll ? 'fa-link' : 'fa-link-slash'} text-[10px]`}></i>
            批量同步 {applyToAll ? '开' : '关'}
          </button>
        </div>
      ...
      <div className="space-y-8 pr-1 max-h-[60vh] overflow-y-auto custom-scrollbar">
        ...
        <section className="bg-slate-50/50 p-4 rounded-lg border border-slate-100 relative">
      ...
      <footer className="mt-8 pt-4 shrink-0">
        <button onClick={onClose} className="w-full py-3.5 bg-indigo-600 text-white rounded-lg text-[10px] font-black uppercase shadow-md hover:bg-black transition-all active:scale-95">完成修改并返回</button>
      </footer>
    </div>
  );
};
