...
const ComparisonMatrixModal: React.FC<ComparisonMatrixModalProps> = ({
    ...
    const [matrixName, setMatrixName] = useState('我的性能对比矩阵');
    const [isEditingName, setIsEditingName] = useState(false);
    const [showLibrary, setShowLibrary] = useState(false);
    const [deleteConfirmId, setDeleteConfirmId] = useState<string | null>(null);
    const [isEditMode, setIsEditMode] = useState(false);
    const [isInsightFolded, setIsInsightFolded] = useState(true); // Default to folded
    ...
    const handleNewMatrix = () => {
        if (savedSimulations.length > 0) {
            if (!window.confirm("确定清空当前矩阵以新建表格吗？建议先点击右上角保存。")) return;
        }
        onClearSimulations?.();
        setMatrixName('未命名对比矩阵');
    };
    ...
    const handleExportAiInsight = () => {
        ...
        showToast({ message: "AI 分析报告已导出为 TXT", type: 'success' });
    };

    const handleCopyInsight = () => {
        if (!aiAnalysisResult) return;
        navigator.clipboard.writeText(aiAnalysisResult);
        showToast({ message: "分析报告已复制到剪贴板", type: 'success' });
    };
    ...
    return (
        <div className="absolute inset-0 bg-slate-950/60 z-[100] backdrop-blur-sm flex items-center justify-center p-4 lg:p-8 animate-reveal">
            <div className="bg-white w-full h-full max-h-[850px] rounded-[2.5rem] shadow-2xl border-4 border-white/50 flex flex-col overflow-hidden relative">
                ...
                {/* Header Section */}
                <div className="flex justify-between items-center px-10 py-8 shrink-0 bg-white z-20">
                    <div className="flex items-center gap-6">
                        ...
                        {/* Editable Matrix Name & New Matrix Button */}
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <button
                            ...
                            className="h-10 px-5 bg-gradient-to-r from-amber-400 to-orange-500 text-white rounded-xl text-[10px] font-black uppercase shadow-lg shadow-amber-100 hover:scale-[1.02] transition-all flex items-center gap-2 active:scale-95 disabled:opacity-50"
                        >
                        ...
                        {/* Action Buttons */}
                        <button onClick={() => setIsEditMode(!isEditMode)} className={`h-10 px-4 rounded-xl text-[10px] font-black uppercase transition-all flex items-center gap-2 border ${isEditMode ? 'bg-indigo-50 border-indigo-200 text-indigo-600' : 'bg-white border-slate-200 text-slate-500 hover:border-indigo-200 hover:text-slate-600'}`}>
                            <i className="fa-solid fa-pen-to-square"></i> 编辑矩阵
                        </button>
                        <button onClick={handleExportTable} className="h-10 px-4 bg-white border border-slate-200 text-slate-600 rounded-xl text-[10px] font-black uppercase hover:bg-slate-50 transition-all flex items-center gap-2">
                        ...
                        <div className="relative">
                            <button
                                ...
                                className={`h-10 px-5 rounded-xl text-[10px] font-black uppercase transition-all flex items-center gap-2 shadow-sm ${showLibrary ? 'bg-indigo-600 text-white shadow-indigo-200' : 'bg-indigo-50 text-indigo-600 hover:bg-indigo-100'}`}
                            >
                                <i className="fa-solid fa-layer-group"></i>
                                矩阵库 ({matrixLibrary.length})
                            ...
                            {showLibrary && (
                                <div className="absolute top-full right-0 mt-3 w-80 bg-white rounded-2xl shadow-2xl border border-slate-100 overflow-hidden z-50 animate-reveal flex flex-col max-h-[300px]">
                                    <div className="p-3 bg-slate-50 border-b border-slate-100 flex justify-between items-center">
                                        <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest px-1">已存档矩阵</p>
                                    ...
                                    <div className="flex-1 overflow-y-auto custom-scrollbar p-1.5 space-y-1">
                                        ...
                                        {matrixLibrary.length === 0 && (
                                            <div className="py-8 text-center flex flex-col items-center opacity-40 gap-2">
                                                ...
                                                <p className="text-[9px] text-slate-400 font-medium">暂无存档记录</p>
                                            </div>
                                        )}
                                    </div>
                                </div>
                        ...
                        <button
                            ...
                            className="h-10 px-6 bg-emerald-500 text-white rounded-xl text-[10px] font-black uppercase hover:bg-emerald-600 transition-all flex items-center gap-2 shadow-lg shadow-emerald-100 active:scale-95"
                        >
                            <i className="fa-solid fa-floppy-disk"></i> 保存矩阵
                        </button>
                ...
                {/* AI Analysis View Integration - Default Folded */}
                {aiAnalysisResult && (
                        ...
                        <div className="bg-indigo-50/50 rounded-[1.5rem] border-2 border-dashed border-indigo-200 relative animate-reveal overflow-hidden">
                            ...
                            >
                                <h4 className="text-[10px] font-black text-indigo-600 uppercase tracking-widest flex items-center gap-2">
                                    <i className="fa-solid fa-robot"></i> AI 横向对比分析洞察
                                ...
                                {!isInsightFolded && (
                                    <div className="flex items-center gap-2" onClick={e => e.stopPropagation()}>
                                        ...
                                        <button onClick={() => { }} className="w-7 h-7 opacity-0 pointer-events-none"></button> {/* spacer */}
                                    </div>
                                )}
                                <button
                ...
                <div className="flex-1 overflow-auto custom-scrollbar px-10 pb-10">
                    <table className="w-full text-left border-collapse">
                        <thead className="sticky top-0 bg-white z-10 shadow-sm">
                            <tr>
                                <th className="py-5 pr-4 text-[11px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-100 w-[18%] pl-4">方案标识 (MATERIAL)</th>
                                <th className="py-5 px-2 text-[11px] font-black text-indigo-400 uppercase tracking-widest border-b border-slate-100 text-center w-[8%]">模式</th>
                                <th className="py-5 px-2 text-[11px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-100 text-center w-[8%]">工况 (PH)</th>
                                ...
                                <th className="py-5 pl-4 text-[11px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-100 text-right w-[10%] pr-4">操作</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-slate-50">
                            {sortedSimulations.map((sim) => (
                                <tr key={sim.id} className="group hover:bg-slate-50/80 transition-colors">
                                        ...
                                        {isEditMode ? (
                                            ...
                                            <span className="text-lg font-black text-slate-800 italic font-mono tracking-tight">
                                                {(() => {
                                                    ...
                                                    // 鲁棒后备：如果审计失败（如缺少能级台阶），尝试自动纠正 AI 原始值的单位错误
                                                    let val = sim.physicalConstants?.eta10 || 0;
                                                    if (val > 10) val = val / 1000; // 自动将 mV 转为 V
                                    ...
                                    {/* Stability Index Column - Bar Removed per Request */}
                                    <td className="py-5 px-2 text-center">
                                        {isEditMode ? (
                                            <input
                                ...
                                <tr>
                                    <td colSpan={9} className="py-32 text-center">
                                        <div className="inline-flex flex-col items-center opacity-40 gap-4">
                                            ...
                                            <div>
                                                <p className="text-xs font-black uppercase tracking-widest text-slate-500">当前矩阵为空</p>
                                                <p className="text-[10px] text-slate-400 mt-1">请从上方加载矩阵库，或在模拟器中添加新方案</p>
                                            </div>
                                        </div>
                                    </td>
                                </tr>
