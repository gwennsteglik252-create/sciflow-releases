                ...
                <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-[999] flex items-center justify-center p-6 animate-fade-in" onClick={() => setShowLibrary(false)}>
                    <div className="bg-white w-[600px] max-h-[80vh] rounded-[3rem] shadow-2xl flex flex-col overflow-hidden animate-scale-up" onClick={e => e.stopPropagation()}>
                        <div className="px-10 py-8 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
                            <div>
                                <h3 className="text-2xl font-black text-slate-800 uppercase italic leading-none">桑基方案库 <span className="text-indigo-600">LIBRARY</span></h3>
                                <p className="text-[10px] font-bold text-slate-400 mt-2 uppercase tracking-widest text-shadow-sm">浏览并加载已保存的学术可视化模型</p>
                            </div>
                            <button onClick={() => setShowLibrary(false)} className="w-12 h-12 rounded-2xl bg-white border border-slate-200 flex items-center justify-center text-slate-400 hover:text-indigo-600 hover:shadow-lg transition-all active:scale-95">
