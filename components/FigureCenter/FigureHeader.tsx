...
interface FigureHeaderProps {
  activeTab: 'generative' | 'structural' | 'assembly' | 'summary' | 'timeline' | 'tree' | 'audit' | 'sankey' | 'mindmap';
  setActiveTab: (tab: 'generative' | 'structural' | 'assembly' | 'summary' | 'timeline' | 'tree' | 'audit' | 'sankey' | 'mindmap') => void;
  ...
  mindmapLogic?: ReturnType<typeof useMindMapDesigner>;
...
export const FigureHeader: React.FC<FigureHeaderProps> = ({
  ...
  treeLogic, sankeyLogic, mindmapLogic, onTemplateClick
}) => {
  ...
  const currentSavedId = (() => {
    switch (activeTab) {
      ...
      case 'mindmap': return mindmapLogic?.currentSavedId ?? null;
      default: return null;
    }
  })();
  ...
  const handleLibraryClick = () => {
    switch (activeTab) {
      ...
      case 'sankey':
        ...
        break;
      case 'mindmap':
        mindmapLogic?.setShowLibrary(true);
        break;
      default:
    ...
    <header className="relative isolate flex justify-between items-center bg-slate-900/90 backdrop-blur-xl px-5 py-3 rounded-2xl border border-white/10 shrink-0 shadow-2xl z-[100]">
      ...
      <div className="flex bg-black/40 p-1 rounded-xl border border-white/5 shadow-inner">
        ...
        <button
          ...
          className={`px-4 py-2 rounded-lg text-[10px] font-black uppercase transition-all flex items-center gap-2 ${activeTab === 'mindmap' ? 'bg-violet-600 text-white shadow-lg' : 'text-slate-400 hover:text-white'}`}
        >
          <i className="fa-solid fa-brain"></i> 框架思维图
        </button>
        <button
      ...
      <div className="flex items-center gap-3">
        ...
        {onTemplateClick && ['structural', 'timeline', 'summary', 'tree', 'mindmap'].includes(activeTab) && (
          <button
