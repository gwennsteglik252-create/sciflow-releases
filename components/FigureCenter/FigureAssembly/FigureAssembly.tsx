...
import { AlignToolbar, AlignType } from './AlignToolbar';
import { SmartGuides, GuideLine, computeSmartGuides } from './SmartGuides';
...
export const FigureAssembly: React.FC<{ generativeLogic: ReturnType<typeof useGenerativeDesigner>, logic: ReturnType<typeof useFigureAssemblyLogic> }> = ({ generativeLogic, logic }) => {
    ...
    const { figures: litFigures, removeFigure: removeLitFigure } = useLiteratureFigureBridge();
    ...
    const {
        ...
        // Canvas & Gap config (功能5/6)
        canvasConfig, setCanvasConfig, canvasWidth, canvasHeight,
        gapConfig, setGapConfig,
    } = logic;

    const gridSize = 20;

    // ─── Resize drag state (功能1) ───────────────────────────────────────────
    const [resizeDragState, setResizeDragState] = useState<{
        panelId: string; mode: string; startX: number; startY: number;
        initialX: number; initialY: number; initialW: number; initialH: number;
    } | null>(null);

    const handleResizeStart = (e: React.MouseEvent, panelId: string, mode: string) => {
        e.stopPropagation();
        const panel = panels.find(p => p.id === panelId);
        if (!panel) return;
        setActivePanelId(panelId);
        setResizeDragState({
            panelId, mode, startX: e.clientX, startY: e.clientY,
            initialX: panel.x, initialY: panel.y, initialW: panel.w, initialH: panel.h
        });
    };

    // ── Smart Guides state (功能9) ──────────────────────────────────────
    ...
    const handleAlign = (type: AlignType) => {
        ...
        const maxX = Math.max(...next.map(p => p.x + p.w));
        const minY = Math.min(...next.map(p => p.y));
        const maxY = Math.max(...next.map(p => p.y + p.h));

        switch (type) {
            case 'left':     next = next.map(p => ({ ...p, x: minX })); break;
            case 'right':    next = next.map(p => ({ ...p, x: maxX - p.w })); break;
            case 'center-h': { const cx = (minX + maxX) / 2; next = next.map(p => ({ ...p, x: cx - p.w / 2 })); break; }
            case 'top':      next = next.map(p => ({ ...p, y: minY })); break;
            case 'bottom':   next = next.map(p => ({ ...p, y: maxY - p.h })); break;
            case 'center-v': { const cy = (minY + maxY) / 2; next = next.map(p => ({ ...p, y: cy - p.h / 2 })); break; }
            case 'dist-h': {
                const sorted = [...next].sort((a, b) => a.x - b.x);
                const totalW = sorted.reduce((s, p) => s + p.w, 0);
                const totalGap = (maxX - minX) - totalW;
                const gapEach = totalGap / Math.max(sorted.length - 1, 1);
                let cx2 = minX;
                const idToX = new Map<string, number>();
                sorted.forEach(p => { idToX.set(p.id, cx2); cx2 += p.w + gapEach; });
                next = next.map(p => ({ ...p, x: idToX.get(p.id) ?? p.x }));
                break;
            }
            case 'dist-v': {
                const sorted = [...next].sort((a, b) => a.y - b.y);
                const totalH = sorted.reduce((s, p) => s + p.h, 0);
                const totalGap = (maxY - minY) - totalH;
                const gapEach = totalGap / Math.max(sorted.length - 1, 1);
                let cy2 = minY;
                const idToY = new Map<string, number>();
                sorted.forEach(p => { idToY.set(p.id, cy2); cy2 += p.h + gapEach; });
                next = next.map(p => ({ ...p, y: idToY.get(p.id) ?? p.y }));
                break;
            }
            case 'canvas-center': {
                const groupW = maxX - minX;
                const groupH = maxY - minY;
                const offsetX = (canvasWidth - groupW) / 2 - minX;
                const offsetY = (canvasHeight - groupH) / 2 - minY;
                next = next.map(p => ({ ...p, x: p.x + offsetX, y: p.y + offsetY }));
                break;
            }
        }
        recordState(next);
    ...
    useEffect(() => {
        const handleKeyDown = (e: KeyboardEvent) => {
            ...
            // 功能13: Ctrl+G 编组 / Ctrl+Shift+G 解组
            if ((e.metaKey || e.ctrlKey) && e.key === 'g') {
                e.preventDefault();
                if (e.shiftKey) {
                    // 解组：清除当前活动面板所在组的所有 groupId
                    if (activePanelId) {
                        const activeP = panels.find(p => p.id === activePanelId);
                        if (activeP?.groupId) {
                            recordState(panels.map(p => p.groupId === activeP.groupId ? { ...p, groupId: undefined } : p));
                            showToast({ message: '已解除编组', type: 'success' });
                        }
                    }
                } else {
                    // 编组：将同组别名赋给当前活动面板及所有选中面板
                    if (activePanelId && panels.length >= 2) {
                        const groupId = 'grp_' + Date.now();
                        // 获取与活动面板相交的面板作为编组候选
                        const activeP = panels.find(p => p.id === activePanelId);
                        if (activeP) {
                            const nearbyIds = panels.filter(p => {
                                if (p.id === activePanelId) return true;
                                // 距离小于50px的相邻面板自动编组
                                const dist = Math.abs(p.x - activeP.x) + Math.abs(p.y - activeP.y);
                                return dist < (activeP.w + activeP.h);
                            }).map(p => p.id);
                            recordState(panels.map(p => nearbyIds.includes(p.id) ? { ...p, groupId } : p));
                            showToast({ message: `已编组 ${nearbyIds.length} 个面板`, type: 'success' });
                        }
                    }
                }
            }
            ...
            if (['ArrowUp', 'ArrowDown', 'ArrowLeft', 'ArrowRight'].includes(e.key)) {
                ...
                if (selectedText && activePanelId) {
                    recordState(panels.map(p => p.id === activePanelId ? { ...p, texts: p.texts?.map(t => t.id === selectedText.textId ? { ...t, x: t.x + dx, y: t.y + dy } : t) } : p));
                } else if (selectedShape && activePanelId) {
                    recordState(panels.map(p => p.id === activePanelId ? { ...p, shapes: p.shapes?.map(s => s.id === selectedShape.shapeId ? { ...s, x1: s.x1 + dx, y1: s.y1 + dy, x2: s.x2 + dx, y2: s.y2 + dy } : s) } : p));
                } else if (activePanelId) {
                    const idsToMove = new Set(selectedPanelIds);
                    idsToMove.add(activePanelId);
                    recordState(panels.map(p => idsToMove.has(p.id) ? { ...p, x: p.x + dx, y: p.y + dy } : p));
                }
            }
            ...
            if ((e.metaKey || e.ctrlKey) && e.key === 'a') {
                ...
                showToast({ message: `已全选 ${panels.length} 个面板`, type: 'success' });
            }
        };
    ...
    const handleMouseDown = (e: React.MouseEvent, id: string) => {
        ...
        setActivePanelId(id); setSelectedText(null); setSelectedShape(null); setIsDragging(true); setDragOffset({ x: e.clientX, y: e.clientY });
    };

    const handleContainerMouseDown = (e: React.MouseEvent) => {
        if (e.target === e.currentTarget || e.target === containerRef.current || e.target === canvasRef.current) {
            // 功能19: Alt 键按下时启动框选而非平移
            ...
            setActivePanelId(null); setSelectedText(null); setSelectedShape(null); setEditingText(null); setSelectedPanelIds(new Set());
        }
    };
    ...
    const handleMouseMove = (e: React.MouseEvent) => {
        ...
        if (marquee && canvasRef.current) {
            const rect = canvasRef.current.getBoundingClientRect();
            setMarquee(prev => prev ? { ...prev, currentX: (e.clientX - rect.left) / canvasScale, currentY: (e.clientY - rect.top) / canvasScale } : null);
            return;
        }
        // Resize drag (功能1)
        if (resizeDragState) {
            const dx = (e.clientX - resizeDragState.startX) / canvasScale;
            const dy = (e.clientY - resizeDragState.startY) / canvasScale;
            const mode = resizeDragState.mode;
            let { initialX: nx, initialY: ny, initialW: nw, initialH: nh } = resizeDragState;
            const MIN_SIZE = 30;

            // Horizontal
            if (mode.includes('e')) nw = Math.max(MIN_SIZE, nw + dx);
            if (mode.includes('w')) { nw = Math.max(MIN_SIZE, nw - dx); nx = nx + (resizeDragState.initialW - nw); }
            // Vertical
            if (mode.includes('s')) nh = Math.max(MIN_SIZE, nh + dy);
            if (mode.includes('n')) { nh = Math.max(MIN_SIZE, nh - dy); ny = ny + (resizeDragState.initialH - nh); }

            // Grid snapping
            if (showGrid) {
                const modX = nx % gridSize, modY = ny % gridSize;
                if (Math.abs(modX) < 10) nx -= modX;
                if (Math.abs(modY) < 10) ny -= modY;
                const modW = nw % gridSize, modH = nh % gridSize;
                if (Math.abs(modW) < 10) nw -= modW;
                if (Math.abs(modH) < 10) nh -= modH;
            }

            setPanels(prev => prev.map(p =>
                p.id === resizeDragState.panelId ? { ...p, x: nx, y: ny, w: nw, h: nh } : p
            ));
            return;
        }
        if (textDragState) {
        ...
        if (isDragging && activePanelId) {
            ...
            // 功能9: 智能参考线 + 磁吸
            const { guides, snapDx, snapDy } = computeSmartGuides(activePanelId, panels, dx, dy);
            setSmartGuides(guides);

            setPanels(prev => prev.map(p => {
                if (p.id === activePanelId) {
                    let nx = p.x + snapDx, ny = p.y + snapDy;
                    if (showGrid) { const modX = nx % gridSize, modY = ny % gridSize; if (Math.abs(modX) < 10) nx -= modX; if (Math.abs(modY) < 10) ny -= modY; }
                    return { ...p, x: nx, y: ny };
                }
                // 功能13: 同组面板同步移动
                const activePanel = prev.find(pp => pp.id === activePanelId);
                if (activePanel?.groupId && p.groupId === activePanel.groupId && p.id !== activePanelId) {
                    return { ...p, x: p.x + snapDx, y: p.y + snapDy };
                }
                // 功能19: 多选面板同步拖拽
                if (selectedPanelIds.has(p.id) && p.id !== activePanelId) {
                    return { ...p, x: p.x + snapDx, y: p.y + snapDy };
                }
                return p;
            }));
    ...
    const handleMouseUp = (e: React.MouseEvent) => {
        ...
        if (isDragging || textDragState || shapeDragState || resizeDragState) recordState(panels);
        setIsDragging(false); setIsPanning(false); setTextDragState(null); setShapeDragState(null); setResizeDragState(null);
        setSmartGuides([]);  // 功能9: 清除参考线
        // 功能19: 完成框选 → 选出框内面板
        if (marquee) {
            const mx = Math.min(marquee.startX, marquee.currentX), my = Math.min(marquee.startY, marquee.currentY);
            const mw = Math.abs(marquee.currentX - marquee.startX), mh = Math.abs(marquee.currentY - marquee.startY);
            if (mw > 5 && mh > 5) {
                const ids = panels.filter(p => {
                    return p.x < mx + mw && p.x + p.w > mx && p.y < my + mh && p.y + p.h > my;
                }).map(p => p.id);
                setSelectedPanelIds(new Set(ids));
                if (ids.length > 0) {
                    setActivePanelId(ids[0]);
                    showToast({ message: `已框选 ${ids.length} 个面板`, type: 'success' });
                }
            }
            setMarquee(null);
        }
    ...
    const handleDrop = (e: React.DragEvent) => {
        ...
        files.forEach(file => {
            ...
            reader.onload = (ev) => {
                ...
                if (imgUrl) handleAddPanel(imgUrl);
            };
            reader.readAsDataURL(file);
        });
        showToast({ message: `正在导入 ${files.length} 张图片...`, type: 'info' });
    };
    const handleDragOver = (e: React.DragEvent) => { e.preventDefault(); e.stopPropagation(); };

    /**
     * handleExport - 深度优化导出质量
     * 采用 pixelRatio: 5.0 以实现超高清位图渲染
     * 支持 PNG 和 SVG 矢量格式（功能4）
     */
    const handleExport = async (format: 'png' | 'svg' = 'png') => {
        ...
        try {
            ...
            if (format === 'svg') {
                // SVG 矢量导出（功能4）
                const svgDataUrl = await htmlToImage.toSvg(canvasRef.current, {
                ...
                // toSvg 返回 data:image/svg+xml 的 Data URL
                const svgContent = decodeURIComponent(svgDataUrl.split(',')[1]);
                const svgBlob = new Blob([svgContent], { type: 'image/svg+xml;charset=utf-8' });
                saveAs(svgBlob, `Figure_Composite_${Date.now()}.svg`);
                showToast({ message: 'SVG 矢量图导出成功', type: 'success' });
            } else {
                // PNG 位图导出
    ...
    return (
        <div className="flex h-full gap-4 overflow-hidden" onMouseMove={handleMouseMove} onMouseUp={handleMouseUp} onMouseLeave={handleMouseUp}>
            <AssemblySidebar
                ...
                gapConfig={gapConfig} setGapConfig={setGapConfig}
                literatureFigures={litFigures}
                onRemoveLiteratureFigure={removeLitFigure}
            />
            <AssemblyCanvas
                ...
                onAddShape={(e, id, type) => recordState(panels.map(p => p.id === id ? { ...p, shapes: [...(p.shapes || []), { id: Date.now().toString(), type, x1: 20, y1: 20, x2: 80, y2: type === 'callout' ? 50 : 80, color: '#4f46e5', strokeWidth: 2, ...(type === 'callout' ? { calloutText: 'Label' } : {}) } as any] } : p))}
                ...
                alignToolbar={<AlignToolbar panelCount={panels.length} onAlign={handleAlign} />}
                smartGuidesNode={<SmartGuides guides={smartGuides} canvasScale={canvasScale} />}
                onDrop={handleDrop} onDragOver={handleDragOver}
                selectedPanelIds={selectedPanelIds} marquee={marquee}
            />

            {/* ── Right Layer Panel ─────────────────────────────────── */}
            <div
                className={`flex flex-col shrink-0 transition-all duration-300 rounded-lg border border-white/10 bg-slate-900/80 backdrop-blur-xl shadow-2xl overflow-hidden ${showLayerPanel ? 'w-52' : 'w-8'
                    }`}
                style={{ minHeight: 0 }}
            >
                {/* Toggle button */}
