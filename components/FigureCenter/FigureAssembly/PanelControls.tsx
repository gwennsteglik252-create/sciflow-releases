...
interface PanelControlsProps {
  ...
  onAddShape: (type: 'arrow' | 'line' | 'rect' | 'circle' | 'bezier' | 'polygon' | 'freehand' | 'callout', e: React.MouseEvent) => void;
...
export const PanelControls: React.FC<PanelControlsProps> = ({
  ...
  return (
    <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex items-center gap-1 p-2 bg-slate-900/90 backdrop-blur-md rounded-lg shadow-2xl border border-white/10 z-50 animate-reveal scale-100 hover:scale-105 transition-all origin-bottom no-export">
      ...
      <div className="flex items-center gap-1.5 px-2 border-r border-white/20 relative" ref={menuRef}>
        ...
        <div className="relative">
          ...
          {showShapeMenu && (
            <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-3 w-32 bg-slate-900 border border-white/10 rounded-lg shadow-2xl p-1.5 animate-reveal z-[60] flex flex-col gap-1">
              <button
              ...
              <button
                onClick={(e) => { onAddShape('bezier', e); setShowShapeMenu(false); }}
                className="w-full py-2 px-3 hover:bg-white/10 rounded-xl text-[10px] font-black uppercase text-white flex items-center gap-3 transition-colors text-left"
              >
                <i className="fa-solid fa-bezier-curve text-teal-400 w-4 text-center"></i> 贝塞尔曲线
              </button>
              <button
                onClick={(e) => { onAddShape('polygon', e); setShowShapeMenu(false); }}
                className="w-full py-2 px-3 hover:bg-white/10 rounded-xl text-[10px] font-black uppercase text-white flex items-center gap-3 transition-colors text-left"
              >
                <i className="fa-solid fa-draw-polygon text-teal-400 w-4 text-center"></i> 多边形
              </button>
              <button
                onClick={(e) => { onAddShape('freehand', e); setShowShapeMenu(false); }}
                className="w-full py-2 px-3 hover:bg-white/10 rounded-xl text-[10px] font-black uppercase text-white flex items-center gap-3 transition-colors text-left"
              >
                <i className="fa-solid fa-pen-nib text-teal-400 w-4 text-center"></i> 徒手画
              </button>
              <button
                onClick={(e) => { onAddShape('callout', e); setShowShapeMenu(false); }}
                className="w-full py-2 px-3 hover:bg-white/10 rounded-xl text-[10px] font-black uppercase text-white flex items-center gap-3 transition-colors text-left"
              >
                <i className="fa-solid fa-comment-dots text-amber-400 w-4 text-center"></i> 标注连线
              </button>
            </div>
          )}
        </div>
