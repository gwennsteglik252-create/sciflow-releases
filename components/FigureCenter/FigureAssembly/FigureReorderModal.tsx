      ...
      <div className="bg-white w-full max-w-5xl rounded-[3rem] p-10 animate-reveal shadow-2xl relative border-4 border-white flex flex-col max-h-[85vh]">
        <div className="flex justify-between items-center mb-6 shrink-0">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h3 className="text-xl font-black text-slate-800 uppercase italic tracking-tighter">{t('figureCenter.assembly.reorderTitle')}</h3>
        ...
        <div className="p-4 bg-indigo-50/50 rounded-lg border border-indigo-100 mb-4 text-[10px] text-indigo-800 font-bold flex items-center gap-2 shrink-0">
          <i className="fa-solid fa-circle-info text-indigo-500"></i>
          Drag cards to reorder. The system will fill the target grid in sequence (a, b, c...).
        </div>

        <div className="flex-1 overflow-y-auto custom-scrollbar bg-slate-50 border border-slate-200 rounded-xl p-6">
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
            {previewItems.map((item, index) => (
              <div
                ...
                onDrop={handleDrop}
                className="aspect-[4/3] bg-white rounded-lg border-2 border-slate-200 shadow-sm relative overflow-hidden cursor-move hover:border-indigo-400 hover:shadow-lg transition-all group active:cursor-grabbing"
              >
            ...
            {Array.from({ length: emptySlots }).map((_, i) => (
              <div key={`placeholder-${i}`} className="aspect-[4/3] rounded-lg border-2 border-dashed border-slate-200 flex flex-col items-center justify-center bg-slate-100/50 opacity-60">
        ...
        <div className="flex gap-4 mt-8 shrink-0">
          <button onClick={onClose} className="flex-1 py-4 bg-slate-100 text-slate-500 rounded-lg text-[11px] font-black uppercase hover:bg-slate-200 transition-all">{t('figureCenter.assembly.reorderCancel')}</button>
          <button onClick={() => onConfirm(items)} className="flex-[2] py-4 bg-emerald-600 text-white rounded-lg text-[11px] font-black uppercase shadow-xl hover:bg-emerald-700 transition-all active:scale-95 flex items-center justify-center gap-2">
            <i className="fa-solid fa-check"></i> {t('figureCenter.assembly.reorderConfirm')}
          </button>
        </div>
      </div>
    </div>
