       ...
       <div className="bg-white w-full max-w-5xl h-[85vh] rounded-[3rem] p-8 animate-reveal shadow-2xl relative border-4 border-white flex flex-col overflow-hidden">
           <div className="flex justify-between items-center mb-6 shrink-0">
               <div className="flex items-center gap-4">
                  ...
                  <div>
                      <h3 className="text-2xl font-black text-slate-800 uppercase italic tracking-tighter">云端图库 (Cloud Gallery)</h3>
           ...
           <div className="flex-1 overflow-y-auto custom-scrollbar p-2">
               {savedLibrary.length > 0 ? (
                           ...
                           <div key={idx} className="bg-slate-50 p-3 rounded-2xl border border-slate-100 group relative hover:shadow-lg transition-all">
                               <div className="aspect-square bg-white rounded-xl overflow-hidden relative mb-3">
                                   ...
                                   <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                                       <button onClick={() => handleDownload(item.url)} className="w-10 h-10 bg-white rounded-full text-indigo-600 flex items-center justify-center hover:scale-110 transition-transform shadow-lg" title="下载"><i className="fa-solid fa-download"></i></button>
                                       <button onClick={() => { handleSelectForIteration(item); setShowLibraryModal(false); }} className="w-10 h-10 bg-white rounded-full text-amber-500 flex items-center justify-center hover:scale-110 transition-transform shadow-lg" title="迭代"><i className="fa-solid fa-pen-to-square"></i></button>
                                       <button onClick={() => deleteFromLibrary(idx)} className="w-10 h-10 bg-white rounded-full text-rose-500 flex items-center justify-center hover:scale-110 transition-transform shadow-lg" title="删除"><i className="fa-solid fa-trash"></i></button>
                                   </div>
                               </div>
                               <div className="px-1">
                   ...
                   <div className="h-full flex flex-col items-center justify-center opacity-30 gap-4">
                       ...
                       <p className="text-sm font-black uppercase tracking-widest text-slate-400">图库暂无内容</p>
                   </div>
               )}
           </div>
