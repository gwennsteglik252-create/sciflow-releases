...
export const StructureModals: React.FC<StructureModalsProps> = ({
    showLibrary, setShowLibrary, savedDiagrams, onLoad, onDelete, onRename,
            ...
            {showLibrary && (
                <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-md z-[3000] flex items-center justify-center p-4">
                    <div className="bg-white w-full max-w-lg rounded-[2.5rem] p-8 animate-reveal shadow-2xl relative border-4 border-white flex flex-col max-h-[80vh]">
                        <h3 className="text-xl font-black text-slate-800 mb-6 uppercase italic border-l-4 border-indigo-600 pl-4">结构图库</h3>
                        <div className="flex-1 overflow-y-auto custom-scrollbar pr-2 space-y-3">
                            {savedDiagrams.map(diagram => (
                                <div key={diagram.id} className="p-4 bg-slate-50 rounded-2xl border border-slate-100 flex justify-between items-center group hover:border-indigo-300 transition-all cursor-pointer" onClick={() => {
                                    if (renamingId !== diagram.id) onLoad(diagram);
                                }}>
                                    <div className="flex-1 mr-4">
                                        {renamingId === diagram.id ? (
                                            <input
                                                ...
                                                onBlur={() => {
                                                    if (tempTitle.trim() && tempTitle !== diagram.title) {
                                                        onRename(diagram.id, tempTitle);
                                                    }
                                                    setRenamingId(null);
                                                }}
                                                onKeyDown={e => {
                                            ...
                                            <>
                                                <div className="flex items-center gap-2">
                                                    <p className="text-[11px] font-black text-slate-800 uppercase">{diagram.title}</p>
                                                    <button
                                                        onClick={(e) => {
                                                            e.stopPropagation();
                                                            setRenamingId(diagram.id);
                                                            setTempTitle(diagram.title);
                                                        }}
                                                ...
                                                <p className="text-[8px] text-slate-400 font-bold uppercase mt-1">{diagram.timestamp}</p>
                                            </>
                                        )}
                                    </div>
                                    <button onClick={(e) => onDelete(diagram.id, e)} className="w-8 h-8 rounded-lg bg-white text-rose-300 hover:text-rose-500 hover:shadow-sm"><i className="fa-solid fa-trash-can text-[10px]"></i></button>
                                </div>
                            ))}
                            {savedDiagrams.length === 0 && <p className="text-center py-10 text-[10px] text-slate-400 italic">暂无保存的结构图</p>}
                        </div>
