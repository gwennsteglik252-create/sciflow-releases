...
const MemberCard: React.FC<MemberCardProps> = ({
    ...
    const toggleLeaveDay = (e: React.MouseEvent, dayIdx: number) => {
        ...
        if (conflictingTasks.length > 0 && !leaveDays.includes(dayIdx)) {
            showToast({
                message: `${member.name} 在周${DAYS_SHORT[dayIdx]}有 ${conflictingTasks.length} 个任务。是否将其一键退回待办池？`,
                type: 'info',
                actionLabel: '一键释放',
    ...
    const handleEvacuateTasksForDay = (dayIdx: number) => {
        ...
        showToast({ message: `周${DAYS_SHORT[dayIdx]}的任务已释放`, type: 'success' });
    };
    ...
    const confirmTaskAssignment = (e: React.MouseEvent) => {
        ...
        showToast({ message: `${member.name} 的任务矩阵已更新`, type: 'success' });
        setShowAssignMenu(false);
    };
    ...
    return (
        <div className={`bg-white rounded-[2.5rem] p-6 border-2 transition-all duration-500 group relative flex flex-col ${isOverloaded ? 'border-rose-200 ring-4 ring-rose-50 shadow-rose-100' :
                        ...
                        {showCalendar && (
                            <div className="absolute top-full left-0 mt-2 w-52 bg-white/95 backdrop-blur-xl border border-slate-200 rounded-2xl shadow-2xl p-3 z-[100] animate-reveal">
                                <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-2 px-1 flex justify-between">
                                    <span>本周请假计划</span>
                                    <span className="text-indigo-600 italic">Weekly</span>
                                </p>
                    ...
                    {onDelete && (
                        <AnimatePresence mode="wait">
                            {deleteStage === 0 ? (
                                ...
                                <motion.button key="stage-1" initial={{ width: 0, opacity: 0 }} animate={{ width: 'auto', opacity: 1 }} exit={{ width: 0, opacity: 0 }} onClick={handleDeleteClick} className="px-3 h-7 bg-amber-500 text-white rounded-lg text-[8px] font-black uppercase shadow-lg border border-amber-400 whitespace-nowrap flex items-center gap-1.5"><i className="fa-solid fa-triangle-exclamation"></i> 确认移除?</motion.button>
                            ) : (
            ...
            <div className="flex justify-center mb-4">
                ...
                >
                    {isDetailsExpanded ? '收起能力画像' : '展开能力画像'}
                ...
                <div className="animate-reveal space-y-5 mb-5">
                    <div className="h-40 w-full bg-slate-50/50 rounded-[2rem] border border-slate-100 shadow-inner relative overflow-hidden flex items-center justify-center shrink-0">
                        {isEditingDNA ? (
                            <div className="w-full h-full p-4 flex flex-col justify-center gap-2 animate-reveal bg-white/80 backdrop-blur-sm z-20">
                                ...
                                <button onClick={handleSaveDNA} className="mt-1 w-full py-1.5 bg-indigo-600 text-white rounded-lg text-[7px] font-black uppercase shadow-lg active:scale-95">保存 DNA 修正</button>
                            </div>
                        ) : (
                            <>
                    ...
                    <div className="p-4 rounded-[2rem] bg-white border border-slate-100 shadow-[0_8px_30px_rgb(0,0,0,0.04)] relative overflow-hidden group/dna">
                        ...
                        <div className="space-y-4 relative z-10">
                            {[
                                { full: '抗压韧性 / Resilience', val: member.resilienceIndex || 50, color: 'from-emerald-400 to-emerald-500', icon: 'fa-shield-heart' },
                                { full: '协同率 / Synergy', val: member.synergyIndex || 50, color: 'from-indigo-400 to-indigo-500', icon: 'fa-people-arrows' },
                                { full: '严谨度 / Consistency', val: member.qcConsistency || 50, color: 'from-amber-400 to-amber-500', icon: 'fa-check-double' },
                            ].map(idx => (
                    ...
                    {member.mastery && member.mastery.length > 0 && (
                        <div className="p-4 rounded-[2rem] bg-slate-50/50 border border-slate-100 shadow-inner">
                            <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-3 flex items-center gap-2"><i className="fa-solid fa-microscope text-indigo-400"></i> 设备操控等级认证</p>
            ...
            <div className="mb-5 flex-1 flex flex-col min-h-0">
                <div className="flex justify-between items-center px-1 mb-2">
                    <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">当前交付序列</p>
            ...
            <div className="pt-4 border-t border-slate-100 shrink-0 relative">
                <div className="flex justify-between items-end px-1 mb-2">
                    <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest">任务带宽</span>
                                ...
                                >
                                    <header className="mb-3 border-l-4 border-indigo-600 pl-3 shrink-0 flex justify-between items-start">
                                        <div className="min-w-0 flex-1 pr-2">
                                            <h3 className="text-[11px] font-black text-slate-800 uppercase italic tracking-tighter">快速指派任务</h3>
                                            <p className="text-[7px] font-bold text-slate-400 uppercase tracking-widest mt-0.5">Manage for {member.name}</p>
                                        </div>
                                        <button
                                            onClick={() => {
                                                setReturnPath('team');
                                    ...
                                    <div className="space-y-4 flex flex-col min-h-0">
                                        <div>
                                            <label className="text-[8px] font-black text-slate-400 uppercase mb-1.5 block">选择目标课题</label>
                                            <select
                                        ...
                                        <div className="flex-1 flex flex-col min-h-0">
                                            <label className="text-[8px] font-black text-slate-400 uppercase mb-1.5 block">待办任务列表 (点选切换指派状态)</label>
                                            <div className="max-h-48 overflow-y-auto custom-scrollbar bg-slate-50 rounded-xl p-2 border border-slate-200 shadow-inner space-y-1.5">
                                                {allProjectTasks.length > 0 ? (
                                                    ...
                                                    <div className="py-4 flex flex-col items-center justify-center opacity-30 gap-2">
                                                        ...
                                                        <p className="text-[7px] font-black uppercase">暂无待处理任务</p>
                                                    </div>
                                                )}
                                            </div>
                                        </div>
                                    ...
                                    <div className="mt-4 flex gap-2">
                                        <button onClick={() => setShowAssignMenu(false)} className="flex-1 py-2 bg-slate-100 text-slate-500 rounded-xl text-[9px] font-black uppercase">取消</button>
                                        <button onClick={confirmTaskAssignment} className="flex-1 py-2 bg-indigo-600 text-white rounded-xl text-[9px] font-black uppercase shadow-lg">确认变更</button>
                                    </div>
                                </motion.div>
                            )}
                        </AnimatePresence>
