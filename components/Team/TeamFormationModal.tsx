...
const TeamFormationModal: React.FC<TeamFormationModalProps> = ({ show, onClose, projects, candidates }) => {
    ...
    const handleRunAnalysis = async () => {
        ...
        try {
            ...
            setResults(res);
            showToast({ message: "AI 组队推演已完成", type: 'success' });
        } catch (e) {
            showToast({ message: "分析失败，请检查配置", type: 'error' });
        } finally {
    ...
    const handleApplySquad = () => {
        ...
        showToast({ message: `已成功将 ${newMemberNames.length} 名推荐专家引入小队`, type: 'success' });
        onClose();
    };
                ...
                <header className="mb-10 flex justify-between items-end shrink-0">
                    <div>
                        <h3 className="text-3xl font-black text-slate-800 uppercase italic tracking-tighter flex items-center gap-4">
                            <i className="fa-solid fa-wand-magic-sparkles text-indigo-600"></i> AI 组队决策工作站
                        </h3>
                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.4rem] mt-2">Team Optimization & Synthesis Engine</p>
                    </div>
                ...
                <div className="flex-1 overflow-hidden flex flex-col min-h-0">
                    {results ? (
                        <div className="flex-1 grid grid-cols-12 gap-8 overflow-hidden animate-reveal">
                                            ...
                                            <div key={rec.id} className="bg-slate-50 p-6 rounded-[2.5rem] border border-slate-200 relative group">
                                                ...
                                                <div className="bg-white p-4 rounded-2xl border border-indigo-100 shadow-inner mb-4">
                                                    <p className="text-[9px] font-black text-indigo-600 uppercase mb-1.5 flex items-center gap-2">
                                                        <i className="fa-solid fa-robot"></i> 匹配逻辑
                                                    </p>
                            ...
                            <div className="col-span-4 flex flex-col gap-6">
                                <div className="bg-slate-900 text-white p-8 rounded-[3rem] shadow-2xl relative overflow-hidden flex-1 border border-white/5">
                                    ...
                                    <h4 className="text-[10px] font-black text-indigo-400 uppercase tracking-widest mb-6 italic">团队整体技能缺口 (SKILL GAP)</h4>
                                    ...
                                    <div className="mt-8 pt-6 border-t border-white/5">
                                        <button onClick={handleApplySquad} className="w-full py-4 bg-indigo-600 text-white rounded-2xl font-black uppercase text-xs shadow-xl shadow-indigo-500/20 hover:bg-emerald-600 transition-all active:scale-95">采纳推荐名单并更新课题</button>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="flex-1 flex flex-col items-center justify-center opacity-40 animate-pulse">
                            ...
                            <h4 className="text-xl font-black text-slate-800 uppercase italic tracking-widest">正在模拟多维人力矩阵对标...</h4>
                        </div>
                    ) : (
                        <div className="flex-1 flex flex-col items-center justify-center opacity-20 text-slate-400">
                             ...
                             <p className="text-sm font-black uppercase tracking-[0.4rem]">请选择课题以开启组队推演</p>
                        </div>
                    )}
                </div>
            </div>
