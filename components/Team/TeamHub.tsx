...
import HRMatrixView from './HRMatrixView';
...
const TeamHub: React.FC = () => {
    ...
    const [viewMode, setViewMode] = useState<'grid' | 'simulation' | 'topology' | 'matrix'>('grid');
    ...
    useEffect(() => {
        ...
        else if (hash.includes('/matrix')) setViewMode('matrix');
    ...
    const handleSaveMember = (updated: UserProfile) => {
        ...
        showToast({ message: t('team.memberSaved', { name: updated.name }), type: 'success' });
    };

    const handleOpenAdd = () => {
        setEditingMember({
            name: '', role: t('dashboard.expertise.assistant', { defaultValue: '研究助理' }), id: `SF-${Math.floor(1000 + Math.random() * 9000)}`,
        ...
        <div className="h-full flex flex-col gap-6 animate-reveal p-6 lg:p-10 bg-[#f8fafc] relative overflow-hidden">
            <header className="flex flex-col xl:flex-row justify-between items-end gap-6 shrink-0">
                <div className="flex items-center gap-5 mb-2 xl:mb-0">
                    {returnPath && (
                        ...
                        >
                            ...
                            <span className="text-[11px] font-black uppercase">{t('team.actions.back')}</span>
                        </button>
                    )}
                    ...
                    <div>
                        <h2 className={`text-2xl font-black italic uppercase tracking-tighter leading-none ${isLight ? 'text-slate-800' : 'text-white'}`}>{t('team.title')}</h2>
                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3rem] mt-2">{t('team.subtitle')}</p>
                    </div>
                </div>
                ...
                <div className={`flex flex-wrap items-center gap-3 p-2 rounded-[2rem] border shadow-sm w-full xl:w-auto ${isLight ? 'bg-white border-slate-200' : 'bg-slate-800/80 border-white/10'}`}>
                    <div className={`p-1 rounded-xl flex gap-1 shrink-0 ${isLight ? 'bg-slate-100' : 'bg-slate-900/50'}`}>
                        ...
                        >
                            <i className="fa-solid fa-grip"></i> <span className="hidden sm:inline">{t('team.views.grid')}</span>
                        </button>
                        <button 
                        ...
                        >
                            <i className="fa-solid fa-vr-cardboard"></i> <span className="hidden sm:inline">{t('team.views.simulation')}</span>
                        </button>
                        <button 
                        ...
                        >
                            <i className="fa-solid fa-circle-nodes"></i> <span className="hidden sm:inline">{t('team.views.topology')}</span>
                        </button>
                        <button 
                            onClick={() => navigate('team', undefined, 'matrix')} 
                            className={`px-4 py-2 rounded-lg text-[10px] font-black uppercase transition-all flex items-center gap-2 ${viewMode === 'matrix' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}
                        >
                            <i className="fa-solid fa-table-cells"></i> <span className="hidden sm:inline">{t('team.views.matrix')}</span>
                        </button>
                    </div>
                    ...
                    <div className="flex gap-2">
                        ...
                        >
                            <i className="fa-solid fa-wand-magic-sparkles"></i> {t('team.actions.smartTeam')}
                        </button>
                        <button 
                        ...
                        >
                            <i className="fa-solid fa-plus"></i> {t('team.actions.addMember')}
                        </button>
                    </div>
                    ...
                    <div className="relative group flex-1 min-w-[180px]">
                        ...
                        <input 
                            ...
                            placeholder={t('team.actions.searchPlaceholder')} 
                            value={search} 
            ...
            <div className="flex-1 overflow-hidden">
                {viewMode === 'simulation' ? (
                    ...
                    <HRMatrixView members={membersWithDynamicWorkload} projects={projects} />
                ) : viewMode === 'grid' ? (
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-6">
                            {filteredMembers.map(member => (
                                <MemberCard 
                                    ...
                                    onDelete={(m) => setConfirmModal({ show: true, title: t('team.deleteConfirm.title'), desc: t('team.deleteConfirm.desc', { name: m.name }), onConfirm: () => { setTeamMembers(prev => prev.filter(x => x.id !== m.id)); setConfirmModal(null); }})}
                                    onAssign={() => {}}
