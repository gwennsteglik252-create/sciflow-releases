...
const DETAIL_OPTIONS: { value: DetailLevel; label: string; desc: string }[] = [
  { value: 'concise', label: '精简', desc: '核心要点' },
  { value: 'standard', label: '标准', desc: '均衡适中' },
  { value: 'detailed', label: '详细', desc: '全面深入' },
];
  ...
  return (
    <div className="flex-1 flex flex-col gap-4 overflow-hidden min-h-0 animate-reveal">
      <div className="bg-slate-50 p-3 rounded-xl border border-slate-100 shrink-0">
        <label className="text-[9px] font-black text-slate-400 uppercase mb-2 block">补充背景信息 (CONTEXT)</label>
        <textarea
          ...
          placeholder="例如：Ag纳米线, AEM电解槽, 60度 1M KOH..."
        ...
        <div className="flex items-center gap-1 mt-2">
          <span className="text-[8px] font-black text-slate-400 uppercase shrink-0 mr-1">详细度</span>
      ...
      <div className="flex-1 flex flex-col min-h-0 bg-white border border-slate-200 rounded-xl overflow-hidden relative shadow-inner">
        <div className="bg-slate-50 px-3 py-2 border-b border-slate-100 flex justify-between items-center">
          <span className="text-[9px] font-black text-slate-500 uppercase">AI 分析结果</span>
          {analysisText && (
            <button
        ...
        <textarea
          ...
          placeholder="深度分析结果将显示在这里（点击下方单独生成或在标签1中联合生成）..."
          value={analysisText}
        ...
        {isAnalyzing && (
          <div className="absolute inset-0 bg-white/80 flex flex-col items-center justify-center backdrop-blur-[1px] z-10">
            ...
            <p className="text-[9px] font-black uppercase text-indigo-400">正在深度解读图片...</p>
          </div>
        )}
      </div>
