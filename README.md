...
#### 📌 战略立项
AI 驱动的课题孵化引擎。从选题灵感到立项报告，四步完成：**选题孵化 → 情报扫射 → 蓝图规划 → 立项评审**。自动生成科学假设、学术贡献预判、全球竞争态势散点图，以及专利/技术风险分析。

#### 🌐 行业动态
实时追踪目标领域的科研快讯、专利公开、行业政策和竞争对手动态。AI 自动聚合多源信息并按相关度排序，让你始终掌握领域最前沿。

#### 📋 研究看板
课题组的数字化指挥中心。一个仪表盘纵览所有在研课题的进度、里程碑、团队分工和预算消耗，支持项目计划和甘特图视图。
...
#### � 文献查找
内置 Web of Science 核心合集检索逻辑。支持按主题词、作者、期刊、时间范围等多维条件全网搜索，AI 自动评估文献质量并支持顶刊优先策略。

#### 📚 情报档案
文献的深度管理与研读中心。支持 PDF 全文导入、AI 摘要提取、多分类标签管理、核心工艺参数自动提取。每篇文献可进行「深度研读」和「知识沉淀」，AI 辅助生成转化建议。

#### 🧠 科研大脑
课题的知识图谱与关联网络。自动构建文献之间的引用关系、概念关联和知识脉络，以可视化节点图的方式呈现，帮助发现研究空白和潜在创新点。
...
#### 📂 课题工作流
基于 **TRL（技术成熟度）** 分级的课题管理系统。支持拓扑节点式课题拆解，每个节点包含实验组对比管理、参数追踪和 AI 对比分析报告。从基础研究到工程验证全程可追溯。

#### 🧪 实验矩阵
结构化的实验设计与执行管理。支持实验参数的联合对标设计（温度、浓度、配比等），自动生成实验编号，并跟踪每轮实验的执行状态和关键产出指标。

#### 🔀 实验路线
自然语言描述实验操作流程，AI 自动转化为标准化的实验流程节点图。支持多分支路线对比、步骤关联和操作规范自动生成。

#### 📈 工艺演化
基于 TRL 分级系统的工艺路线追踪。可视化展示技术从实验室到工厂的演化路径，记录每个阶段的关键工艺参数变化和性能指标。

#### 🏭 工业流程
面向工业化的流程模拟与优化。支持工艺参数的批量模拟、产线效率分析和规模化放大预测，帮助科研成果从实验室走向产业化。

#### ⭐ DOE 迭代
实验设计法 (Design of Experiments) 智能优化。AI 根据前期实验数据推荐下一轮最优参数组合，支持全因子设计、正交实验和响应面分析，加速实验迭代效率。
...
#### 🔬 实验表征
一站式表征数据分析中心，支持：
- **XRD**：智能物相检索（COD 数据库）、晶体结构匹配、消光规则验证
- **XPS**：化学态自动识别、元素价态分析、峰拟合
- **SEM/TEM**：AI 视觉模型检测颗粒尺寸分布、孔隙率、晶格间距
- **BET**：比表面积计算、孔径分布分析

#### ⚛️ 机理推演
AI 驱动的反应机理模拟器。基于实验数据和文献证据，推演可能的反应机理路径，支持催化机理图生成（双金属协同、电子转移、界面调控等），以及 AI 多角度专家辩论论证。

#### 📊 数据分析
实验数据的统计分析与可视化。支持异常值诊断、趋势预测、统计检验（t检验、ANOVA、回归分析等），AI 自动推荐最合适的分析方法。
...
#### 🎨 绘图中心 (Scientific Visual Engine)
专业科研绘图引擎，支持 **20+ 种图表类型**：
- **经典图表**：柱状图、折线图、饼图、雷达图、热力图
- **学术专用**：桑基图、弦图、时间线、机理图、演化路径图
- **高级功能**：多图联排、主题配色方案、LaTeX 标注、一键切换期刊风格

#### 📖 文献绘图
针对文献研读场景的专属绘图模式。可直接引用文献中的数据和图表，自动生成文献对比图、性能参数散点图等学术图表。

#### 🔄 综述绘图
面向综述论文的可视化工具。支持弦图（文献关联网络）、桑基图（技术演进路径）、时间线（研究发展脉络）等综述论文常用的高级图表类型。

#### 🖼️ 图表拼版
论文投稿前的图表组装工具。支持多图自由拼版、统一标注、自动编号（Fig. 1a, 1b, 1c...），导出为 SVG/PNG/PDF，分辨率高达 600 DPI。
...
#### ✍️ 写作工坊
学术论文的全功能编辑器。支持：
- **多栏排版**：单栏 / 双栏 / 三栏实时预览，适配 Nature、Science 等期刊模板
- **LaTeX 公式**：行内/行间公式实时渲染
- **AI 三档润色**：轻度纠错 / 中度优化 / 深度学术改写
- **引文管理**：一键插入文献引用，自动生成 References 列表
- **图表交叉引用**：Fig./Table/Eq. 智能编号，全文自动同步

#### 🎬 视频工坊
科研成果的多媒体展示工具。支持将实验过程、数据分析结果和关键发现制作成可分享的科研视频或动态演示。

#### 👥 人员矩阵
课题组的团队协作管理。支持成员角色分配、任务跟踪、在线协作状态、实验日志共享，以及基于项目的权限管理。

#### 📦 库存管理
实验室耗材与药品的智能管理。支持药品出入库记录、低库存预警、有效期追踪和采购清单自动生成。
