...
- 🔌 **开放 API** — 内置本地 HTTP Server，支持与 OpenClaw 等 AI Agent 深度联用
...
## 🔌 开放 API 与 AI Agent 集成

SciFlow Pro 内置本地 HTTP API Server，支持与 **OpenClaw**、**AutoGPT** 等外部 AI Agent 无缝协作。启动 SciFlow Pro 后，API 自动在 `http://127.0.0.1:17930` 上线。

### ⭐ 基础层：RESTful API

开箱即用的 RESTful 接口，覆盖系统状态、设置管理、项目/文献查询、AI 对话代理、文件操作：

```bash
# 检查 SciFlow Pro 是否运行
curl http://127.0.0.1:17930/api/v1/status

# 通过 SciFlow 的 AI 引擎发起查询
curl -X POST http://127.0.0.1:17930/api/v1/ai/chat \
  -H "Content-Type: application/json" \
  -d '{"prompt": "分析钙钛矿太阳能电池的最新研究趋势"}'

# 获取所有项目列表
curl http://127.0.0.1:17930/api/v1/projects
```

<details>
<summary><b>📋 完整 API 端点列表</b>（点击展开）</summary>

| 方法 | 路径 | 说明 |
|------|------|------|
| `GET` | `/api/v1/status` | 系统状态和版本信息 |
| `GET` | `/api/v1/openapi` | OpenAPI 3.0 文档 |
| `POST` | `/api/v1/token` | 生成 API 认证 Token |
| `GET` | `/api/v1/settings` | 读取应用设置 |
| `PUT` | `/api/v1/settings` | 更新应用设置 |
| `GET` | `/api/v1/projects` | 获取项目列表 |
| `GET` | `/api/v1/resources` | 获取情报档案 |
| `POST` | `/api/v1/resources` | 导入新文献 |
| `POST` | `/api/v1/ai/chat` | AI 对话（代理 UniversalAIAdapter） |
| `GET` | `/api/v1/ai/metrics` | AI 使用指标 |
| `POST` | `/api/v1/files/read` | 读取文件 |
| `POST` | `/api/v1/files/write` | 写入文件 |
| `POST` | `/api/v1/files/list` | 列出目录 |
| `POST` | `/api/v1/pipeline/setup` | 一键配置导入管道 |
| `POST` | `/api/v1/pipeline/ingest` | 批量导入文件 |
| `GET` | `/api/v1/workflow/rules` | 工作流规则列表 |
| `POST` | `/api/v1/workflow/rules` | 创建工作流规则 |
| `POST` | `/api/v1/workflow/trigger` | 手动触发规则 |
| `GET` | `/api/v1/workflow/logs` | 执行日志 |
| `GET` | `/api/v1/events` | 事件轮询 |
| `GET` | `/api/v1/events/stream` | SSE 实时事件流 |
| `POST` | `/api/v1/broadcast` | 广播自定义事件 |

</details>

### ⭐⭐ 中级：文件系统协作

一条命令让 AI Agent 配置好「论文/数据自动导入管道」：

```bash
curl -X POST http://127.0.0.1:17930/api/v1/pipeline/setup \
  -H "Content-Type: application/json" \
  -d '{"basePath": "~/SciFlow/incoming"}'
```

配置完成后，AI Agent 只需将文件放入目录，SciFlow 的 **WorkflowEngine** 自动执行链式动作：

```
📄 论文 PDF 投入 → AI 分析文件名 → 提取关键词 → 录入情报档案 → 发送通知
📊 CSV 数据投入 → AI 分析数据格式 → 推测实验类型 → 通知就绪
🔬 XRD 数据投入 → 自动导入资源 → 触发表征分析 → 通知前往表征中心
```

### ⭐⭐⭐ 高级：事件系统联动

SciFlow 内置事件缓冲区，自动收集工作流完成、AI 调用、设置变更等事件。AI Agent 可以通过**轮询**或 **SSE（Server-Sent Events）** 实时获取：

```bash
# 轮询最新事件
curl "http://127.0.0.1:17930/api/v1/events?limit=10"

# SSE 实时订阅（长连接）
curl -N http://127.0.0.1:17930/api/v1/events/stream
```

### 🔐 安全设计
- 🏠 仅监听 `127.0.0.1` 回环地址，**拒绝外部网络访问**
- 🔑 支持 Bearer Token 认证，Token 存储于 Electron userData
- 📴 未配置 Token 时允许所有本地请求（方便首次使用）
