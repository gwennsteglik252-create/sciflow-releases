...
import { PRO_MODEL, FAST_MODEL, NATIVE_IMAGE_GEN_MODEL, getAppSettings, getAISpeedMode } from './constants';
...
export class UniversalAIAdapter {
    ...
    private async executeDirectRoute(provider: string, params: any, taskType: string) {
        if (provider === 'gemini') {
            ...
            // 检测原生图像生成任务（responseModalities 含 IMAGE 或带 imageConfig）
            const isNativeImageGen = isImageTask || (
                Array.isArray(params.config?.responseModalities) &&
                params.config.responseModalities.some((m: string) => m === 'IMAGE' || m === 'image')
            );

            // 特殊任务：始终使用专用模型，不受用户配置影响
            // - 原生图像生成 → 用户配置的 imageModelName 或默认 NATIVE_IMAGE_GEN_MODEL
            // - TTS 音频 → 代码指定的原始模型
            // - 普通文本 → 允许用户配置覆盖
            const imageModel = this.settings?.imageModelName || NATIVE_IMAGE_GEN_MODEL;
            const rawModelToUse = isNativeImageGen
                ? imageModel
                : (isAudioTask)
    ...
    private async callOpenAIStyle(provider: string, config: any, params: any) {
        ...
        const buildRequestBody = (messages: any[]) => {
            ...
            // 当原始请求包含 JSON 模式时，自动启用 OpenAI 的 JSON 模式
            if (params.config?.responseMimeType === 'application/json') {
                body.response_format = { type: 'json_object' };
                // 将 responseSchema 作为 system 提示注入，引导模型输出正确结构
                if (params.config?.responseSchema) {
                    const schemaHint = `IMPORTANT: You must respond with valid JSON that follows this exact schema:\n${JSON.stringify(params.config.responseSchema, null, 2)}`;
                    const hasSystem = messages.some((m: any) => m.role === 'system');
                    if (hasSystem) {
                        messages.forEach((m: any) => {
                            if (m.role === 'system') m.content = `${m.content}\n\n${schemaHint}`;
                        });
                    } else {
