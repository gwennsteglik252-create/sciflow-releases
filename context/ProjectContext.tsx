...
import { generateFlowchartData, generateStepBOM } from '../services/gemini/flowchart';
import { normalizePhysicalConstants, auditStabilityPrediction } from '../components/Mechanism/physicsUtils';
...
export const ProjectProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const {
    ...
    notebookSession, updateNotebookSession,
    isSessionsReady
  ...
  // 3b. DOE 状态纠偏：防止 isCalculating 卡死
  useEffect(() => {
    if (isSystemReady && doeSession.isCalculating) {
      const hasDoeTask = activeTasks.some(t => t.id.startsWith('doe_'));
      if (!hasDoeTask) {
        console.log('[Context] Reconciling DOE state: no active task found, resetting isCalculating');
        updateDoeSession({ isCalculating: false });
      }
    }
  }, [isSystemReady, activeTasks, doeSession.isCalculating, updateDoeSession]);
  ...
  const runFlowchartModeling = async () => {
    await startGlobalTask({ id: 'flowchart_modeling', type: 'transformation', status: 'running', title: '工艺数字化建模' }, async () => {
      console.log('[FlowchartModeling] 调用 AI 生成...', { desc: flowchartSession.description.substring(0, 50), scale: flowchartSession.scaleFactor, trl: flowchartSession.targetTrl });
      ...
      console.log('[FlowchartModeling] AI 返回结果:', { hasSteps: !!res?.steps, stepCount: res?.steps?.length, title: res?.title });
      if (res?.steps) {
        updateFlowchartSession({
          currentFlowchart: {
            id: Date.now().toString(),
            title: res.title || 'AI 工艺建模方案',
            timestamp: new Date().toLocaleString(),
            originalDescription: flowchartSession.description,
            steps: res.steps.map((s: any, i: number) => {
              console.log(`[FlowchartModeling] Step ${i} 数据:`, { text: s.text?.substring?.(0, 30), desc: s.description?.substring?.(0, 30), doeAnchor: s.doeAnchor?.substring?.(0, 30), bomCount: s.bomItems?.length });
              return {
                ...s,
                id: s.id || `step_${i}_${Date.now()}`,
                text: (s.text && s.text.trim()) || (s.doeAnchor && s.doeAnchor.trim()) || `步骤 ${i + 1}`,
                description: (s.description && s.description.trim()) || (s.scalingInsight && s.safetyAlert ? `${s.scalingInsight}；${s.safetyAlert}` : s.scalingInsight || s.safetyAlert || ''),
              };
            }),
            scaleFactor: flowchartSession.scaleFactor,
            batchYieldGrams: res.batchYieldGrams || flowchartSession.scaleFactor,
        ...
        console.log('[FlowchartModeling] currentFlowchart 已成功设置');
      } else {
        console.warn('[FlowchartModeling] AI 返回数据中无 steps，原始响应:', JSON.stringify(res).substring(0, 200));
        throw new Error('AI 未返回有效的工艺步骤数据');
      }
    });
  };

  const runFlowchartBOM = async () => {
    if (!flowchartSession.currentFlowchart?.steps?.length) return;
    await startGlobalTask({ id: 'flowchart_bom', type: 'transformation', status: 'running', title: 'BOM 物料成本核算' }, async () => {
      const steps = flowchartSession.currentFlowchart!.steps.map(s => ({ text: s.text, description: s.description }));
      console.log('[FlowchartBOM] 发送给AI的步骤:', steps.map(s => s.text));
      const res = await generateStepBOM(steps, flowchartSession.scaleFactor);
      
      // 调试：暴露到 window 便于控制台检查
      (window as any).__lastBomResult__ = res;
      console.log('[FlowchartBOM] AI 返回完整数据:', JSON.stringify(res).substring(0, 500));
      console.log('[FlowchartBOM] res 类型:', typeof res, '| keys:', res ? Object.keys(res) : 'null');
      console.log('[FlowchartBOM] stepBOMs:', res?.stepBOMs?.length, '| 第一个stepBOM:', JSON.stringify(res?.stepBOMs?.[0]).substring(0, 200));
      
      if (res?.stepBOMs && flowchartSession.currentFlowchart) {
        // 判断 stepIndex 是 0-based 还是 1-based
        const isOneBased = res.stepBOMs.length > 0 && res.stepBOMs.every((b: any) => b.stepIndex >= 1);
        console.log('[FlowchartBOM] isOneBased:', isOneBased);
        
        const updatedSteps = flowchartSession.currentFlowchart.steps.map((step, idx) => {
          // 按 stepIndex 匹配（支持 0-based 和 1-based）
          let bomData = res.stepBOMs.find((b: any) =>
            b.stepIndex === idx || (isOneBased && b.stepIndex === idx + 1)
          );
          // 回退：如果没匹配到，按数组顺序直接取对应位置
          if (!bomData && idx < res.stepBOMs.length) {
            bomData = res.stepBOMs[idx];
          }
          const bomItems = bomData?.bomItems || [];
          console.log(`[FlowchartBOM] Step ${idx} (${step.text?.substring(0, 15)}): bomItems=${bomItems.length}, firstItem=${JSON.stringify(bomItems[0])?.substring(0, 100)}`);
          return { ...step, bomItems };
        });
        
        // 验证更新前的数据完整性
        const totalBomItems = updatedSteps.reduce((sum, s) => sum + (s.bomItems?.length || 0), 0);
        console.log(`[FlowchartBOM] ✅ 即将写入 session: totalBomItems=${totalBomItems}, steps=${updatedSteps.length}`);
        
        updateFlowchartSession({
          currentFlowchart: { ...flowchartSession.currentFlowchart, steps: updatedSteps }
        });
      } else {
        console.warn('[FlowchartBOM] ❌ AI 响应中无有效 stepBOMs');
        console.warn('[FlowchartBOM] 原始数据:', JSON.stringify(res).substring(0, 500));
      }
    });
  };
  ...
  const runMechanismAnalysis = async () => {
    await startGlobalTask({ id: `mechanism_${Date.now()}`, type: 'diagnose', status: 'running', title: `仿真解算: ${mechanismSession.material}` }, async () => {
      ...
      // 电化学稳定性物理审计：基于 Pourbaix 热力学纠偏 AI 输出
      const totalDoping = (mechanismSession.dopingConcentration || 0) + (mechanismSession.coDopingConcentration || 0);
      result.stabilityPrediction = auditStabilityPrediction(
        result.stabilityPrediction,
        mechanismSession.material,
        mechanismSession.pH,
        mechanismSession.potential,
        mechanismSession.dopingElement,
        totalDoping
      );
  ...
  const handleRunWeeklyReportTask = async (project: ResearchProject, type: any = 'weekly', dateRange?: { start: string; end: string }) => {
    return startGlobalTask({ id: `weekly_${project.id}`, type: 'weekly_report', status: 'running', title: '生成研究报告' }, async () => {
      ...
      // 根据用户指定的日期范围或报告类型默认范围过滤日志
      let cutoffStart: number;
      let cutoffEnd: number;
      if (dateRange?.start && dateRange?.end) {
        cutoffStart = new Date(dateRange.start).getTime();
        cutoffEnd = new Date(dateRange.end + 'T23:59:59').getTime();
      } else {
        const now = Date.now();
        const rangeDays = type === 'weekly' ? 7 : type === 'monthly' ? 30 : 365;
        cutoffStart = now - rangeDays * 24 * 60 * 60 * 1000;
        cutoffEnd = now;
      }
      ...
      const filteredLogs = allLogs.filter(log => {
        ...
        try {
          ...
          return !isNaN(logTime) && logTime >= cutoffStart && logTime <= cutoffEnd;
        } catch {
  ...
  const contextValue = useMemo(() => ({
    ...
    updateFlowchartSession, runFlowchartModeling, runFlowchartBOM,
    ...
    updateDoeSession, runDoeInference,
    updateInceptionSession, runInceptionBrainstorm, runInceptionResearch, runInceptionBlueprint, runInceptionReview,
    updateDataAnalysisSession, updateWritingSession, updateStructuralSession, updateTimelineSession, updateVisionSession, updateNotebookSession,
  ...
  const uiContextValue = useMemo<UIContextType>(() => ({
    ...
    toast, modals, isAiCliOpen, aiCliHistory, isVoiceMode, searchQuery, aiStatus,
