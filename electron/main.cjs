...
function getMachineId() {
  const raw = `${import_node_process.default.platform}-${require("os").hostname()}-${require("os").userInfo().username}-SciFlowPro`;
...
function createWindow() {
  const win = new import_electron.BrowserWindow({
    ...
    webPreferences: {
      preload: import_path.default.join(_dirname, "preload.cjs"),
      nodeIntegration: false,
      contextIsolation: true,
      backgroundThrottling: false
    }
  });
  const isDev = import_node_process.default.env.NODE_ENV === "development" || !import_electron.app.isPackaged;
  if (isDev) {
    win.loadURL("http://localhost:5173");
  } else {
    win.loadFile(import_path.default.join(_dirname, "../dist/index.html"));
  }
}
import_electron.app.whenReady().then(() => {
  import_electron.powerSaveBlocker.start("prevent-app-suspension");
  ...
  const verifyToken = (req) => {
    const tokenFilePath = import_path.default.join(import_electron.app.getPath("userData"), "api_token.txt");
    let storedToken = "";
    try {
      if (import_fs.default.existsSync(tokenFilePath)) {
        storedToken = import_fs.default.readFileSync(tokenFilePath, "utf-8").trim();
      }
    } catch {
    }
  ...
  const generateApiToken = () => {
    ...
    const tokenFilePath = import_path.default.join(import_electron.app.getPath("userData"), "api_token.txt");
    import_fs.default.writeFileSync(tokenFilePath, token, "utf-8");
    return token;
  };
  const routeHandlers = {
    // ── 系统 ──
    ...
    "POST /api/v1/files/read": {
      handler: async (req, res) => {
        ...
        if (!import_fs.default.existsSync(payload.path)) {
          return jsonRes(res, 404, { ok: false, error: "File not found" });
        }
        try {
          const ext = import_path.default.extname(payload.path).toLowerCase();
          const isBinary = [".png", ".jpg", ".jpeg", ".webp", ".gif", ".pdf"].includes(ext);
          if (isBinary) {
            const data = await import_fs.default.promises.readFile(payload.path, { encoding: "base64" });
            jsonRes(res, 200, { ok: true, data, encoding: "base64", mimeType: ext === ".pdf" ? "application/pdf" : `image/${ext.replace(".", "")}` });
          } else {
            const data = await import_fs.default.promises.readFile(payload.path, "utf-8");
    ...
    "POST /api/v1/files/write": {
      handler: async (req, res) => {
        ...
        try {
          const dir = import_path.default.dirname(payload.path);
          if (!import_fs.default.existsSync(dir)) {
            import_fs.default.mkdirSync(dir, { recursive: true });
          }
          const encoding = payload.encoding === "base64" ? "base64" : "utf-8";
          import_fs.default.writeFileSync(payload.path, payload.content, { encoding });
    ...
    "POST /api/v1/files/list": {
      handler: async (req, res) => {
        ...
        try {
          const files = import_fs.default.readdirSync(payload.path);
          const items = files.map((file) => {
            const fullPath = import_path.default.join(payload.path, file);
            const stats = import_fs.default.statSync(fullPath);
            return {
              name: file,
              path: fullPath,
              isDirectory: stats.isDirectory(),
              ext: import_path.default.extname(file),
              size: stats.size,
    ...
    "POST /api/v1/pipeline/setup": {
      handler: async (req, res) => {
        ...
        if (!import_fs.default.existsSync(payload.basePath)) {
          try {
            import_fs.default.mkdirSync(payload.basePath, { recursive: true });
          } catch (e) {
            return jsonRes(res, 500, { ok: false, error: `Cannot create directory: ${e.message}` });
          }
        }
        const watchResult = await new Promise((resolve) => {
          const stats = import_fs.default.statSync(payload.basePath);
          ...
          const watcher = import_fs.default.watch(payload.basePath, (eventType, filename) => {
            if (filename && eventType === "rename") {
              const fullPath = import_path.default.join(payload.basePath, filename);
              if (import_fs.default.existsSync(fullPath)) {
                ...
                wins.forEach((w) => w.webContents.send("fs-notification", {
                  ...
                  path: fullPath,
                  name: filename,
                  ext: import_path.default.extname(filename),
    ...
    "POST /api/v1/characterization/xrd/analyze": {
      handler: async (req, res) => {
        ...
        if (!rawDataText && payload.filePath) {
          try {
            rawDataText = import_fs.default.readFileSync(payload.filePath, "utf-8");
          } catch (e) {
...
4. **\u5B9E\u9A8C\u5EFA\u8BAE**: \u4E0B\u4E00\u6B65\u8868\u5F81\u5EFA\u8BAE`;
  ...
  const apiServer = import_http.default.createServer(async (req, res) => {
    ...
    if (req.method === "GET" && urlObj.pathname.startsWith("/api/v1/settings/")) {
      ...
      jsonRes(res, 200, { ok: true, key, value: value !== void 0 ? value : null });
      return;
    }
    if (urlObj.pathname === "/" || urlObj.pathname === "") {
      jsonRes(res, 200, {
        ok: true,
        message: "\u{1F9EA} SciFlow Pro API Server is running!",
        version: import_electron.app.getVersion(),
...
\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550`);
    ...
    console.log(`  \u{1F511} Generate Token: POST http://127.0.0.1:${API_PORT}/api/v1/token`);
    console.log(`\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550
`);
  });
  ...
  import_electron.ipcMain.handle("download-update", async () => {
    try {
      import_electron.shell.openExternal(RELEASE_URL);
      return { success: true };
    } catch (err) {
  ...
  import_electron.ipcMain.handle("get-license-state", () => {
    return getLicenseState();
  });
  import_electron.ipcMain.handle("activate-license", async (_event, code) => {
  ...
  import_electron.ipcMain.handle("reset-trial", () => {
    const isDev2 = import_node_process.default.env.NODE_ENV === "development" || !import_electron.app.isPackaged;
...
import_electron.ipcMain.handle("capture-page", async (_event, rect) => {
...
import_electron.app.on("window-all-closed", () => {
  if (import_node_process.default.platform !== "darwin") import_electron.app.quit();
});
var lastUsedPaths = {};
import_electron.ipcMain.handle("select-local-file", async (_event, arg) => {
  ...
  if (contextKey) {
    lastUsedPaths[contextKey] = import_path.default.dirname(filePath);
  }
  if (multiple) {
  ...
  return { name: import_path.default.basename(filePath), path: filePath };
});
...
import_electron.ipcMain.handle("open-path", async (_event, targetPath) => {
  if (import_fs.default.existsSync(targetPath)) {
    return await import_electron.shell.openPath(targetPath);
  }
  return "File not found";
});
import_electron.ipcMain.handle("save-file", async (_event, opts) => {
  const result = await import_electron.dialog.showSaveDialog({
    defaultPath: opts.defaultPath ? import_path.default.join(opts.defaultPath, opts.name) : opts.name,
  ...
  try {
    import_fs.default.writeFileSync(result.filePath, opts.content);
...
import_electron.ipcMain.handle("read-file", async (_event, targetPath) => {
  if (!import_fs.default.existsSync(targetPath)) return null;
  try {
    const ext = import_path.default.extname(targetPath).toLowerCase();
    if (ext === ".pdf") {
      const data = await import_fs.default.promises.readFile(targetPath, { encoding: "base64" });
      return { mimeType: "application/pdf", data };
    } else if ([".png", ".jpg", ".jpeg", ".webp"].includes(ext)) {
      const data = await import_fs.default.promises.readFile(targetPath, { encoding: "base64" });
      return { mimeType: `image/${ext.replace(".", "")}`, data };
    } else {
      const data = await import_fs.default.promises.readFile(targetPath, "utf-8");
...
import_electron.ipcMain.handle("show-item-in-folder", async (_event, targetPath) => {
  if (import_fs.default.existsSync(targetPath)) {
    import_electron.shell.showItemInFolder(targetPath);
  }
});
var watchers = /* @__PURE__ */ new Map();
import_electron.ipcMain.handle("watch-directory", async (event, targetPath) => {
  if (!import_fs.default.existsSync(targetPath)) return { success: false, error: "Directory not found" };
  ...
  try {
    const watcher = import_fs.default.watch(targetPath, (eventType, filename) => {
      if (filename && eventType === "rename") {
        const fullPath = import_path.default.join(targetPath, filename);
        if (import_fs.default.existsSync(fullPath)) {
...
import_electron.ipcMain.handle("list-directory", async (_event, targetPath) => {
  if (!import_fs.default.existsSync(targetPath)) return [];
  try {
    const files = import_fs.default.readdirSync(targetPath);
    return files.map((file) => {
      const fullPath = import_path.default.join(targetPath, file);
      const stats = import_fs.default.statSync(fullPath);
      return {
        name: file,
        path: fullPath,
...
import_electron.ipcMain.handle("unwatch-directory", async (_event, targetPath) => {
...
import_electron.ipcMain.handle("http-request", async (_event, payload) => {
  try {
...
import_electron.ipcMain.handle("open-popout-window", async (_event, payload) => {
  const { viewId, title } = payload;
  const popout = new import_electron.BrowserWindow({
    width: 1e3,
    height: 700,
  ...
  if (isDev) {
    ...
    popout.loadFile(import_path.default.join(_dirname, "../dist/index.html"), {
      query: { popout: viewId }
    });
  }
  return { success: true };
