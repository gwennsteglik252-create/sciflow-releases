...
function fetchWithCookie(targetUrl, cookie, maxRedirects = 5) {
  return new Promise((resolve) => {
    const doRequest = (reqUrl, redirects) => {
      ...
      const mod = parsed.protocol === "https:" ? import_https.default : import_http.default;
      const req = mod.get(reqUrl, {
        ...
        if (res.statusCode && res.statusCode >= 300 && res.statusCode < 400 && res.headers.location) {
          const next = res.headers.location.startsWith("http") ? res.headers.location : import_url.default.resolve(reqUrl, res.headers.location);
...
function registerIpcHandlers(_dirname2) {
  ...
  import_electron3.ipcMain.handle("open-docx-dialog", async () => {
    const win = import_electron3.BrowserWindow.getFocusedWindow();
    const result = await import_electron3.dialog.showOpenDialog(win, {
      properties: ["openFile"],
      filters: [
  ...
  import_electron3.ipcMain.handle("save-docx", async (_event, opts) => {
    ...
    try {
      ...
      return { success: true, filePath: targetPath, name: import_path2.default.basename(targetPath) };
    } catch (err) {
      console.error("Failed to save docx:", err);
      return { success: false, error: err.message };
    }
  });
  import_electron3.ipcMain.handle("open-in-word", async () => {
    const win = import_electron3.BrowserWindow.getFocusedWindow();
    const result = await import_electron3.dialog.showOpenDialog(win, {
      properties: ["openFile"],
      filters: [
    ...
    if (result.canceled || result.filePaths.length === 0) return null;
    const filePath = result.filePaths[0];
    const errMsg = await import_electron3.shell.openPath(filePath);
    if (errMsg) {
      console.error("Failed to open in Word:", errMsg);
      return { success: false, error: errMsg };
    }
    return { success: true, filePath, name: import_path2.default.basename(filePath) };
  });
  import_electron3.ipcMain.handle("create-new-docx", async () => {
    const win = import_electron3.BrowserWindow.getFocusedWindow();
    ...
    try {
      if (!import_fs2.default.existsSync(filePath)) {
        await import_fs2.default.promises.writeFile(filePath, "");
      }
      const errMsg = await import_electron3.shell.openPath(filePath);
  ...
  import_electron3.ipcMain.handle("open-external-url", async (_event, url2) => {
    if (url2 && (url2.startsWith("http:") || url2.startsWith("https:"))) {
      await import_electron3.shell.openExternal(url2);
      return { success: true };
    }
    return { success: false, error: "Invalid URL" };
  });
  import_electron3.ipcMain.handle("find-in-page", async (_event, text) => {
    const win = import_electron3.BrowserWindow.getFocusedWindow();
  ...
  import_electron3.ipcMain.handle("stop-find-in-page", async () => {
    const win = import_electron3.BrowserWindow.getFocusedWindow();
    if (!win) return;
    try {
  ...
  const DEFAULT_PDF_DIR = import_path2.default.join(import_electron4.app.getPath("documents"), "SciFlow-PDFs");
  const PDF_DIR_KEY = "sciflow_pdf_download_dir";
  import_electron3.ipcMain.handle("get-pdf-download-dir", () => {
    const configPath = import_path2.default.join(import_electron4.app.getPath("userData"), "pdf-config.json");
    try {
      if (import_fs2.default.existsSync(configPath)) {
        const config = JSON.parse(import_fs2.default.readFileSync(configPath, "utf-8"));
        if (config.downloadDir && import_fs2.default.existsSync(config.downloadDir)) {
  ...
  import_electron3.ipcMain.handle("set-pdf-download-dir", (_event, dir) => {
    const configPath = import_path2.default.join(import_electron4.app.getPath("userData"), "pdf-config.json");
    try {
      let config = {};
      if (import_fs2.default.existsSync(configPath)) {
  ...
  import_electron3.ipcMain.handle("download-pdf", async (_event, payload) => {
    const { url: url2, filename } = payload;
    if (!url2 || !filename) return { success: false, error: "\u7F3A\u5C11 URL \u6216\u6587\u4EF6\u540D" };
    const configPath = import_path2.default.join(import_electron4.app.getPath("userData"), "pdf-config.json");
    ...
    if (!import_fs2.default.existsSync(downloadDir)) {
      import_fs2.default.mkdirSync(downloadDir, { recursive: true });
    }
    const filePath = import_path2.default.join(downloadDir, filename);
    if (import_fs2.default.existsSync(filePath)) {
      return { success: true, filePath };
    }
    const quickResult = await new Promise((resolve) => {
      const request = import_electron3.net.request({ url: url2, method: "GET", redirect: "follow" });
      ...
      request.on("response", (response) => {
        if (response.statusCode !== 200) {
          done({ success: false, error: `HTTP ${response.statusCode}` });
          return;
        }
        const ct = (response.headers["content-type"] || "").toLowerCase();
        if (ct.includes("text/html")) {
          done({ success: false, error: "HTML" });
          return;
        }
        const chunks = [];
        ...
        response.on("error", (err) => done({ success: false, error: err.message }));
      });
      request.on("error", (err) => done({ success: false, error: err.message }));
      setTimeout(() => {
        request.abort();
    ...
    return new Promise((resolve) => {
      ...
      const downloadWin = new import_electron3.BrowserWindow({
        show: false,
        width: 1024,
        height: 768,
        webPreferences: {
          ...
          contextIsolation: true,
          plugins: false
          // 禁用 PDF 插件，确保 PDF 不被内置 viewer 吞噬
        }
      });
      ...
      downloadWin.webContents.debugger.on("message", async (_event2, method, params) => {
        if (resolved) return;
        if (method === "Fetch.requestPaused") {
          ...
          if (responseStatusCode === 200 && (ct.includes("application/pdf") || ct.includes("application/octet-stream"))) {
            try {
              ...
              const buf = body.base64Encoded ? Buffer.from(body.body, "base64") : Buffer.from(body.body);
              ...
              if (buf.length >= 10240 && buf.slice(0, 5).toString("ascii") === "%PDF-") {
                import_fs2.default.writeFileSync(filePath, buf);
                console.log(`[download-pdf] \u2705 Saved PDF: ${buf.length} bytes \u2192 ${filePath}`);
                try {
                  downloadWin.webContents.debugger.sendCommand("Fetch.failRequest", { requestId, reason: "Aborted" });
                } catch {
                }
                done({ success: true, filePath });
                return;
      ...
      console.log(`[download-pdf] Loading URL in BrowserWindow: ${url2}`);
      downloadWin.loadURL(url2).catch((e) => {
  ...
  const wytsgSession = import_electron3.session.fromPartition("persist:wytsg");
  function fetchWithWytsgSession(targetUrl, maxRedirects = 5) {
    return new Promise((resolve) => {
      const doRequest = (reqUrl, redirects) => {
        if (redirects <= 0) {
          resolve(null);
        ...
        const req = import_electron3.net.request({
          url: reqUrl,
        ...
        req.on("response", (res) => {
          if (res.statusCode >= 300 && res.statusCode < 400 && res.headers.location) {
            ...
            const next = location.startsWith("http") ? location : import_url.default.resolve(reqUrl, location);
            doRequest(next, redirects - 1);
            return;
          }
          let body = "";
  ...
  import_electron3.ipcMain.handle("open-wytsg-login", async () => {
    return new Promise((resolve) => {
      const loginUrl = "http://www.wytsg.com/e/member/login/";
      const loginWin = new import_electron3.BrowserWindow({
        width: 900,
        height: 700,
  ...
  import_electron3.ipcMain.handle("get-wytsg-cookies", async () => {
    try {
  ...
  import_electron3.ipcMain.handle("wytsg-download-pdf", async (_event, doi) => {
    try {
      ...
      let mirrorUrl = "https://www.tesble.com";
      if (entryHtml) {
        const mirrorMatch = entryHtml.match(/href=["'](https?:\/\/[^"']*(?:sci-hub|tesble|sci\.hub)[^"']*)/i) || entryHtml.match(/href=["'](https?:\/\/(?:www\.)?[a-z0-9.-]+\.[a-z]{2,})/gi);
        if (mirrorMatch) {
          mirrorUrl = (mirrorMatch[1] || mirrorMatch[0].replace(/^href=["']/, "")).replace(/\/+$/, "");
        }
      }
      const mirrorPageUrl = `${mirrorUrl}/${cleanDoi}`;
      const pageHtml = await fetchWithCookie(mirrorPageUrl, "");
      if (!pageHtml) {
  ...
  const wytsgCredPath = import_path2.default.join(import_electron4.app.getPath("userData"), "wytsg-cred.json");
  import_electron3.ipcMain.handle("save-wytsg-credentials", async (_event, payload) => {
    try {
      const data = {
        u: Buffer.from(payload.username).toString("base64"),
        p: Buffer.from(payload.password).toString("base64")
      };
      import_fs2.default.writeFileSync(wytsgCredPath, JSON.stringify(data));
      return { success: true };
    } catch (err) {
      return { success: false, error: err.message };
    }
  });
  import_electron3.ipcMain.handle("get-wytsg-credentials", async () => {
    try {
      if (!import_fs2.default.existsSync(wytsgCredPath)) return { saved: false };
      const data = JSON.parse(import_fs2.default.readFileSync(wytsgCredPath, "utf-8"));
      return {
        saved: true,
  ...
  import_electron3.ipcMain.handle("auto-wytsg-login-captcha", async () => {
    try {
      ...
      return new Promise((resolve) => {
        ...
        req.on("response", (res) => {
          ...
          res.on("end", () => {
            ...
            const contentType = (Array.isArray(res.headers["content-type"]) ? res.headers["content-type"][0] : res.headers["content-type"]) || "image/png";
  ...
  import_electron3.ipcMain.handle("auto-wytsg-login-submit", async (_event, payload) => {
    try {
        ...
        req.on("response", (res) => {
          ...
          res.on("end", () => {
            ...
            if (isRedirect && !redirectToLogin) {
              ...
              const hasJsRedirect = asciiLower.includes("self.location") || asciiLower.includes("location.href") || asciiLower.includes("window.location") || asciiLower.includes("location.replace");
...
// electron/main.ts
var import_meta = {};
var _filename = typeof __filename !== "undefined" ? __filename : (0, import_url2.fileURLToPath)(import_meta.url);
...
import_electron5.app.commandLine.appendSwitch("disable-background-timer-throttling");
function createWindow() {
  ...
  const isDev = import_node_process.default.env.NODE_ENV === "development" || !import_electron5.app.isPackaged;
  if (isDev) {
...
import_electron5.app.whenReady().then(() => {
  import_electron5.powerSaveBlocker.start("prevent-app-suspension");
  const { session: session2 } = require("electron");
  session2.defaultSession.webRequest.onBeforeSendHeaders(
    { urls: ["https://*/*"] },
    (details, callback) => {
  ...
  session2.defaultSession.webRequest.onHeadersReceived(
    { urls: ["https://*/*"] },
    (details, callback) => {
  ...
  createWindow();
  try {
    const wordSession = session2.fromPartition("persist:word-online");
  ...
  import_node_process.default.on("uncaughtException", (err) => {
    ...
    if (msg.includes("Render frame was disposed") || msg.includes("ERR_ABORTED") || msg.includes("GUEST_VIEW_MANAGER") || msg.includes("Object has been destroyed")) {
      console.warn("[Main] Suppressed webview error:", msg.slice(0, 120));
      return;
    }
    console.error("[Main] Uncaught exception:", err);
  });
  import_electron5.app.on("web-contents-created", (_event, contents) => {
    if (contents.getType() === "webview") {
      contents.setWindowOpenHandler(({ url: url2 }) => {
        console.log("[Main] webview window-open \u2192", url2);
        setImmediate(() => {
          try {
            contents.loadURL(url2);
          } catch {
          }
        });
        return { action: "deny" };
      ...
      contents.on("will-navigate", (event, url2) => {
        console.log("[Main] webview will-navigate \u2192", url2);
      });
    }
  });
  ...
  const verifyToken = (req) => {
    const tokenFilePath = import_path3.default.join(import_electron5.app.getPath("userData"), "api_token.txt");
    let storedToken = "";
    try {
  ...
  const routeHandlers = {
    ...
    "GET /api/v1/openapi": {
      handler: async (_req, res) => {
        jsonRes(res, 200, {
          openapi: "3.0.3",
          info: { title: "SciFlow Pro Local API", version: import_electron5.app.getVersion(), description: "RESTful API for external AI agents" },
          ...
          const watcher = import_fs3.default.watch(payload.basePath, (eventType, filename) => {
            if (filename && eventType === "rename") {
              ...
              if (import_fs3.default.existsSync(fullPath)) {
                import_electron5.BrowserWindow.getAllWindows().forEach((w) => w.webContents.send("fs-notification", {
                  type: "added",
                  path: fullPath,
                  name: filename,
...
4. **\u5B9E\u9A8C\u5EFA\u8BAE**: \u4E0B\u4E00\u6B65\u8868\u5F81\u5EFA\u8BAE`;
  ...
  const apiServer = import_http2.default.createServer(async (req, res) => {
    setHeaders(res);
    ...
    if (urlObj.pathname === "/" || urlObj.pathname === "") {
      jsonRes(res, 200, { ok: true, message: "\u{1F9EA} SciFlow Pro API Server is running!", version: import_electron5.app.getVersion(), docs: `http://127.0.0.1:${API_PORT}/api/v1/openapi`, endpoints: Object.keys(routeHandlers) });
      return;
    }
...
import_electron5.app.on("web-contents-created", (_event, contents) => {
  const isMainWindowContents = () => {
    try {
      const win = import_electron5.BrowserWindow.fromWebContents(contents);
      return win && contents.getType() !== "webview" && win.webContents === contents && win.webContents.getURL().includes("localhost:5173");
    } catch {
      return false;
    }
  };
  contents.on("will-navigate", (event, url2) => {
    const isExternal = url2.startsWith("http:") || url2.startsWith("https:");
    const isLocalhost = url2.includes("localhost:") || url2.includes("127.0.0.1:");
    if (isExternal && !isLocalhost && isMainWindowContents()) {
      event.preventDefault();
      import_electron5.shell.openExternal(url2);
    }
  });
  contents.setWindowOpenHandler(({ url: url2 }) => {
...
import_electron5.app.on("window-all-closed", () => {
  if (import_node_process.default.platform !== "darwin") import_electron5.app.quit();
});
